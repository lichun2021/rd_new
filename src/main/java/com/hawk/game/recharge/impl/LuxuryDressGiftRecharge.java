...
import com.hawk.activity.type.impl.luxurydress.LuxuryDressActivity;
 ...
 *
 */
public class LuxuryDressGiftRecharge extends AbstractGiftRecharge {
	
	@Override
	public boolean detailGiftBuyCheck(Player player, PayGiftCfg giftCfg, RechargeBuyItemRequest req, int protocol) {
		Optional<ActivityBase> opActivity = ActivityManager.getInstance().getActivity(Activity.ActivityType.LUXURY_DRESS_391_VALUE);
		if (!opActivity.isPresent()) {
			player.sendError(protocol, Status.Error.ACTIVITY_NOT_OPEN_VALUE, 0);
			return false;
		}
		
		LuxuryDressActivity activity = (LuxuryDressActivity)opActivity.get();
	...
	public int getGiftType() {
		return RechargeType.LUXURY_DRESS_ACTIVITY;
	}
