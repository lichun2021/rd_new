...
public class GsApp extends HawkApp {
	...
	public boolean init(String cfg) {
		...
		if (appCfg.isDebug()) {
			...
			if (!AsmCheckUtil.checkJedisUnclose(
					Arrays.asList("com.hawk.game.global.LocalRedis", "com.hawk.game.global.RedisProxy"))) {
				return false;
			}
		}
		...
		if (!CSRedisProxyPool.getInstance().init()) {
			return false;
		}
		...
		// 初始化sdk服务
		SDKManager.initInstance(SDKConst.SDKType.MSDK, appCfg.getHttpUrlTimeout(),
		...
		if (!Ann7Party384Service.getInstance().init()) {
			return false;
		}
		...
		// 里程碑任务服务类
		if (!RookieMilestoneService.getInstance().init()) {
			return false;
		}
		...
		if (!XQHXSeasonService.getInstance().init()) {
			HawkLog.errPrintln("XQHXSeasonService init failed");
			return false;
		}
		...
		if (!FindStoreHouseService.getInstance().init()) {
			HawkLog.errPrintln("FindStoreHouse init failed");
			return false;
		}

		// CDK初始化
		if (!HawkCdkService.getInstance().install(appCfg.getGameId(), appCfg.getServerId(), appCfg.getCdkHost(),
				appCfg.getHttpUrlTimeout())) {
			return false;
		}
		...
		if (!GsConfig.getInstance().isDebug() && GsConfig.getInstance().isZonineEnable()) {
			if (!ZonineSDK.getInstance().init(appCfg.getGameId(), appCfg.getAreaId(), appCfg.getServerId(),
					appCfg.getZonineAddrList())) {
				return false;
			}

			ZonineSDK.getInstance()
					.addMergeServers(GlobalData.getInstance().getMergeServerList(GsConfig.getInstance().getServerId()));
		}
		...
		// 天奢装扮技能初始化
		LuxuryDressService.getInstance().init();
	...
	private void waitBatchStatus() {
		...
		do {
			...
			if (count < GsConfig.getInstance().getMaxBatchServerCount()) {
				...
				RedisProxy.getInstance().getRedisSession().hSet("server_batch_status", appCfg.getServerId(),
						"" + batchStatus, 300);
				return;
			}
			HawkOSOperator.osSleep(5000);
	...
	public void flushActivityTable() {
		try {
			// 拆服表
			ConfigIterator<SeparateActivityCfg> iter = HawkConfigManager.getInstance()
					.getConfigIterator(SeparateActivityCfg.class);
			...
			RedisProxy.getInstance().getRedisSession().sAdd("merge_activity_tables", 0,
					mergeTables.toArray(new String[mergeTables.size()]));
		} catch (Exception e) {
			HawkException.catchException(e);
		}
	}
	...
	private boolean initAppObjMan() {
		...
		createObj(HawkXID.valueOf(GsConst.ObjType.MANAGER, GsConst.ObjId.XQHX_SEASON));
		...
		createObj(HawkXID.valueOf(GsConst.ObjType.MANAGER, GsConst.ObjId.ANNY_PARTY));
		createObj(HawkXID.valueOf(GsConst.ObjType.MANAGER, GsConst.ObjId.FIND_STORE_HOUSE));
		return true;
	}
	...
	public boolean updateSlienceWords() {
		try {
			...
			ConfigIterator<SilenceWordCfg> cfgIt = HawkConfigManager.getInstance()
					.getConfigIterator(SilenceWordCfg.class);
	...
	public void reportException(Throwable throwable, Object... params) {
		if (GsConfig.getInstance().isDebug() || GsConfig.getInstance().isSpecialServer()) {
			if (GsConfig.getInstance().isReportException2Feishu()) {
				...
				ExceptionReportUtil.reportException2Feishu(GsConfig.getInstance().getTargetCehuaUrl(), throwable,
						params);
				return;
			}
		}
	...
	protected void onClosed() {
		try {
			...
			GlobalData.getInstance().resetServerStatus(GsConfig.getInstance().getServerId(), registerCount, 0,
					HawkTime.getSeconds());
	...
	public void onShutdown() {
		try {
			...
			Ann7Party384Service.getInstance().onShutdown();
	...
	protected HawkAppObj onCreateObj(HawkXID xid) {
		...
		if (xid.getType() == GsConst.ObjType.PLAYER) {
			...
			} else if (xid.getId() == GsConst.ObjId.ANNY_PARTY) {
				appObj = new Ann7Party384Service(xid);
			} else if (xid.getId() == GsConst.ObjId.XQHX_SEASON) {
				appObj = new XQHXSeasonService(xid);
			} else if (xid.getId() == GsConst.ObjId.FIND_STORE_HOUSE) {
				appObj = new FindStoreHouseService(xid);
			}
	...
	public boolean onSessionOpened(HawkSession session) {
		if (!isServerOpened()) {
			// 记录未开服
			HawkLog.logPrintln("server not opened, server will open at: {}",
					HawkTime.formatTime(GameUtil.getServerOpenTime()));
	...
	private void registerTickable() {
		...
		addTickable(new HawkPeriodTickable(redisCheckPeriod) {
			...
			public void onPeriodTick() {
				String serverId = GsConfig.getInstance().getServerId();
				long retCode = RedisProxy.getInstance().getRedisSession().hSet("server_identify", serverId,
						getServerIdentify());
					...
					synchronized (waitSessionQueue) {
						...
						while (diff-- > 0) {
							...
							if (waitingSession != null) {
								...
								waitingSession
										.sendProtocol(HawkProtocol.valueOf(HP.code.LOGIN_PREPARE_SYNC_S, builder));
								...
								HawkLog.logPrintln(
										"waitlogin status, openid: {}, allow login, session: {}, onlineCnt: {}",
										waitingSession.getUserObject("openid"), waitingSession,
										GlobalData.getInstance().getOnlineUserCount());
							}
						}
					}
				}
		...
		if (session != null && player != null && player.getData() != null) {
			...
			if (!HawkOSOperator.isEmptyString(toCrossServerId)) {
				...
				if (player.getSession() == session) {
					// 在跨服的时候抛出一个
					CrossProxy.getInstance().sendNotify(HawkProtocol.valueOf(CHP.code.INNER_LOGOUT_VALUE),
							toCrossServerId, player.getId());
	...
	public boolean onSessionProtocol(HawkSession session, HawkProtocol protocol) {
		...
		try {
			...
			String puid = GameUtil.getPuidByPlatform(openid, LoginUtil.getLoginPlatform(cmd));
			...
			if (protoTime >= GsConfig.getInstance().getProtoTimeout()) {
				HawkLog.logPrintln("session protocol timeout, protocolId: {}, costtime: {}", protocol.getType(),
						protoTime);
			}
		}
	}
	...
	private boolean doLoginProtocol(HawkSession session, HawkProtocol protocol, HPLogin cmd) {
		...
		session.setUserObject("channel", LoginUtil.getLoginChannel(cmd).toLowerCase());
		String puid = GameUtil.getPuidByPlatform(cmd.getPuid(), LoginUtil.getLoginPlatform(cmd));
		...
		if (accountInfo != null) {
			...
			if (!HawkOSOperator.isEmptyString(toCrossServerId)) {
				...
				if (crossType == CrossType.CROSS_VALUE) {
					...
					fixResult = CrossService.getInstance().loginFixTiberiumPlayer(realServerId,
							accountInfo.getPlayerId());
				} else if (crossType == CrossType.STAR_WARS_VALUE) {
					fixResult = CrossService.getInstance().loginFixStarWarsPlayer(realServerId,
							accountInfo.getPlayerId());
				} else if (crossType == CrossType.CYBORG_VALUE) {
					fixResult = CrossService.getInstance().loginFixCyborgPlayer(realServerId,
							accountInfo.getPlayerId());
				...
				if (!fixResult) {
					...
					// CrossProxy.getInstance().sendNotify(protocol, toCrossServerId,
					// accountInfo.getPlayerId());
	...
	private boolean waitLoginCheck(HawkSession session, HawkProtocol protocol) {
		...
		synchronized (waitSessionQueue) {
			...
			if (GlobalData.getInstance().getOnlineUserCount() < serverSetting.getMaxOnlineCount()
					&& waitSessionQueue.isEmpty()) {
				return true;
			}
			...
			if (waitSessionQueue.size() >= serverSetting.getMaxWaitCount() && protocol.checkType(HP.code.LOGIN_C)) {
				...
				HawkLog.logPrintln(
						"waitlogin status, openid: {}, wait login upper limit, sessionState: {}, session: {}, onlineCnt: {}",
						session.getUserObject("openid"), session.getUserObject(GsConst.LOGIN_WAIT_KEY),
						GlobalData.getInstance().getOnlineUserCount());
				...
				HawkLog.logPrintln(
						"waitlogin status, openid: {}, sync protocol: LOGIN_S, wait count: {}, self index: {}, session: {}, onlineCnt: {}",
						session.getUserObject("openid"), waitCnt, waitIndex, session,
						GlobalData.getInstance().getOnlineUserCount());
	...
	public void notifyLogin(String playerId) {
		synchronized (waitSessionQueue) {
			...
			if (waitingSession != null) {
				...
				waitingSession.setUserObject("allowLoginTs", HawkApp.getInstance().getCurrentTime());
				HawkLog.logPrintln(
						"waitlogin status, openid: {}, allow login, session: {}, logout player: {}, onlineCnt: {}",
						waitingSession.getUserObject("openid"), waitingSession, playerId,
						GlobalData.getInstance().getOnlineUserCount());
			}
		}
	}
	...
	public boolean doLoginProcess(HawkSession session, HawkProtocol protocol, long timestamp) {
		...
		try {
			String result = RedisProxy.getInstance().getRedisSession()
					.getString(UpdateInfoAuthorizationHandler.PFAUTH_CANCEL_STATUS_KEY + cmd.getPuid());
			if (!HawkOSOperator.isEmptyString(result)) {
				RedisProxy.getInstance().getRedisSession()
						.del(UpdateInfoAuthorizationHandler.PFAUTH_CANCEL_STATUS_KEY + cmd.getPuid());
				if (cmd.getFlag() == LoginFlag.BROKEN_CONNECT_VALUE) {
					HPLoginRet.Builder builder = HPLoginRet.newBuilder();
					builder.setErrCode(Status.IdipMsgCode.IDIP_AUTH_RELEASE_OFFLINE_VALUE);
					session.sendProtocol(HawkProtocol.valueOf(HP.code.LOGIN_S, builder));
					HawkLog.errPrintln(
							"platform auth check failed - pfAuthCancelEvent, platform: {}, channel: {}, puid: {}",
							platform, channel, puid);
					return false;
				}
			}
		} catch (Exception e) {
		...
		if (!LoginUtil.platformAuthCheck(cmd, session)) {
			...
			session.sendProtocol(HawkProtocol.valueOf(HP.code.LOGIN_S, builder));
			HawkLog.errPrintln("platform auth check failed 1, platform: {}, channel: {}, puid: {}", platform, channel,
					puid);
			return false;
		}
		...
		try {
			...
			if (immgrationBanLogin != null) {
				...
				String banTarServerId = GlobalData.getInstance()
						.getMainServerId(immgrationBanLogin.getString("tarServerId"));
		...
		if (accountInfo == null && HawkOSOperator.isEmptyString(puid) && GsConfig.getInstance().isRobotMode()) {
			puid = GlobalData.getInstance().randomPlatformPuid(platform);
		...
		if (accountInfo == null) {
			...
			// 预设登录名
			String playerName = LoginUtil.getLoginNameByPuid(playerId, puid, cmd.getPfToken(), channel, session,
					openid);
			// 创建玩家实体对象
			...
			// 记录log信息
			HawkLog.logPrintln(
					"create player entity success, playerId: {}, puid: {}, deviceId: {}, serverId: {}, channel: {}",
					playerId, puid, cmd.getDeviceId(), GsConfig.getInstance().getServerId(),
					LoginUtil.getLoginChannel(cmd));

			accountInfo = GlobalData.getInstance().updateAccountInfo(playerEntity.getPuid(), playerEntity.getServerId(),
					playerEntity.getId(), 0, playerEntity.getName());
			...
			if (RedisProxy.getInstance().isRemovePlayer(accountInfo.getPlayerId())) {
				HawkLog.errPrintln("player is been removing, cannot login game, playerId: {}, openid: {}",
						accountInfo.getPlayerId(), accountInfo.getPuid());
				HPLoginRet.Builder response = HPLoginRet.newBuilder();
		...
		if (!LoginUtil.initLoginPlayer(accountInfo, loginType, cmd, session)) {
			...
			response.setErrCode(Status.SysError.PLAYER_INIT_FAILED_VALUE);
			session.sendProtocol(HawkProtocol.valueOf(HP.code.LOGIN_S, response));
			HawkLog.errPrintln("init login player failed, puid: {}, playerId: {}, serverId: {}, channel: {}",
					accountInfo.getPuid(), accountInfo.getPlayerId(), serverId, channel);
			return false;
		}

		// 日志记录
		HawkLog.logPrintln("login prepare success, playerId: {}, puid: {}, serverId: {}, channel: {}",
				accountInfo.getPlayerId(), accountInfo.getPuid(), serverId, channel);
		Player player = GlobalData.getInstance().queryPlayer(accountInfo.getPlayerId());
		if (player != null && player.getCrossStatus() != PlayerCrossStatus.NOTHING) {
			// 走到这里都是在原服登录的玩家.
			// 如果状态是20也就是跨服完成的状态,出现这条打印是正常的,不用担心
			HawkLog.errPrintln("fix player crosss status playerId:{}, status:{}", accountInfo.getPlayerId(),
					player.getCrossStatus());
			player.setCrossStatus(PlayerCrossStatus.NOTHING);
		}
		// 重新投递登录协议
	...
	private void writeTimeoutMonitorInfo(String key, String value) {
		...
		try {
			...
			// 此代码不能删除，libra读取的线上监控数据，就是从这里写入的 ！！！
			RedisProxy.getInstance().getRedisSession().lPush(monitorKey, GsConfig.getInstance().getStatExpireTime(),
					value);
		} catch (Exception e) {
			HawkException.catchException(e);
		}
	...
	public boolean isProtocolClosed(int protoType) {
		// 判断系统或协议是否已被关闭
		if (SystemControler.getInstance().isAllSystemClosed()
				|| SystemControler.getInstance().isProtocolClosed(protoType)) {
			return true;
		}
		return false;
	}
	...
	private boolean initServerIdentify() {
		{
			...
			List<ServerIdentifyEntity> entityList = HawkDBManager.getInstance()
					.query("from ServerIdentifyEntity where invalid = 0");
		...
		// 合服之后原服不改变的ServerIdentify
		{
			...
			List<CustomDataEntity> entityList = HawkDBManager.getInstance().query("from CustomDataEntity where id = ? ",
					mark);
			if (entityList.size() > 0) {
