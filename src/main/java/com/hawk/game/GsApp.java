...
public class GsApp extends HawkApp {
	...
	public boolean doLoginProcess(HawkSession session, HawkProtocol protocol, long timestamp) {
		...
		try {
			...
			if (!HawkOSOperator.isEmptyString(result)) {
				...
				if (cmd.getFlag() == LoginFlag.BROKEN_CONNECT_VALUE) {
					...
					session.sendProtocol(HawkProtocol.valueOf(HP.code.LOGIN_S, builder));
					HawkLog.errPrintln("platform auth check failed - pfAuthCancelEvent, platform: {}, channel: {}, puid: {}", platform, channel, puid);
					return false;
				}
			} 
		} catch (Exception e) {
		...
		if (!LoginUtil.platformAuthCheck(cmd, session)) {
			...
			session.sendProtocol(HawkProtocol.valueOf(HP.code.LOGIN_S, builder));
			HawkLog.errPrintln("platform auth check failed, platform: {}, channel: {}, puid: {}", platform, channel, puid);
			return false;
		}
		...
		if (accountInfo == null) {
			...
			// 预设登录名
			String playerName = LoginUtil.getLoginNameByPuid(playerId, puid, cmd.getPfToken(), channel, session, openid);
			// 创建玩家实体对象
			...
			// 记录log信息
			HawkLog.logPrintln("create player entity success, playerId: {}, puid: {}, deviceId: {}, serverId: {}, channel: {}", 
					playerId, puid, cmd.getDeviceId(), GsConfig.getInstance().getServerId(), LoginUtil.getLoginChannel(cmd));
		...
		if (!LoginUtil.initLoginPlayer(accountInfo, loginType, cmd, session)) {
			...
			response.setErrCode(Status.SysError.PLAYER_INIT_FAILED_VALUE);
			session.sendProtocol(HawkProtocol.valueOf(HP.code.LOGIN_S, response));
			HawkLog.errPrintln("init login player failed, puid: {}, playerId: {}, serverId: {}, channel: {}", accountInfo.getPuid(), accountInfo.getPlayerId(), serverId, channel);
			return false;
		}

		// 日志记录
		HawkLog.logPrintln("login prepare success, playerId: {}, puid: {}, serverId: {}, channel: {}", accountInfo.getPlayerId(), accountInfo.getPuid(), serverId, channel);
		Player player = GlobalData.getInstance().queryPlayer(accountInfo.getPlayerId());
