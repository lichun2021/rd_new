...
//		}
	...
	protected void logPushEvent() {
		...
		long leftTime = now - getStartTime();
		long minute5Sec = TimeUnit.MINUTES.toMillis(5);
	    if (leftTime >= minute5Sec-3000 && leftTime <= minute5Sec+3000 && now - pushEventTime > minute5Sec) {
	    	pushEventTime = now;
	    	logTopGuildPushEvent();
        }
	}
	 ...
	 * 日志上报
	 */
	private void logTopGuildPushEvent() {
				...
				for (int i = 0 ; i < Math.min(3, rankList.size()); i++) {
					...
					for (String playerId : GuildService.getInstance().getGuildMembers(guildId)) {
						try {
							...
							LogUtil.logPushEvent(player, PushEventType.PRESIDENT, 0);
							count++;
						} catch (Exception e) {
							HawkException.catchException(e);
						}
