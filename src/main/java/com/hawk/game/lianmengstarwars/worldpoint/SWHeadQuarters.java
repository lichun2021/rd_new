...
public class SWHeadQuarters extends ISWBuilding {
	...
	private void checkWin() {
		try {
			...
			final int winNum = StarWarsConstCfg.getInstance().getUpGuildCnt();
			List<String> winList = new ArrayList<>(winNum);
			if (Objects.nonNull(winGuild)) {
				winList.add(winGuild);
			}

			List<PBSWGuildInfo> rankList = new ArrayList<>(getParent().getLastBilingpb().getGuildInfoList());
			rankList.sort(Comparator.comparingInt(PBSWGuildInfo::getHonorRank));
			for (PBSWGuildInfo ginfo : rankList) {
				if (winList.size() >= winNum) {
					break;
				}
				if (!winList.contains(ginfo.getGuildId())) {
					winList.add(ginfo.getGuildId());
				}
			}

			getParent().setWinGuild(winList);
