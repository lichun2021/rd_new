...
public class SWWorldModule extends PlayerModule {
	...
	private void onPlayerEnterWorld(HawkProtocol protocol) {
		...
		player.getParent().getWorldPointService().getWorldScene().onPlayerEnterWorld(player);
	...
	private boolean onPlayerWorldMove(HawkProtocol protocol) {
		...
		int[] movpos = player.getWorldMovePos();
		double moveDis = AlgorithmUtil.lineDistance(movpos[0], movpos[1], cmd.getX(), cmd.getY());
		if (moveDis < 5 ) {
			return true;
		}
		player.setWorldMovePos(new int[]{cmd.getX(), cmd.getY()});
		...
		player.getParent().getWorldPointService().getWorldScene().onPlayerWorldMove(player);
		// 行军同步
	...
	protected boolean onGetGuildWarInfo(HawkProtocol protocol) {
		...
		if(cacheResp!=null){
			player.sendProtocol(HawkProtocol.valueOf(HP.code.GET_GUILD_WAR_INFO_S_VALUE, cacheResp.toBuilder()));
			return true;
		}
		// 回复协议
