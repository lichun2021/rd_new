...
public class CrossActivitySeasonService  extends HawkAppObj{
	...
	public void doMatch(int crossTermId) {
		...
		//固定匹配
		CrossManualMatchCfg cfg = HawkConfigManager.getInstance().getConfigByKey(CrossManualMatchCfg.class, crossTermId);
		if(Objects.nonNull(cfg)){
			...
			for(HawkTuple2<String, String> tuple : mlist){
				if(serverMap.containsKey(tuple.first) && 
					...
					HawkLog.logPrintln("CrossActivitySeasonService-CrossManualMatchCfg-match:{},{}",crossTermId,sj.toString());
				}
			}
		}
		//规则匹配
