...
public class BeatArmyTargetParser implements CrossTargetParser<PvpBattleEvent> {
	...
	public boolean calcScore(int termId, CrossIntegralCfg circularCfg, PvpBattleEvent event) {
		...
		if (playerId.startsWith("NPC")) {
			HawkLog.errPrintln("BeatArmyTargetParser calcScore playerId: {}, termId: {}", playerId, termId);
			return false;
		}
		...
		if (player == null) {
			//2025-12-05 13:38:55,677 ERROR [TaskExecutor-7] [HawkLog.java:71] - BeatArmyTargetParser calcScore player null, playerId: NPC-heiheihahei7qr-hrrk8-p, termId: 165
			HawkLog.errPrintln("BeatArmyTargetParser calcScore player null, playerId: {}, termId: {}", playerId, termId);
		}
