...
public class DressEntity extends HawkDBEntity {
	...
	@Transient
	Map<Integer, PlayerLuxuryInfo> skillMap = new ConcurrentHashMap<>();
	...
	public void afterRead() {
		...
		String skillInfo = LocalRedis.getInstance().getLuxurySkillPerson(playerId);
		if (!HawkOSOperator.isEmptyString(skillInfo)) {
			List<PlayerLuxuryInfo> skillList = SerializeHelper.stringToList(PlayerLuxuryInfo.class, skillInfo);
			skillList.forEach(e -> skillMap.put(e.getSkillId(), e));
		}
		super.afterRead();
	}
	
	public PlayerLuxuryInfo getSkillInfo(int skillId) {
		return skillMap.get(skillId);
	}
	
	public void addSkillInfo(PlayerLuxuryInfo skillInfo) {
		skillMap.put(skillInfo.getSkillId(), skillInfo);
	}
	
	public void saveSkillInfo() {
		String skillInfo = SerializeHelper.collectionToString(skillMap.values(), SerializeHelper.ELEMENT_DELIMITER, SerializeHelper.ATTRIBUTE_SPLIT);
		LocalRedis.getInstance().saveLuxurySkillPerson(playerId, skillInfo);
	}
