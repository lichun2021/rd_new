...
public class PlayerMeterialModule extends PlayerModule {
	...
	protected boolean onPlayerLogin() {
		loginUpdateBattleData();
		return super.onPlayerLogin();
	}

	private void loginUpdateBattleData() {
		try {
			if (!bfalse && !player.isCsPlayer() && MeterialTransportService.getInstance().getTermId() > 0) {
				Optional<PresetMarchInfo> preSetInfo = getPresetInfo(player.getId(), MTMarchIndex.YACHE);
				if (preSetInfo.isPresent()) { // 没有预设行军, 不能出征
					int errorCode = saveBattlePlayer(preSetInfo.get(), MTMarchIndex.YACHE);
					HawkLog.logPrintln("meterialtransport saveBattlePlayer auto {}, {} ", player.getId(), errorCode);
					bfalse = true;
				}
			}
		} catch (Exception e) {
	...
	private boolean onMT_TRUCK_HISTORY_C(HawkProtocol protocol) {
		...
		for (String tid : MeterialTransportService.getInstance().getPlayerTruck().get(player.getId())) {
			...
			PBMTTruck pbObj = truck.toPBObj();
			PBMTTruck.Builder builder = pbObj.toBuilder();
			for(PBMTTruckGroup.Builder cbuilder:   builder.getCompartmentsBuilderList()){
				for(PBMTPlayer.Builder me : cbuilder.getMembersBuilderList()){
					me.clearBattleData();
				}
			}
			for(PBMTTruckBattleRecord.Builder rb: builder.getBattleRecordBuilderList()){
				for(PBMTPlayer.Builder me : rb.getAtkerBuilderList()){
					me.clearBattleData();
				}
			}
			
			if (req.getType() == 1 && truck.isMember(player.getId())) {
				resp.addTrucks(builder.build());
			}
	...
	private void syncTruckList() {
		...
		for (MTTruck truck : refreshList) {
			try {
				...
				resp.addOtherTrucks(builder.build());
			} catch (Exception e) {
				HawkException.catchException(e);
			}
		}
		...
		if (truck1 != null) {
			try {
				PBMTTruck pbObj = truck1.toPBObj();
				PBMTTruck.Builder builder = pbObj.toBuilder();
				for(PBMTTruckGroup.Builder cbuilder:   builder.getCompartmentsBuilderList()){
					for(PBMTPlayer.Builder me : cbuilder.getMembersBuilderList()){
						me.clearBattleData();
				...
				resp.addSelfTrucks(builder.build());
			} catch (Exception e) {
				HawkException.catchException(e);
			}
		}
		...
		if (truck2 != null) {
			try {
				PBMTTruck pbObj = truck2.toPBObj();
				PBMTTruck.Builder builder = pbObj.toBuilder();
				for(PBMTTruckGroup.Builder cbuilder:   builder.getCompartmentsBuilderList()){
					for(PBMTPlayer.Builder me : cbuilder.getMembersBuilderList()){
						me.clearBattleData();
		...
		if (guildTruck != null && guildTruck.getState() == MTTruckState.PRE) {
			try {
				PBMTTruck pbObj = guildTruck.toPBObj();
				PBMTTruck.Builder builder = pbObj.toBuilder();
				builder.getLeaderBuilder().clearBattleData();
				for(PBMTTruckGroup.Builder cbuilder:   builder.getCompartmentsBuilderList()){
					for(PBMTPlayer.Builder me : cbuilder.getMembersBuilderList()){
						me.clearBattleData();
					}
				...
				resp.setGuildTruck(builder.build());
			} catch (Exception e) {
				HawkException.catchException(e);
			}
		}
		...
		for (MTTruckRob rob : robs) {
			try {
				PBMTTruckRob pbObj = rob.toPBObj();
				PBMTTruckRob.Builder builder = pbObj.toBuilder();
				builder.getTargetBuilder().getLeaderBuilder().clearBattleData();
				for(PBMTTruckGroup.Builder cbuilder:   builder.getTargetBuilder().getCompartmentsBuilderList()){
					for(PBMTPlayer.Builder me : cbuilder.getMembersBuilderList()){
						me.clearBattleData();
					}
				}
				for(PBMTTruckBattleRecord.Builder rb: builder.getTargetBuilder().getBattleRecordBuilderList()){
					for(PBMTPlayer.Builder me : rb.getAtkerBuilderList()){
						me.clearBattleData();
					}
				}
				
				for(PBMTPlayer.Builder me : builder.getMembersBuilderList()){
					me.clearBattleData();
				}
				resp.addMassMarch(builder.build());
			} catch (Exception e) {
				HawkException.catchException(e);
			}
		}
	...
	private void sendPreMarch(String playerId) {
		...
		Optional<PresetMarchInfo> preOp = getPresetInfo(playerId, MTMarchIndex.YACHE);
		if(preOp.isPresent()){
			resp.setQuganMarch(preOp.get());
		}
	...
	private boolean onGuildfMarchCreate(HawkProtocol protocol) {
		...
		synchronized (guildId) {
			...
			int trainCnt = getDBEntity(player.getId()).incNumber(trainNumberMap.first);
			member.setHasAward(trainCnt <= trainNumberMap.second);
	...
	private boolean onGuildMarchLeader(HawkProtocol protocol) {
		...
		MTMember oldLeader = truck.getLeader();
		if(HawkTime.isSameDay(truck.getCreateTime(), HawkTime.getMillisecond())){
			getDBEntity(oldLeader.getPlayerId()).drcNumber(trainNumberMap.first);
		}
		...
		int trainCnt = getDBEntity(leader.getId()).incNumber(trainNumberMap.first);
		member.setHasAward(trainCnt <= trainNumberMap.second);
		truck.setLeader(member);
	...
	private boolean onGuildMarchPaiDui(HawkProtocol protocol) {
		...
		if(newmember == null){
			HawkTuple2<NumberType, Integer> trainNumberMap = MeterialTransportService.getInstance().getTrainNumberMap(truck.getType());
			newmember = MTMember.valueOf(player, MeterialTransportService.getInstance().getBattleData(MTMarchIndex.YACHE, player.getId()));
			int trainCnt = getDBEntity(player.getId()).incNumber(trainNumberMap.first);
			newmember.setHasAward(trainCnt <= trainNumberMap.second);
		}

		tarcompartment.getMemberList().add(newmember);
	...
	private boolean onGuildTruckRob(HawkProtocol protocol) {
		...
		PBChampionPlayer battleData = MeterialTransportService.getInstance().getBattleData(MTMarchIndex.YACHE, player.getId());
		if (battleData == null) {
			sendError(protocol.getType(), Status.MTTError.MT_TRUCK_PRE_MARCH_VALUE);
			return false;
		}
	...
	private boolean onAttackTruck(HawkProtocol protocol) {
		...
		rob.setTarTruckId(truckId);
		rob.setTarTruck(truck);
		rob.getMass().add(MTMember.valueOf(player, MeterialTransportService.getInstance().getBattleData(MTMarchIndex.YACHE, player.getId())));
	...
	private boolean onMarchPresetInfo(HawkProtocol protocol) {
		PresetMarchInfo req = protocol.parseProtocol(PresetMarchInfo.getDefaultInstance());
		int idx = MTMarchIndex.YACHE.getNumber();
		...
		RedisProxy.getInstance().getRedisSession().hSetBytes(key, idx + "", req.toByteArray());
		int errorCode = saveBattlePlayer(req, MTMarchIndex.valueOf(idx));
		HawkLog.logPrintln("meterialtransport saveBattlePlayer op {}, {} ", player.getId(), errorCode);
