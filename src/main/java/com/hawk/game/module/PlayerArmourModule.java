...
public class PlayerArmourModule extends PlayerModule {
    ...
    protected boolean onPlayerLogin() {
        ...
        player.getPush().syncImmortReshapeInfo();
        ...
        player.getPush().syncEquipImmortShow();
    ...
    private void putOn(HawkProtocol protocol) {
        ...
        if (armour.getImmort().isImmortCore()) {
            sendError(protocol.getType(), Status.ArmourError.ARMOUR_IMMORT_CORE_CANT_NOT_WEAR_VALUE);
            return;
        }
        // 脱下之前的铠甲
    ...
    private void intensify(HawkProtocol protocol) {
        ...
        if (armour.getImmort().canImmortRedLevel()) {
            levelLimit += ArmourConstCfg.getInstance().getImmortLevelLimitAdd();
        }
    ...
    public int resolveArmour(Set<String> ids) {
        ...
        for (String id : ids) {
            ...
            if (armour.getImmort().isImmortCore()) {
                result = Status.ArmourError.ARMOUR_IMMORT_CORE_CANT_NOT_WEAR_VALUE;
                return result;
            }
            if (armour.isLock()) {
                continue;
            ...
            if (armour.getImmort().canImmortRedLevel()) {
                if (!armour.getImmort().canResolve(player)) {
                    result = Status.ArmourError.ARMOUR_IMMORT_CORE_ONLY_POS_RESOLVE_VALUE;
                    return result;
                }
            }
            //同部位同兵种只有一件不能分解
            int level = armour.getImmort().getArmourImmortEntity().getInfinityLevel();
            ArmourImmortLevelCfg immortLevelCfg = HawkConfigManager.getInstance().getCombineConfig(ArmourImmortLevelCfg.class, level, armour.getArmourId());
            if (immortLevelCfg != null) {
                resolveAward.addItemInfos(immortLevelCfg.getResolveItem());
    ...
    public void extrAttrIncre() {
        ...
        for (ArmourEntity armour : armours) {
            ...
            for (int i = 0; i < effAddCount; i++) {
                ...
                int armourQuality = armour.getImmort().canImmortRedLevel() ? ArmourConstCfg.getInstance().getImmortQualityLimit() -1 : armour.getQuality();
                List<ArmourAdditionalCfg> attrCfgs = AssembleDataManager.getInstance().getArmourAdditionCfgs(additionType, armourQuality);
                if (attrCfgs.isEmpty()) {
                    continue;
                }
                int randTimes = 0;
    ...
    private void onStarUp(HawkProtocol protocol) {
        ...
        int starlimit = qualityCfg.getStarLimit();
        if (armour.getImmort().canImmortRedLevel()) {
            starlimit += ArmourConstCfg.getInstance().getImmortStarLimitAdd();
        }
        // 超过此品质可升级的最大星级
        if (armour.getQuantum() >= ArmourConstCfg.getInstance().getQuantumRedLevel()) {
            starlimit += ArmourConstCfg.getInstance().getStarLimitAdd();
        }
        if (afterStar > starlimit) {
            sendError(protocol.getType(), Status.ArmourError.ARMOUR_IMMORT_STAR_LIMIT_VALUE);
            return;
        }
        // 下一星级配置不存在
    ...
    private void onArmourStarAttrUp(HawkProtocol protocol) {
        ...
        if (starAttr.getRate() > ArmourConstCfg.getInstance().getChargeImmortLimit()) {
            sendError(protocol.getType(), Status.ArmourError.ARMOUR_NOT_FOUND_VALUE);
            return;
        }
        ...
        } else if (starAttr.getRate() < ArmourConstCfg.getInstance().getChargeRedLimit()) {
            // 红装充能
            starAttrUpRed(armour, req.getAttrId(), addRateList, 1);
            if (addRateList.size() > 0) {
                starAttr.setBreakthrough(2);
            }
        } else if (starAttr.getRate() < ArmourConstCfg.getInstance().getChargeImmortLimit()) {
            // 不朽充能
            starAttrUpImmort(armour, req.getAttrId(), addRateList, 1);
            if (!addRateList.isEmpty()) {
                starAttr.setBreakthrough(3);
            }
        }
    ...
    private void onArmourStarAttrUpTen(HawkProtocol protocol) {
        ...
        if (starAttr.getRate() < ArmourConstCfg.getInstance().getChargeCommonLimit()) {
            ...
            // 不朽充能
            starAttrUpImmort(armour, req.getAttrId(), addRateList, 10);
            if (!addRateList.isEmpty()) {
                starAttr.setBreakthrough(3);
            }
        }
     ...
     * @param starAttrId
     */
    private void starAttrUpImmort(ArmourEntity armour, int starAttrId, List<Integer> addRateList, int count) {
        ArmourEffObject starAttr = getStarAttr(armour, starAttrId);
        // 充能前进度
        int beforeRate = starAttr.getRate();
        int curRate = beforeRate;
        ...
        for (int i = 1; i <= count; i++) {
            ...
            // 已经到最大进度了
            if (curRate >= ArmourConstCfg.getInstance().getChargeImmortLimit()) {
                break;
            }
            // 消耗 小于满紫进度值用普通消耗,否则用高级消耗
            ConsumeItems consume = ConsumeItems.valueOf();
            consume.addConsumeInfo(ArmourConstCfg.getInstance().getChargeConsumeImmort(i));
            if (!consume.checkConsume(player)) {
                break;
            }
            // 随机增加的进度值
            Integer randRate = HawkRand.randomWeightObject(ArmourConstCfg.getInstance().getChargeImmortRateMap());
            // 充能后进度
            curRate = Math.min(ArmourConstCfg.getInstance().getChargeImmortLimit(), curRate + randRate.intValue());
            addRateList.add(randRate);
            upCount++;
        }
        if (upCount > 0) {
            ConsumeItems itemUse = ConsumeItems.valueOf();
            itemUse.addConsumeInfo(ArmourConstCfg.getInstance().getChargeConsumeImmort(upCount));
            if (!itemUse.checkConsume(player)) {
                return;
            }
            itemUse.consumeAndPush(player, Action.ARMOUR_STAR_ATTR_CHARGE);
            ...
            starAttr.setRate(curRate);
            // 已消耗道具记录
            armour.addConsume(ArmourConstCfg.getInstance().getChargeConsumeImmort(upCount));
            logger.info("armourChange, attrUpImmort, playerId:{}, armourId:{}, beforeRate:{}, afterRate:{}", player.getId(), armour.getId(), beforeRate, curRate);
        }
    }
    ...
    private void onArmourStarAttrBreakthrough(HawkProtocol protocol) {
        ...
        if (starAttr.getBreakthrough() == 0) {
            ...
            starAttr.setBreakthrough(1);
            armour.notifyUpdate();
        } else if(starAttr.getBreakthrough() == 1){
            ...
            if (starAttr.getRate() < ArmourConstCfg.getInstance().getChargeRedLimit()) {
                return;
            }
            // 不朽装备才能突破
     ...
     * @param protocol
     */
    @ProtocolHandler(code = HP.code2.EUQUIP_IMMORT_ECHO_SHOW_CHANGE_REQ_VALUE)
    private void onArmourImmortShowReq(HawkProtocol protocol) {
        WorldPointService.getInstance().updateEquipImmortShow(player, true);
        player.getPush().syncEquipImmortShow();
    }
     ...
     * 不朽熔铸强化
     */
    @ProtocolHandler(code = HP.code2.ARMOUR_IMMORT_INTENSIFY_REQ_VALUE)
    private void immortIntensify(HawkProtocol protocol) {
        ArmourImmortIntensifyReq req = protocol.parseProtocol(ArmourImmortIntensifyReq.getDefaultInstance());
        // 装备不存在
        ArmourEntity armour = player.getData().getArmourEntity(req.getArmourId());
        if (armour == null) {
            sendError(protocol.getType(), Status.ArmourError.ARMOUR_NOT_FOUND_VALUE);
        ...
        ArmourImmortComp comp = armour.getImmort();
        if (!comp.checkUnLock()) {
            sendError(protocol.getType(), Status.ArmourError.ARMOUR_IMMORT_PRE_LEVEL_ERROR);
            return;
        }
        ArmourImmortCompEntity armourImmortEntity = comp.getArmourImmortEntity();
        // 此等级不能升级不朽熔铸等级
        int level = armour.getLevel();
        if (!ArmourConstCfg.getInstance().canLevelImmort(level)) {
            sendError(protocol.getType(), Status.ArmourError.ARMOUR_IMMORT_PRE_LEVEL_ERROR);
            return;
        }
        // 升级后的不朽熔铸等级
        int afterInfinity = armourImmortEntity.getInfinityLevel() + 1;
        ArmourImmortLevelCfg afterImmortCfg = HawkConfigManager.getInstance().getCombineConfig(ArmourImmortLevelCfg.class, afterInfinity, armour.getArmourId());
        if (afterImmortCfg == null) {
            sendError(protocol.getType(), Status.ArmourError.ARMOUR_CONFIG_NOT_FOUND_VALUE);
            return;
        }
        //玩家7个装备部位均解锁不朽品质装备开启不朽核心熔铸
        if (armourImmortEntity.isImmortCore()) {
            if (!comp.checkPosArmourAll(player)) {
                player.sendError(protocol.getType(), Status.ArmourError.ARMOUR_IMMORT_CORE_INTENSIFY_PRE_ERROR, 0);
                return;
            }
        }
        //同兵种同部位的装备同时只有一件装备可以进行不朽熔铸
        ArmourCfg armourCfg = HawkConfigManager.getInstance().getConfigByKey(ArmourCfg.class, armour.getArmourId());
        if (armourCfg == null) {
            player.sendError(protocol.getType(), Status.ArmourError.ARMOUR_CONFIG_NOT_FOUND_VALUE, 0);
            return;
        }
        if (comp.checkCanIntensify(player)) {
            player.sendError(protocol.getType(), Status.ArmourError.ARMOUR_IMMORT_POS_SUIT_ONLY_VALUE, 0);
            return;
        }
        if (comp.checkCreateNewArmour(player, afterInfinity)) {
            player.sendError(protocol.getType(), Status.ArmourError.ARMOUR_IMMORT_CREATE_MAX_ERROR, 0);
            return;
        }
        // 升级不朽锻造消耗
        List<ItemInfo> resItems = afterImmortCfg.getConsume();
        ConsumeItems consume = ConsumeItems.valueOf();
        consume.addConsumeInfo(resItems);
        if (!consume.checkConsume(player)) {
            return;
        }
        consume.consumeAndPush(player, Action.ARMOUR_IMMORT_INFINITY_UP);
        // 升级
        armourImmortEntity.setInfinityLevel(afterInfinity);
        //额外属性
        comp.addImmortExtAttr(player, afterImmortCfg.getRandomAttrNu());
        //产生不朽核心
        int echoId = comp.createImmortCore(player);
        //打点
        comp.logArmourImmortIntensify(player, echoId);

        comp.notifyUpdate(player);
        // 通用成功返回
        player.responseSuccess(protocol.getType());
    }
    /**
     * 不朽熔铸重塑
     */
    @ProtocolHandler(code = HP.code2.ARMOUR_IMMORT_ATTR_RANDOM_REQ_VALUE)
    private void immortAttrRand(HawkProtocol protocol) {
        ArmourImmortRandomReq req = protocol.parseProtocol(ArmourImmortRandomReq.getDefaultInstance());
        // 装备不存在
        ArmourEntity armour = player.getData().getArmourEntity(req.getArmourId());
        if (armour == null) {
            sendError(protocol.getType(), Status.ArmourError.ARMOUR_NOT_FOUND_VALUE);
        ...
        comp.attrRandom(player, protocol.getType(), req.getConfirm(),req.getClear());
        player.responseSuccess(protocol.getType());
    }
     ...
     * 不朽重塑属性锁定
     */
    @ProtocolHandler(code = HP.code2.ARMOUR_IMMORT_ATTR_LOCK_REQ_VALUE)
    private void immortAttrLock(HawkProtocol protocol) {
        ArmourImmortAttrLockReq req = protocol.parseProtocol(ArmourImmortAttrLockReq.getDefaultInstance());
        // 装备不存在
        ArmourEntity armour = player.getData().getArmourEntity(req.getArmourId());
        if (armour == null) {
            sendError(protocol.getType(), Status.ArmourError.ARMOUR_NOT_FOUND_VALUE);
        ...
        comp.attrLock(player, req.getAttrId(),req.getLock(), protocol.getType());
        player.responseSuccess(protocol.getType());
    }
}
