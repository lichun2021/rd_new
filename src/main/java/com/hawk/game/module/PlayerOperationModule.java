...
public class PlayerOperationModule extends PlayerModule {
	...
	private boolean onChangePlayerShow(HawkProtocol protocol) {
		...
		if (cfg.getType() == PlayerShowCfg.LUXURY) {
			if (!checkDressCond(cfg, protocol.getType())) {
				return false;
			}
		} else if (useGold && cfg.getType() == PlayerShowCfg.PURCHASE) {
			// 购买切换头像
			...
			if (cfg.getType() == PlayerShowCfg.PURCHASE && !findIconInBuyIds(iconId, getIconIds())) {
	 ...
	 * @return
	 */
	private boolean checkDressCond(PlayerShowCfg cfg, int protocol) {
		//激活女娲天奢基地后即可解锁，不必实时穿戴
		DressToolCfg dressToolCfg = HawkConfigManager.getInstance().getConfigByKey(DressToolCfg.class, cfg.getLuxuryDressID());
		if (dressToolCfg == null) {
			sendError(protocol, Status.SysError.CONFIG_ERROR_VALUE);
			HawkLog.errPrintln("change playerShow error, dressToolCfg null, playerId: {}, dressToolId: {}", player.getId(), cfg.getLuxuryDressID());
			return false;
		}
		...
		if (dressCfg == null) {
			sendError(protocol, Status.SysError.CONFIG_ERROR_VALUE);
			HawkLog.errPrintln("change playerShow error, dressCfg null, playerId: {}, dressId: {}", player.getId(), dressToolCfg.getDressId());
			return false;
		}
		...
		if (!dressEntity.hasDress(dressCfg.getDressType(), dressCfg.getModelType())) {
			sendError(protocol, Status.Error.LUXURY_DRESS_MISSING_VALUE);
			HawkLog.errPrintln("change playerShow error, need dress, playerId: {}, dressId: {}", player.getId(), dressToolCfg.getDressId());
			return false;
		}
		return true;
	}
