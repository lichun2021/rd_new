...
public class PlayerAgencyModule extends PlayerModule {
	...
	private boolean checkEventRefresh(AgencyEntity agencyEntity){
		...
		if (refreshTime > 0L && refreshTime < HawkTime.getMillisecond()) {
			...
			nextRefreshTime = this.checkNextRefreshTime(nextRefreshTime);
	...
	public long checkNextRefreshTime(long refreshTime){
		Calendar calendar = Calendar.getInstance();
		calendar.setTimeInMillis(refreshTime);
		int delay = AgencyConstCfg.getInstance().getRefreshDelay();
		int refreshHour = calendar.get(Calendar.HOUR_OF_DAY);
		int delayHour =  delay / 3600;
		int[] refreshHourArray = AgencyConstCfg.getInstance().getRefreshTimeArray();
		int curHour = HawkTime.getHour();
		
		int fixHour = -1;
		for(int hour : refreshHourArray){
			int hourMax = hour + delayHour;
			if(hour <= refreshHour && refreshHour <= hourMax){
				//不需要修复  刷新时间 在配置小时内
				return refreshTime;
			}
			if(curHour < hour && fixHour < 0){
				fixHour = hour;
			}
		}
		//按照当前时间 重算一下刷新时间
		int minHour = refreshHourArray[0];
		int randomDelay = HawkRand.randInt(1, delay) * 1000;
		if(fixHour > 0){
			//当天刷新
			refreshTime = HawkTime.getAM0Date().getTime()  + fixHour * GsConst.HOUR_MILLI_SECONDS + randomDelay;
		}else{
			//没拿到  第二天刷新
			refreshTime = HawkTime.getNextAM0Date()  + minHour * GsConst.HOUR_MILLI_SECONDS + randomDelay;
		}
		return refreshTime;
	}
