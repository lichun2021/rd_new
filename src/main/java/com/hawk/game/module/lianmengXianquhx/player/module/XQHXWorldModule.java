...
import com.hawk.game.protocol.XQHX.PBXQHXAnchorGetGuildWarCellInfoReq;
import com.hawk.game.protocol.XQHX.PBXQHXAnchorGetGuildWarInfoReq;
import com.hawk.game.protocol.XQHX.PBXQHXAnchorListMailReq;
...
//	}
	 ...
	 * 主播查看建筑战斗邮件
	 */
	@ProtocolHandler(code = HP.code2.XQHX_ANCHOR_LIST_MAIL_C_VALUE)
	...
	private void onPlayerEnterWorld(HawkProtocol protocol) {
		...
		player.getParent().getWorldSence().onPlayerEnterWorld(player);
	...
	private boolean onPlayerWorldMove(HawkProtocol protocol) {
		...
		player.getParent().getWorldSence().onPlayerWorldMove(player);
		return true;
	}
	...
	public void onPlayerMove(IXQHXPlayer player, int x, int y) {
		...
		for (IXQHXWorldMarch march : player.getParent().getWorldMarchList()) {
			try {
				...
				boolean inview = march.isInview(player);
				if (inview) {
				...
				if (inview && !hasPush) {
					MarchData.Builder dataBuilder = MarchData.newBuilder();
					dataBuilder.setMarchId(march.getMarchId());
					dataBuilder.setMarchPB(GameUtil.simplifiedMarchSyncPB(march.toBuilder(relation)));
	...
	@ProtocolHandler(code = HP.code2.XQHX_ANCHOR_GET_GUILD_WAR_INFO_C_VALUE)
	protected boolean onAnchorGetGuildWarInfo(HawkProtocol protocol) {

		PBXQHXAnchorGetGuildWarInfoReq req = protocol.parseProtocol(PBXQHXAnchorGetGuildWarInfoReq.getDefaultInstance());
		
		if (!player.isAnchor()) {
			return false;
		}
		String viwerGuildId = player.getParent().getCampGuild(XQHX_CAMP.A);
		if (req.getCamp() == 2) {
			viwerGuildId = player.getParent().getCampGuild(XQHX_CAMP.B);
		}
		player.setGuildId(viwerGuildId);
		HPGetGuildWarInfoResp.Builder resp = buildGuildWarInfo(player);
		player.sendProtocol(HawkProtocol.valueOf(HP.code2.XQHX_ANCHOR_GET_GUILD_WAR_INFO_S_VALUE, resp));
		return true;
	}

	@ProtocolHandler(code = HP.code2.XQHX_ANCHOR_GET_GUILD_WAR_CELL_INFO_C_VALUE)
	protected boolean onAnchorGetGuildWarCellInfo(HawkProtocol protocol) {
		
		if (!player.isAnchor()) {
			return false;
		}
		PBXQHXAnchorGetGuildWarCellInfoReq req = protocol.parseProtocol(PBXQHXAnchorGetGuildWarCellInfoReq.getDefaultInstance());
		final String marchId = req.getMarchId();
		String viwerGuildId = player.getParent().getCampGuild(XQHX_CAMP.A);
		if (req.getCamp() == 2) {
			viwerGuildId = player.getParent().getCampGuild(XQHX_CAMP.B);
		...
		// 回复协议
		HPGetGuildWarInfoResp.Builder resp = buildGuildWarCellInfo(marchId, player);
		player.sendProtocol(HawkProtocol.valueOf(HP.code2.XQHX_ANCHOR_GET_GUILD_WAR_CELL_INFO_S_VALUE, resp));
		return true;
	}

	private HPGetGuildWarInfoResp.Builder buildGuildWarInfo(IXQHXPlayer player) {
		HPGetGuildWarInfoResp cacheResp = player.getParent().guildWarInfoCache.getIfPresent(player.getGuildId());
		if(cacheResp!=null){
			return cacheResp.toBuilder();
		}
		// 回复协议
		...
		player.getParent().guildWarInfoCache.put(player.getGuildId(), resp.build());
		return resp;
	}
