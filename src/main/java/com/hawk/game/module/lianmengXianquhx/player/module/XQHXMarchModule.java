...
//		}
		...
		IXQHXWorldMarch march = player.getParent().getMarch(req.getMarchId());
		if (march == null) {
			fixMarchState(req.getMarchId(),MarchEvent.MARCH_DELETE);
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_NOT_EXISIT);
			return false;
		}
	...
	private boolean onPointMarchCallBack(HawkProtocol protocol) {
		...
		HawkTaskManager.getInstance().postTask(new HawkTask("onPointMarchCallBack") {
			@Override
			public Object run() {
				pointMarchCallBack(protocol, req, worldPoint);
				return null;
			}
		}, worldPoint.getHashThread(threadNum));
		return true;
	}
	
	private boolean pointMarchCallBack(HawkProtocol protocol, WorldPointMarchCallBackReq req, IXQHXWorldPoint worldPoint) {
		IXQHXWorldMarch march = null; 
		if (StringUtils.isNotEmpty(req.getMarchId())) {
			march = player.getParent().getPlayerMarch(player.getId(), req.getMarchId());
		} else {
			march = player.getParent().getPointMarches(GameUtil.combineXAndY(req.getX(), req.getY())).stream()
	...
	private void fixMarchState(String marchId, MarchEvent eventType ) {
		if(StringUtils.isEmpty(marchId)){
			return;
		}
		WorldMarchDeletePush.Builder builder = WorldMarchDeletePush.newBuilder();
