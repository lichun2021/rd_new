...
public class XQHXTickAbleTask extends HawkTask {
	...
	private ConcurrentSkipListMap<Integer, XQHXWorldPointUpdateEventFunc> noticyWorldPointUpdateMap = new ConcurrentSkipListMap<>();
	/** 准备运行 */
	private AtomicBoolean readyToRace = new AtomicBoolean(false);
	@Override
	public Object run() {
		long now = HawkTime.getMillisecond();
		long beginTimeMs = now;
		try {
			...
			try { // 广播行军变化
				if (now - lastapply > 987) {
					...
					while (!noticyWorldPointUpdateMap.isEmpty()) {
						// 移除当前时间节点集合
						XQHXWorldPointUpdateEventFunc func = noticyWorldPointUpdateMap.pollFirstEntry().getValue();
						func.apply(null);
					}
				}
			} catch (Exception e) {
			...
			// 时间消耗的统计信息
			long costTimeMs = now - beginTimeMs;
			if (costTimeMs > 200) {
	...
	public void doUpdateWorldPoint(XQHXWorldPointUpdateEventFunc func) {
		if(func.isDelete()){
			func.apply(null);
			return;
		}
