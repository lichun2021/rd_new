...
public class PlayerLoginModule extends PlayerModule {
	...
	private void logRegisterInfo() {
		LogUtil.recordPersonalLog(player, LogInfoType.createuser, (logParam)->{
			logParam.put("inviteComing", player.getEntity().isBeInvited() ? 1: 0);
		});
		...
		try {
			...
			player.getData().createCustomDataEntity(CustomKeyConst.CONTINUE_LOGIN_KEY, HawkTime.getSeconds(), String.format("%d_%d", 1, 1));
			...
			if (accountRoleMap.isEmpty() || (accountRoleMap.size() == 1 && roleInfoObj != null && player.getId().equals(roleInfoObj.getPlayerId()))) {
				LogUtil.recordPersonalLog(player, LogInfoType.account_register, (logParam)->{
					logParam.put("inviteComing", player.getEntity().isBeInvited() ? 1: 0);
				});
			}
		} catch (Exception e) {
	...
	private boolean onLoginRequest(HawkProtocol protocol) {
		...
		// 版本信息
		String platform = LoginUtil.getLoginPlatform(loginCmd);
		VersionInfo versionInfo = RedisProxy.getInstance().getVersionInfo(GsConfig.getInstance().getAreaId(), platform);
		if (versionInfo != null) {
	...
	private boolean onPlayerDataLoadSuccess(PlayerData playerData, HawkSession session, HPLogin loginCmd) {
		...
		String platform = LoginUtil.getLoginPlatform(loginCmd);
		String channel = LoginUtil.getLoginChannel(loginCmd);
		boolean isWin32 = GameUtil.isWin32Platform(platform, channel); 
	...
	private void logLoginInfo() {
		try {
			CustomDataEntity customData = player.getData().getCustomDataEntity(CustomKeyConst.CONTINUE_LOGIN_KEY);
...
//			}
		...
		try {
			if (!HawkOSOperator.isEmptyString(pushInfo) && !HawkOSOperator.isEmptyString(lang)) {
				...
				if (!HawkOSOperator.isEmptyString(json.getString("pushDeviceId"))) {
					json.put("lang", lang);
					CustomDataEntity customData = player.getData().getCustomDataEntity(CustomKeyConst.PUSH_INFO_KEY);
					if (customData == null) {
						customData = player.getData().createCustomDataEntity(CustomKeyConst.PUSH_INFO_KEY, 0, json.toString());
					} else {
	...
	protected void checkNameAfterMergeArea() {
		try {
			int iconId = player.getEntity().getIcon();
			int cfgId = iconId + PlayerShowCfg.ID_BASE;
			PlayerShowCfg showCfg = HawkConfigManager.getInstance().getConfigByKey(PlayerShowCfg.class, cfgId);
			//开启了天奢形象
			if (showCfg.getType() == PlayerShowCfg.LUXURY) {
				DressToolCfg dressToolCfg = HawkConfigManager.getInstance().getConfigByKey(DressToolCfg.class, showCfg.getLuxuryDressID());
				...
				if (!dressEntity.hasDress(dressCfg.getDressType(), dressCfg.getModelType())) {
					player.getEntity().setIcon(10);
					HawkLog.logPrintln("playerlogin reset player show, playerId: {}", player.getId());
				}
			}
		} catch (Exception e) {
			HawkException.catchException(e);
