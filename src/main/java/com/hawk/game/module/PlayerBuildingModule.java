...
public class PlayerBuildingModule extends PlayerModule {
	...
	private boolean onRemoveBuilding(HawkProtocol protocol) {
		...
		double costTime = BuildingService.getInstance().getCostTime(player, buildingCfg.getDismantleTime(), buildingCfg.getLevel(),buildingCfg.getLevel());
		// 检查建筑队列是否被占用
	...
	private boolean onBuildingCreate(HawkProtocol protocol) {
		...
		double buildTime = BuildingService.getInstance().getCostTime(player, buildingCfg.getBuildTime(),1, 1);
		// 检查建筑队列是否被占用
	...
	private boolean onBuildingUpgrade(HawkProtocol protocol) {
		...
		for(int level = 1;level<=multi;level++){
			...
			double levelBuildTime = BuildingService.getInstance().getCostTime(player, nextLevelCfg.getBuildTime(),currCfg.getLevel(), nextLevelCfg.getLevel());
		...
		// 建筑升级操作打点
		LogUtil.logBuildLvUpOperation(player, buildingEntity, currCfg.getLevel(), immediate,multi);
	 ...
	 * 战斗解锁区块
	 */
	@ProtocolHandler(code = HP.code2.BUILDING_FIGHT_AREA_UNLOCK_C_VALUE)
	public boolean battleUnlockArea(HawkProtocol protocol) {
		World.BuildingAreaUnlockFightReq req = protocol.parseProtocol(World.BuildingAreaUnlockFightReq.getDefaultInstance());

		int buildArenId = req.getAreaId();
		BuildAreaCfg cfg = HawkConfigManager.getInstance().getConfigByKey(BuildAreaCfg.class, buildArenId);
		if (cfg == null) {
			sendError(protocol.getType(), Status.SysError.CONFIG_ERROR);
			logger.error("build area config error, playerId: {}, areaId: {}", player.getId(), buildArenId);
			return false;
		}
		// 判断区块是否已解锁
		Set<Integer> unlockedAreas = player.getData().getPlayerBaseEntity().getUnlockedAreaSet();
		if (unlockedAreas.contains(buildArenId)) {
			sendError(protocol.getType(), Status.Error.BUILDING_AREA_UNLOCKED_VALUE);
			logger.error("the area already unlocked, playerId: {}, areaId: {}", player.getId(), buildArenId);
			return false;
		}
		// 判断是否是当前可解锁的区块
		List<Integer> areaList = BuildAreaCfg.getUnlockedArea(player.getCityLevel());
		if (!areaList.contains(buildArenId)) {
			sendError(protocol.getType(), Status.Error.BUILDING_AREA_UNLOCK_NOT_ALLOWED);
			logger.error("the area not allowed unlock, playerId: {}, areaId: {}", player.getId(), buildArenId);
			return false;
		}

		int chapterId = player.getData().getRookieMilestoneEntity().getCurrentChapterId();
		if (chapterId < cfg.getUnlockDramaLevel()) {
			sendError(protocol.getType(), Status.Error.BUILDING_AREA_UNLOCK_NOT_ALLOWED);
			logger.error("the area not allowed unlock, playerId: {}, areaId: {}, chapterId: {}", player.getId(), buildArenId, chapterId);
			return false;
		}
		...
		if (!cfg.isAllowedUnlock()) {
			sendError(protocol.getType(), Status.Error.BUILDING_AREA_UNLOCK_NOT_ALLOWED);
			logger.error("build area unlock not allowed, playerId: {}, areaId: {}", player.getId(), buildArenId);
			return false;
		}

		// 检测出征的军队信息
		List<ArmyInfo> armyList = new ArrayList<>();
		if (!checkPveTroopsReq(req.getPveInfo(), protocol.getType())) {
			logger.error("battleUnlockArea world attack monster failed,armyList:{}", armyList);
			return false;
		}
		// 获取 战斗进度
		Map<Integer, Integer> unlockAreaProgressMap = player.getData().getPlayerBaseEntity().getUnlockAreaProgressMap();
		int nextBattleProgress = unlockAreaProgressMap.getOrDefault(buildArenId, 0) + 1;

		List<Integer> battleNpcList = cfg.getBattleNpcList();
		if (battleNpcList.size() < nextBattleProgress) {
			nextBattleProgress = battleNpcList.size();
		}
		int npcId = battleNpcList.get(nextBattleProgress - 1);
		// 战斗信息
		HeadOnBattleResult result = BattleService.getInstance().headOnBattleResult(player, req.getPveInfo(), npcId);
		if (result.isAtkWin()) {
			// 战斗成功 更新战斗进度
			player.getPlayerBaseEntity().addUnlockAreaProgress(buildArenId, nextBattleProgress);
			// 刷新任务
			MissionManager.getInstance().postMsg(player, new EventAreaProgress(buildArenId, nextBattleProgress));
			// 进度和配置相同：解锁区块
			if (battleNpcList.size() == nextBattleProgress) {
				unlockArea(buildArenId, cfg);
			}
		}
		World.BuildingAreaUnlockFightResp.Builder builder = World.BuildingAreaUnlockFightResp.newBuilder();
		builder.setAreaId(buildArenId)
				.setBattleProgress(unlockAreaProgressMap.getOrDefault(buildArenId, 0))
				.setResult(result.isAtkWin() ? 1 : 0)
				.setAtkStrength(result.getAtkRealStrength());
		sendProtocol(HawkProtocol.valueOf(HP.code2.BUILDING_FIGHT_AREA_UNLOCK_S_VALUE, builder));
		return true;
	}
	 ...
	 * 检查pve战斗出兵数据
	 */
	public boolean checkPveTroopsReq(World.PveBattleTroopsInfo req, int hpCode) {
		List<Army.ArmySoldierPB> reqArmyList = req.getArmyInfoList();
		if (reqArmyList.isEmpty()) {
			sendError(hpCode, Status.Error.WORLD_MARCH_NO_ARMY);
			return false;
		}

		for (Army.ArmySoldierPB marchArmy : reqArmyList) {
			if (marchArmy.getCount() <= 0) {
				sendError(hpCode, Status.Error.WORLD_MARCH_NO_ARMY);
				return false;
			}
		}
	 ...
	 * 解锁区块
	 */
	private void unlockArea(int buildArenId, BuildAreaCfg cfg) {
		player.unlockArea(buildArenId);
		...
		// 解锁地块任务
		MissionManager.getInstance().postMsg(player, new EventUnlockGround(buildArenId));
		MissionManager.getInstance().postSuperSoldierTaskMsg(player, new SuperSoldierTriggeTaskMsg(SupersoldierTaskType.UNLOCK_AREA_TASK, 1));
		BehaviorLogger.log4Service(player, Source.USER_OPERATION, Action.BUIDING_AREA_UNLOCK, Params.valueOf("buildAreaId", buildArenId));
	}
