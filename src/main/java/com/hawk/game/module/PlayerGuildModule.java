...
public class PlayerGuildModule extends PlayerModule {
	...
	protected boolean onPlayerLogin() {
		...
		if (GsConfig.getInstance().isDebug()) {
			PlayerActiveStatService.getInstance().syncGuildActiveStatInfo(player);
		}
	...
	private boolean onCheckGuildName(HawkProtocol protocol) {
		...
		MsgCategory reportCategory = player.hasGuild() ? MsgCategory.GUILD_NAME_CHANGE : MsgCategory.GUILD_NAME;
		GameTssService.getInstance().wordUicChatFilter(player, req.getGuildName(), 
				reportCategory.getNumber(), GameMsgCategory.CHECK_GUILD_NAME, 
	...
	private boolean onGetRecommendGuild(HawkProtocol protocol) {
		...
		GetRecommendGuildListResp.Builder builder = GuildService.getInstance().onGetRecommendGuild(player, req.getPageNum(), player.getLanguage());
...
//		}
	...
	private boolean onApplyGuild(HawkProtocol protocol) {
		...
		GuildInfoObject guild = GuildService.getInstance().getGuildInfoObject(req.getGuildId());
		if(Objects.isNull(guild)){
			sendError(protocol.getType(), Status.Error.GUILD_NOT_EXIST_VALUE);
			return false;
		}
		long serverMergeJoinLimit = GuildService.getInstance().checkServerMergeJoinTimeLimit(this.player, guild);
		if(serverMergeJoinLimit > 0){
			player.sendError(protocol.getType(), Status.Error.SERVER_MERGE_GUILD_JOIN_LIMIT_VALUE,0,HawkTime.formatTime(serverMergeJoinLimit));
			return false;
		}
	...
	private boolean onAcceptApplyGuild(HawkProtocol protocol) {
		...
		String guildId = player.getGuildId();
		String joinerId = req.getPlayerId();
		Player Joiner = GlobalData.getInstance().makesurePlayer(joinerId);
	...
	private boolean onGuildInvite(HawkProtocol protocol) {
		...
		GuildInfoObject guild = GuildService.getInstance().getGuildInfoObject(this.player.getGuildId());
		if(Objects.isNull(guild)){
			sendError(protocol.getType(), Status.Error.GUILD_NOT_EXIST_VALUE);
			return false;
		}
		Player targetPlayer = GlobalData.getInstance().makesurePlayer(req.getPlayerId());
...
//		}
	...
	private boolean onEditMainForceInfo(HawkProtocol protocol) {
		...
		if (needUpdate) {
			...
			CustomDataEntity customDataEntity = player.getData().getCustomDataEntity(CustomKeyConst.PLAYER_MAIN_FORCE);
			if (customDataEntity == null) {
				player.getData().createCustomDataEntity(CustomKeyConst.PLAYER_MAIN_FORCE, 0, sb.toString());
			} else {
	 ...
	 * @return
	 */
	@ProtocolHandler(code = HP.code2.GUILD_ACTIVE_STAT_INFO_C_VALUE)
	private boolean queryGuildActiveInfo(HawkProtocol protocol) {
