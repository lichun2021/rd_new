...
public class YQZZBattleData {
	private static final String redisKey = "YQZZ_ACTIVITY_BATTLE_DATA_20251211";
	...
	public void parserData(PBYQZZGameInfoSync builder,String roomId){
		try {
			...
			for(PBYQZZGuildInfo guildInfo : guildInfoList){
				...
				data.setCamp(guildInfo.getCamp());
				//当时控制ID
			...
			for(PBYQZZPlayerInfo playerInfo : playerInfoList){
				...
				data.setCamp(playerInfo.getCamp());
				//刷怪记录
				...
				for(PBYQZZFoggyHonor yuri : yuris){
					...
					long guildScore = yuri.getGuildHonor();
					YQZZGameDataYuriKill guildYuriData = guildData.getYuriScore().get(level);
					if(guildYuriData == null){
						guildYuriData = new YQZZGameDataYuriKill(level, 0, 0l);
						guildData.getYuriScore().put(level, guildYuriData);
					}
					int guildYuriCount = guildYuriData.getCount() + yuri.getKillCount();
					long guildYuriScore = guildYuriData.getScore() + guildScore;
					guildYuriData.setCount(guildYuriCount);
					guildYuriData.setScore(guildYuriScore);
			...
			HawkLog.logPrintln("YQZZBattleData parserData player size:{},guildSize:{}",playerMap.size(),guildMap.size());
		} catch (Exception e) {
	...
	public static class YQZZGuildGameData{
		...
		public void setYuriScore(Map<Integer, YQZZGameDataYuriKill> yuriScore) {
			this.yuriScore = yuriScore;
		}
