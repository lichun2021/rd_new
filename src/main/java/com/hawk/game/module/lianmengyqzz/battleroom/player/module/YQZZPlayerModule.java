...
public class YQZZPlayerModule extends PlayerModule {
	...
	private void getSecondMap(HawkProtocol protocol) {
		String guildId = this.player.getGuildId();
		YQZZMatchRoomData roomData = YQZZMatchService.getInstance().getDataManger().getGuildYQZZMatchRoomData(guildId);
		if (roomData == null) {
			player.sendError(HP.code2.YQZZ_SECOND_MAP_C_VALUE,
					Status.YQZZError.YQZZ_WAR_NOT_IN_BATTLE_TIME_VALUE, 0);
			return;
		}
		YQZZGaiLanResp gailan = YQZZRoomManager.getInstance().getGaiLanResp(roomData.getRoomId());
		if (gailan == null) {
			player.sendError(HP.code2.YQZZ_SECOND_MAP_C_VALUE,
					Status.YQZZError.YQZZ_WAR_NOT_IN_BATTLE_TIME_VALUE, 0);
			return;
