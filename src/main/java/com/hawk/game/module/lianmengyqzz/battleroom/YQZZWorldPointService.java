...
public class YQZZWorldPointService {
	...
	private LinkedListMultimap<YQZZBuildType, IYQZZBuilding> builTypeMap = LinkedListMultimap.create();
	private LinkedListMultimap<Integer, IYQZZBuilding> builCircleMap = LinkedListMultimap.create();
	...
	private void refreshBuild(YQZZBuildCfg bcfg) {
		try {
			...
			case 8:
				...
				YQZZGuild guild = getParent().getGuildInfo(bornCamp);
				((YQZZBase) icd).setBornCamp(bornCamp);
				((YQZZBase) icd).setServerId(guild.getServerId());
				((YQZZBase) icd).setGuildId(guild.getGuildId());
			...
			builCircleMap.put(icd.getCircleNumber(), icd);
			index++;
			addViewPoint(icd);
		} catch (Exception e) {
	...
	public IYQZZBuilding randomBuildByType(YQZZBuildType type) {
		List<IYQZZBuilding> list = builTypeMap.get(type);
		if (list.isEmpty()) {
			return null;
		}
	...
	public IYQZZBuilding randomBuildByCircle(int circle) {
		List<IYQZZBuilding> list = builCircleMap.get(circle);
		if (list.isEmpty()) {
			return null;
		}
		int randomPointIndex = HawkRand.randInt();
