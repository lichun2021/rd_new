...
public abstract class IYQZZWorldMarch implements IWorldMarch {
	...
	public static boolean onArmyBack(final Player player, List<ArmyInfo> armyList, List<Integer> heroId, int supersoldierId, IWorldMarch worldMarch) {
		int totalWound = 0;
		int fuhuo = 0;
		double pct = player.getEffect().getEffVal(EffType.EFF1096) * GsConst.EFF_PER;
		for (ArmyInfo army : armyList) {
//			int deadCount = army.getDeadCount();
//			if (deadCount > 0) {
//				totalWound += deadCount;
//				int save = (int) (deadCount * pct);
//				army.setDeadCount(deadCount - save);
//				fuhuo += save;
//			}
			int woundedCount = army.getWoundedCount();
			if (woundedCount > 0) {
				totalWound += woundedCount;
				int save = (int) (woundedCount * pct);
				army.setWoundedCount(woundedCount - save);
				fuhuo += save;
			}
		}
		if (fuhuo > 0) {
			MailService.getInstance().sendMail(MailParames.newBuilder()
					...
					.setMailId(MailId.YQZZ_WOUNDARMY_OK)
					.addContents(totalWound, pct*100, fuhuo)
					.build());
		}
	...
	public void pushMarchEvent(MarchEvent eventType) {
		...
		for (IYQZZPlayer player : getParent().getParent().getPlayerList(YQZZState.GAMEING)) {
			int[] pos = player.getWorldMovePos();
			if (pos == null || !isInview(pos[0], pos[1])) {
				continue;
			}
