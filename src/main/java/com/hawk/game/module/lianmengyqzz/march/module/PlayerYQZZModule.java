...
public class PlayerYQZZModule extends PlayerModule {
	...
	public void onEnterInstance(HawkProtocol hawkProtocol) {	
		...
		YQZZMatchRoomData roomData = YQZZMatchService.getInstance().getDataManger().getGuildYQZZMatchRoomData(playerGuild);
		if(roomData == null){
			player.sendError(HP.code2.YQZZ_WAR_ENTER_INSTANCE_REQ_VALUE,
					Status.YQZZError.YQZZ_WAR_NOT_IN_BATTLE_TIME_VALUE, 0);
			return;
		...
		YQZZJoinGuild guildData = YQZZMatchService.getInstance().getDataManger().getBattleGuilds().get(playerGuild);
		if(guildData == null){
			player.sendError(HP.code2.YQZZ_WAR_ENTER_INSTANCE_REQ_VALUE,
					Status.YQZZError.YQZZ_WAR_NOT_IN_BATTLE_TIME_VALUE, 0);
			return;
		...
		if(!guildData.getFreePlayers().containsKey(player.getId())){
			...
			boolean result = YQZZMatchService.getInstance().getDataManger().addJoinExtraPlayer(playerGuild,player.getId(), joinExtraCount);
			if (!result) {
	...
	private int sourceCheckEnterInstance(String serverId) {
		...
		YQZZMatchRoomData roomData = YQZZMatchService.getInstance().getDataManger().getGuildYQZZMatchRoomData(player.getGuildId());
		if(roomData == null){
	...
	private HawkTuple2<String, Integer> getCrossToServerId() {
		...
		Map<String, YQZZJoinGuild> guildMap= YQZZMatchService.getInstance().getDataManger().getBattleGuilds();
		if(guildMap == null){
