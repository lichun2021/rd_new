...
import com.hawk.game.protocol.Status.YQZZError;
...
public class YQZZMarchModule extends PlayerModule {
	...
	private boolean pointMarchCallBack(HawkProtocol protocol, WorldPointMarchCallBackReq req, IYQZZWorldPoint worldPoint) {
	...
	public boolean checkMarchReq(IYQZZPlayer player, WorldMarchReq req, int hpCode, List<ArmyInfo> armyList,
		...
		if (worldPoint.getClass() != YQZZBarrier.class) {
			...
			if (tarScirl != circle) { // 不同圈层
				if (!player.getBase().passable(tarScirl, circle)) {
					YQZZError err = Status.YQZZError.YQZZ_NOT_CIRLE;
					if(tarScirl== YQZZConst.NEI2 || tarScirl == player.getBase().getCircleNumber()){
						err = Status.YQZZError.YQZZ_NO_TYPE9_BUILD;
					}
					player.sendError(hpCode, err);
					return false;
				}
			}
		}
