...
public class YQZZBase extends IYQZZBuilding {
	...
	public boolean passable(int from , int tar){
		if(from == tar){
			return true;
		}
		Set<Integer> passableCircle = passableCircle();
		return passableCircle.contains(from) && passableCircle.contains(tar);
	}
	
	public Set<Integer> passableCircle(){
		Set<Integer> canGo = new HashSet<>();
		canGo.add(getCircleNumber());
		if (controlBuildTypes.contains(YQZZBuildType.JIU)) {
			canGo.add(YQZZConst.NEI2);
		}
		return canGo;
	}
