...
//		return addCnt;
	...
	public int getHonor() {
		...
		int buildHonor = getParent().getCampBase(getGuildId()).buildPlayerHonor;
	...
	public int[] getWorldMovePos() {
		HawkAOIObj aoiObj = getParent().getWorldPointService().getWorldScene().getObjById(eye.getAoiObjId());
		if (Objects.isNull(aoiObj)) {
			return null;
		}
		return new int[]{aoiObj.getX(),aoiObj.getY()};
	}
