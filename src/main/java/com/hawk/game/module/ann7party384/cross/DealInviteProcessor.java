...
@Ann7PartyCrossProcessor.Declare(proto = HP.code2.ANN7_PARTY_DEAL_INVITE_C_VALUE)
public class DealInviteProcessor extends Ann7PartyCrossProcessor {

	@Override
	public void process(PartyServiceReq req) {
		Party384Room room = Ann7Party384Service.getInstance().getRoomById(req.getRoomId());
		if (room != null) {
			DealInviteReq.Builder reqInfo = DealInviteReq.newBuilder();
			try {
				reqInfo.mergeFrom(req.getReqData());
			} catch (InvalidProtocolBufferException e) {
				HawkException.catchException(e);
			}
			Ann7Party384Service.getInstance().dealInvite(reqInfo.getRoomId(), room, reqInfo.build(), false);
		}
	}
}
