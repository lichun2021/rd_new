...
@Ann7PartyCrossProcessor.Declare(proto = HP.code2.ANN7_PARTY_ROOM_KICKOUT_C_VALUE)
public class RoomKickoutProcessor extends Ann7PartyCrossProcessor {

	@Override
	public void process(PartyServiceReq req) {
		Party384Room room = Ann7Party384Service.getInstance().getRoomById(req.getRoomId());
		if (room != null) {
			KickoutMemberReq.Builder builder = KickoutMemberReq.newBuilder();
			try {
				builder.mergeFrom(req.getReqData());
			} catch (InvalidProtocolBufferException e) {
				HawkException.catchException(e);
			}
			Ann7Party384Service.getInstance().kickoutMember(room, builder.build(), false, 0);
		}
	}
