...
public class TBLYTickAbleTask extends HawkTask {
	...
	private ConcurrentSkipListMap<Integer, TBLYWorldPointUpdateEventFunc> noticyWorldPointUpdateMap = new ConcurrentSkipListMap<>();
	...
	public Object run() {
		...
		try {
			...
			while (!noticyWorldPointUpdateMap.isEmpty()) {
				// 移除当前时间节点集合
				TBLYWorldPointUpdateEventFunc func = noticyWorldPointUpdateMap.pollFirstEntry().getValue();
				func.apply(null);
			}
	...
	private void marchTick(ITBLYWorldMarch march) {
		try {
			...
			long costTimeMs = HawkTime.getMillisecond() - beginTimeMs;
			if (costTimeMs > 50) {
				DungeonRedisLog.log(battleId, "{} march tick too much time, start:{} costtime: {} Origion:{},{} Terminal:{},{} marchtype: {} marchstatus: {} -> {} army:{} -> {}",
						march.getMarchId(), HawkTime.formatTime(beginTimeMs), costTimeMs,
	...
	public void doUpdateWorldPoint(TBLYWorldPointUpdateEventFunc func) {
		if(func.isDelete()){
			func.apply(null);
			return;
		}
		noticyWorldPointUpdateMap.put(func.getPoint().getPointId(), func);
	}
