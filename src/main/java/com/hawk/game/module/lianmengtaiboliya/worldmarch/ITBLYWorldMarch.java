...
public abstract class ITBLYWorldMarch implements IWorldMarch {
	...
	public void notifyMarchEvent(MarchEvent eventType) {
		...
		TBLYNotifyMarchEventFunc func = new TBLYNotifyMarchEventFunc();
		func.setMarch(this);
	...
	public void pushMarchEvent(MarchEvent eventType) {
		Multimap<WorldMarchRelation, String> csMap = HashMultimap.create();
		for (ITBLYPlayer player : getParent().getParent().getPlayerList(TBLYState.GAMEING)) {
			...
			if (player.isCsPlayer()) {
				csMap.put(relation, player.getId());
			} else {
		...
		for (WorldMarchRelation relation : csMap.keySet()) {
			Set<String> playerIds = new HashSet<>(csMap.get(relation));
			MarchEventSync.Builder builder = createSyncPB(eventType, relation);
			HawkProtocol protocol = HawkProtocol.valueOf(HP.code.WORLD_MARCH_EVENT_SYNC_VALUE, builder);
			CrossProxy.getInstance().broadcastProtocolV2(getParent().getParent().getCsServerId(), playerIds, protocol);
		}
	...
	public boolean isInview(ITBLYPlayer player) {
		int[] aoiObj = player.getWorldMovePos();
		if (Objects.isNull(aoiObj)) {
			return false;
		}
		double dis = AlgorithmUtil.getDisPointToLine(aoiObj[0], aoiObj[1], getOrigionX(), getOrigionY(), getTerminalX(), getTerminalY());
		if (dis > 12) {
			return false;
		}
		return true;
	...
	private MarchEventSync.Builder createSyncPB(MarchEvent eventType, WorldMarchRelation relation) {
		...
		if (eventType != MarchEvent.MARCH_DELETE) {
			dataBuilder.setMarchPB(GameUtil.simplifiedMarchSyncPB(this.toBuilder(relation)));
		}
	...
	public void sendBattleResultInfo(boolean isWin, List<ArmyInfo> atkArmyList, List<ArmyInfo> defArmyList, boolean isMonsterDead) {
		...
		if (!csplayerIds.isEmpty()) {
			CrossProxy.getInstance().broadcastProtocolV2(getParent().getParent().getCsServerId(), csplayerIds, HawkProtocol.valueOf(HP.code.BATTLE_INFO_S_VALUE, builder));
		}
	...
	public double getMarchBaseSpeed() {
		...
		return WorldUtil.minSpeedInArmy(getParent(), this, armyList);
	}
