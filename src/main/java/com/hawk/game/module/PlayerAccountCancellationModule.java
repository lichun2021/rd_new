...
public class PlayerAccountCancellationModule extends PlayerModule {
	...
	private boolean onCheck(int hp) {
		...
		if (Ann7Party384Service.getInstance().playerInRoom(player.getId())) {
			sendError(hp, Status.Error.ANN7_PARTY_CANNOT_CANCEL_ACCOUNT_VALUE);
			return false;
		}
		return true;
	}
	...
	public int certification(String name, String cardNumber) {
		...
		try {
			...
			if (HawkOSOperator.isEmptyString(ret) || !json.containsKey("isCert") || !json.containsKey("isMatch")) {
				return -1;
			}
			
			int isCert = json.getInteger("isCert");
			int isMatch = json.getInteger("isMatch");
