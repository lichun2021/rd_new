...
public class PlayerQueueModule extends PlayerModule {
	...
	private boolean onQueueFinishFree(HawkProtocol protocol) {
		...
		//连续升级时，免费时间翻倍
		int multiply = queueEntity.getMultiply();
		if (multiply > 1){
			freeTime *= multiply;
		}
	...
	public QueueCustomData getQueueCustomData() {
		...
		CustomDataEntity customData = this.player.getData().getCustomDataEntity(CustomKeyConst.QUEUE_CUSTOM_DATA_KEY);
	...
	public void saveQueueCustomData(QueueCustomData data) {
		CustomDataEntity customData = this.player.getData().getCustomDataEntity(CustomKeyConst.QUEUE_CUSTOM_DATA_KEY);
		if(Objects.isNull(customData)){
			customData = player.getData().createCustomDataEntity(CustomKeyConst.QUEUE_CUSTOM_DATA_KEY, 0, data.toDataString());
		}else{
	...
	private boolean queueSpeedByGold(QueueEntity queueEntity, int protoType) {
		...
		} else if (queueType == QueueType.SCIENCE_QUEUE || queueType == QueueType.PLANT_SCIENCE_QUEUE) {
			needTime -= player.getFreeTechTime() * queueEntity.getMultiply();
		}
