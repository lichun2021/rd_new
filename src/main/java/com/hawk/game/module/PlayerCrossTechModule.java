...
public class PlayerCrossTechModule extends PlayerModule {
	...
	protected boolean onLevelUp(HawkProtocol protocol) {
		...
		// 升级到目标科技配置
		CrossTechCfg targetCfg = HawkConfigManager.getInstance().getConfigByKey(CrossTechCfg.class, cfgId);
		// 科技配置数据错误
		if (targetCfg == null) {
			logger.error("crossTech level up failed, tech config error, playerId: {}, targetCfg: {}", player.getId(), cfgId);
			sendError(protocol.getType(), Status.SysError.CONFIG_ERROR_VALUE);
			return false;
		}

		CrossTechEntity entity = player.getData().getCrossTechEntityByTechId(targetCfg.getTechId());
		if (entity == null) {
			entity = player.getData().createCrossTechEntity(targetCfg);
		}
		// 科技已达到对应的等级
		if (entity.getLevel() >= targetCfg.getLevel()) {
			logger.error("crossTech level up failed, lvl already arrive, playerId: {}, targetCfg: {}", player.getId(), cfgId);
		...
		if (multi >= 2) {
			...
			if (Objects.isNull(limitCfg)) {
				...
				logger.error("CROSS_TECH_UPLEVEL onLevelUp failed, limit level cfg is null, playerId: {} cityLevel:{} techContinuousUpgradeCondition:{}", player.getId(), cityLevel, techContinuousUpgradeCondition);
				return false;
			}
			if (cityLevel < limitCfg.getLevel()) {
				sendError(protocol.getType(), Status.SysError.PARAMS_INVALID);
				logger.error("CROSS_TECH_UPLEVEL onLevelUp failed, city level is err, playerId: {}, cityLevel: {}", player.getId(), cityLevel);
				return false;
			}
			//一键升级超过上限
			if (multi > ConstProperty.getInstance().getTechContinuousUpgradeLimit()) {
				sendError(protocol.getType(), Status.SysError.PARAMS_INVALID);
				logger.error("CROSS_TECH_UPLEVEL onLevelUp failed, One-click upgrade exceeds the limit, playerId: {}, multi: {}", player.getId(), multi);
				return false;
			}
		}
		...
		for (int level = entity.getLevel() + 1; level <= targetCfg.getLevel(); level++) {
			int cfgIdByLv = CrossTechCfg.getCfgIdByTechAndLevel(targetCfg.getTechId(), level);
			CrossTechCfg cfg = HawkConfigManager.getInstance().getConfigByKey(CrossTechCfg.class, cfgIdByLv);
			...
			totalNeedTime += cfg.getLevelUpTime();
		...
		if (totalNeedTime <= 0) {
			logger.error("crossTech level up failed, time error, playerId: {}, cfgId: {} totalNeedTime: {} multi: {}", player.getId(), cfgId, totalNeedTime, multi);
			sendError(protocol.getType(), Status.SysError.PARAMS_INVALID_VALUE);
			return false;
		}
		...
		// 科技研究操作打点
		LogUtil.logCrossTechResearchOperation(player, targetCfg.getTechId(), entity.getLevel(), 1, multi);
		
		if (immediate) {
			techLevelUp(targetCfg.getTechId(), multi);
		} else {
			QueueService.getInstance().addReusableQueue(player, QueueType.CROSS_TECH_QUEUE_VALUE, QueueStatus.QUEUE_STATUS_COMMON_VALUE,
					String.valueOf(cfgId), BuildingType.AIRCRAFT_CARRIER_VALUE, totalNeedTime, costItems, GsConst.QueueReusage.CROSS_TECH_UPGRADE, multi);
		}

		BehaviorLogger.log4Service(player, Source.CROSS_TECH_OPERATION, Action.CROSS_TECH_LEVEL_UP, Params.valueOf("id", targetCfg.getId()), Params.valueOf("multi", multi));
		player.responseSuccess(protocol.getType());

		MissionManager.getInstance().postMsg(player, new EventStartTechUpgrade(targetCfg.getTechId()));
		return true;
	}
...
//		GameUtil.reduceByEffect(itemInfos, goldArr, oillArr, tombArr, stelArr);
	 ...
	 * @return
	 */
	private boolean techLevelUp(int techId, int multi) {
		...
		int afterLvl = beforeLvl + multi;
		if (afterLvl <= beforeLvl) {
			return false;
		}
		CrossTechCfg afterCfg = HawkConfigManager.getInstance().getConfigByKey(
				CrossTechCfg.class, CrossTechCfg.getCfgIdByTechAndLevel(techId, afterLvl));
		if (afterCfg == null) {
			return false;
		}
		for (int lv = beforeLvl; lv < afterLvl; lv++) {
		...
		// 科技研究操作打点
		LogUtil.logCrossTechResearchOperation(player, techId, entity.getLevel(), 2, multi);
		return true;
	}
