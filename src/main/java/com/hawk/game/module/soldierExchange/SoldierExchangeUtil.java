...
public class SoldierExchangeUtil {
	...
	private void changeArmour(ArmourEntity armour, int from, int to) {
		...
		for (ArmourImmortEffObject armourEffObject : new ArrayList<>(armour.getImmort().getArmourImmortEntity().getInfinityEffMap().values())) {
			// 将代替换的属性重置，防止转职后属性重复
			int oldAttrId = armourEffObject.getAttrId();
			changeArmourImmortEff(armourEffObject, from, to);
			int newAttrId = armourEffObject.getAttrId();
			if (oldAttrId != newAttrId) {
				armour.getImmort().getArmourImmortEntity().getInfinityEffMap().remove(oldAttrId);
				armour.getImmort().getArmourImmortEntity().getInfinityEffMap().put(newAttrId, armourEffObject);
			}
		}
		//转共鸣
		...
		int echoId = armour.getImmort().getArmourImmortEntity().getEchoId();
		...
		for (ArmourImmortEchoCfg armourImmortEchoCfg : echoCfgList) {
			if (armourImmortEchoCfg.getType() == to && armourImmortEchoCfg.getEchoType() == echoCfg.getEchoType()) {
				armour.getImmort().getArmourImmortEntity().setEchoId(armourImmortEchoCfg.getId());
				HawkLog.errPrintln("SoldierExchange-changeArmourEcho playerId:{},armourId:{},oldEcho:{},changeEcho:{}", player.getId(),
						armour.getId(), echoId, armourImmortEchoCfg.getId());
				break;
			}
		}
        //转核心
		List<ArmourEntity> armourEntities = player.getData().getArmourEntityList();
		for (ArmourEntity armourCore : armourEntities) {
			if (!armourCore.getImmort().isImmortCore()) {
				continue;
			}
			if (armourCore.getArmourId() != fromCfg.getArmourId()) {
				continue;
			}
			int coreArmourId = armourCore.getArmourId();
			armourCore.setArmourId(armour.getArmourId());
			armourCore.getExtraAttrEff().clear();
			armourCore.getExtraAttrEff().addAll(armour.getExtraAttrEff());
			armourCore.getStarEff().clear();
			armourCore.getStarEff().addAll(armour.getStarEff());
			armourCore.getSkillEff().clear();
			armourCore.getSkillEff().addAll(armour.getSkillEff());
			armourCore.setLock(false);
			armourCore.notifyUpdate();
			HawkLog.errPrintln("SoldierExchange-changeArmour playerId:{},coreArmourId:{},coreId:{},changeArmourId:{},changeArmour:{}", player.getId(),
					coreArmourId, armourCore.getId(), armour.getArmourId(), armour.getId());
		}
	}
	private static void changeArmourImmortEff(ArmourImmortEffObject armourEffObject, int from, int to) {
		ArmourImmortRandomAttrCfg fromCfg = HawkConfigManager.getInstance().getConfigByKey(ArmourImmortRandomAttrCfg.class, armourEffObject.getAttrId());
		if (fromCfg == null) {
			return;
		}
		...
		ConfigIterator<ArmourImmortRandomAttrCfg> iterator = HawkConfigManager.getInstance().getConfigIterator(ArmourImmortRandomAttrCfg.class);
		for (ArmourImmortRandomAttrCfg cfg : iterator) {
			if (cfg.getEffectGroup() == fromCfg.getEffectGroup() && cfg.getSoldierType().contains(to)) {
				armourEffObject.setAttrId(cfg.getId());
				armourEffObject.setEffectType(cfg.getEffectId());
				if (armourEffObject.getEffectValue() < cfg.getEffectValueMax()) {
					int toEffectValue = (int) Math.floor((double) armourEffObject.getEffectValue() / fromCfg.getEffectValueMax() * cfg.getEffectValueMax());
					armourEffObject.setEffectValue((toEffectValue));
				}else{
					armourEffObject.setEffectValue(cfg.getEffectValueMax());
				}
			}
		}
	}
