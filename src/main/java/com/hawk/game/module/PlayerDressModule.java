...
import com.hawk.game.protocol.Dress.LuxuryDressAchieveRewardReq;
import com.hawk.game.protocol.Dress.LuxurySkillCastReq;
...
public class PlayerDressModule extends PlayerModule {
	...
	protected boolean onPlayerLogin() {
		...
		//同步装扮技能数据
		LuxuryDressService.getInstance().syncSkillInfo(player);
		LuxuryDressService.getInstance().syncCollectLuxuryDress(player);
		return true;
	}
	...
	private boolean checkGodDressUpdate(){
		...
		while (iterator.hasNext()) {
			...
			if(ConstProperty.getInstance().isDressGodOpen() && dressCfg.getIsShowMyth() > 0 && dressInfo.getStartTime() + dressInfo.getContinueTime() >= now){
				CustomDataEntity customData = player.getData().getCustomDataEntity(CustomKeyConst.DRESS_GOD_ACTIVE);
				if(customData == null){
					player.getData().createCustomDataEntity(CustomKeyConst.DRESS_GOD_ACTIVE, 0, "");
					needUpdate = true;
				}
			}
		}
	...
	public void doPlayerDressAskReq(HawkProtocol protocol) {
		...
		try {
			...
			CustomDataEntity customSetEntity = tPlayer.getData().getCustomDataEntity(CustomKeyConst.PLAYER_DRESS_ASKREFUSE);
	 ...
	 * @param protocol
	 */
	@ProtocolHandler(code = HP.code2.LUXURY_SKILL_CAST_C_VALUE)
	public void doLuxurySkillCast(HawkProtocol protocol) {
		if (player.isCsPlayer() || player.isInDungeonMap()) {
			sendError(protocol.getType(), Status.Error.LUXURY_SKILL_CROSS_FORBID_VALUE);
			return;
		}
		LuxurySkillCastReq req = protocol.parseProtocol(LuxurySkillCastReq.getDefaultInstance());
		int skillId = req.getSkillId();
		SkillCfg skillCfg = HawkConfigManager.getInstance().getConfigByKey(SkillCfg.class, skillId);
		if (skillCfg == null) {
			HawkLog.logPrintln("cast luxury skill error config, playerId: {}, skillId: {}", player.getId(), skillId);
		...
		if (highendCfg == null) {
			HawkLog.logPrintln("cast luxury skill error config dress, playerId: {}, skillId: {}", player.getId(), skillId);
			sendError(protocol.getType(), Status.SysError.CONFIG_ERROR_VALUE);
			return;
		}
		if (!highendCfg.isLuxuryActiveSkill(skillId)) {
			HawkLog.logPrintln("cast luxury skill error not in luxuryActiveSkills, playerId: {}, skillId: {}", player.getId(), skillId);
			sendError(protocol.getType(), Status.SysError.PARAMS_INVALID_VALUE);
			return;
		}

		DressCfg dressCfg = HawkConfigManager.getInstance().getConfigByKey(DressCfg.class, highendCfg.getDressId());
		DressEntity dressEntity = player.getData().getDressEntity();
		if (!dressEntity.hasDress(dressCfg.getDressType(), dressCfg.getModelType())) {
			HawkLog.logPrintln("cast luxury skill error miss dress, playerId: {}, skillId: {}", player.getId(), skillId);
			sendError(protocol.getType(), Status.Error.LUXURY_SKILL_DRESS_MISS_VALUE);
			return;
		}
		...
		if (skillInfo != null) {
			long cdTimeLong = skillCfg.getCd() * 1000L;
			if (skillInfo.getCoolStartTime() + cdTimeLong > now) {
				sendError(protocol.getType(), Status.Error.LUXURY_SKILL_CD_COOLING_VALUE);
				HawkLog.logPrintln("cast luxury skill error working, playerId: {}, skillId: {}, effectStartTime: {}", player.getId(), skillId, skillInfo.getCoolStartTime());
				return; //冷却时间还未结束
			}
		}
		
		long effectStartTime = now; //LuxuryDressService.getInstance().getSkillEffectStartTime(skillId, now);
		int result = LuxuryDressService.getInstance().castSkill(player.getId(), skillId, effectStartTime);
		if (result != 0) {
		...
		if (skillInfo == null) {
			skillInfo = new PlayerLuxuryInfo(skillId, highendCfg.getDressId(), now, 0);
			dressEntity.addSkillInfo(skillInfo);
		}
		skillInfo.setCastTime(now);
		skillInfo.setCoolStartTime(now);
		dressEntity.saveSkillInfo();
		LuxuryDressService.getInstance().syncSkillInfo(player);
		player.responseSuccess(protocol.getType());
		LuxuryDressService.getInstance().syncSkill2OnlinePlayer(skillId);
		
		//全服广播
		CustomDataEntity gameSetData = player.getData().getCustomDataEntity(LuxuryDressService.LUXURY_GAME_SET);
		//value为1标识关闭提示广播，默认为0标识开启
		if (gameSetData == null || gameSetData.getValue() == 0) {
			ChatParames build = ChatParames.newBuilder()
					.setChatType(Const.ChatType.SPECIAL_BROADCAST)
					.setKey(Const.NoticeCfgId.LUXURY_SKILL_CAST)
					.addParms(player.getName(), skillId)
					.build();
	 ...
	 * @param protocol
	 */
	@ProtocolHandler(code = HP.code2.LUXURY_SKILL_INFO_REQ_VALUE)
	public void luxurySkillInfoReq(HawkProtocol protocol) {
		LuxuryDressService.getInstance().syncSkillInfo(player);
	}
	 ...
	 * @param protocol
	 */
	@ProtocolHandler(code = HP.code2.LUXURY_DRESS_ACHIEVE_REWARD_C_VALUE)
	public void takeLuxuryDressAchieveReward(HawkProtocol protocol) {
		LuxuryDressAchieveRewardReq req = protocol.parseProtocol(LuxuryDressAchieveRewardReq.getDefaultInstance());
		int cfgId = req.getAchieveId();
		int result = LuxuryDressService.getInstance().takeLuxuryDressAchieveReward(player, cfgId);
		if (result != 0) {
			sendError(protocol.getType(), result);
			return;
		}
	 ...
	 */
	@ProtocolHandler(code = HP.code2.LUXURY_PROTECT_SHOW_SET_REQ_VALUE)
	public void luxuyProtectEndShowSet(HawkProtocol protocol) {
		LuxuyProtectShowSetReq req = protocol.parseProtocol(LuxuyProtectShowSetReq.getDefaultInstance());
		LuxuryDressService.getInstance().luxuyProtectEndShowSet(player, req);
		HawkLog.logPrintln("luxuyProtectEndShowSet, playerId: {}, showList: {}", player.getId(), req.getShow2TypeList());
	}
