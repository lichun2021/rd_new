...
public class HomeLandConstKVCfg extends HawkConfigBase {
    ...
    protected List<Integer> npcRefreshTime;
    ...
    protected boolean assemble() {
        ...
        if (!HawkOSOperator.isEmptyString(issueTime)) {
            String[] timeArray = issueTime.split("_");
            List<Integer> intTimeArray = new ArrayList<>();
            for (String s : timeArray) {
                int intTime = Integer.parseInt(s);
                if(intTime<=0){
                    continue;
                }
                intTimeArray.add(intTime);
            }
            npcRefreshTime = intTimeArray;
        }
    ...
    public List<Integer> getNpcRefreshTime() {
        return npcRefreshTime;
    }
