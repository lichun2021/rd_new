...
public class HomeLandMapBlock {
     ...
     *排除集合后随机获取block
     */
    public int randStopPoint(List<Integer> excludeList) {
        TreeSet<Integer> filteredSet = new TreeSet<>();
        for (Integer num : stops) {
            if (!excludeList.contains(num)) {
                filteredSet.add(num);
            }
        }
        Random random = new Random();
        int randomIndex = random.nextInt(filteredSet.size());
        List<Integer> list = new ArrayList<>(filteredSet);
