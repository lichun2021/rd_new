...
public class HLMapBuildComp implements SerializJsonStrAble {
    ...
    public int checkEffectBuild(int buildType,int maxLevel) {
        int maxMapLevelNumber = 0;
        for (HomeLandBuilding value : buildingMap.values()) {
            HomeLandBuildingCfg mapBuildCfg = HawkConfigManager.getInstance().getConfigByKey(HomeLandBuildingCfg.class, value.getConfigId());
            if (mapBuildCfg == null) {
                continue;
            }
            if (mapBuildCfg.getBuildType() != buildType) {
                continue;
            }
            if (mapBuildCfg.getLevel() < maxLevel) {
                continue;
            }
            maxMapLevelNumber++;
        }
        return maxMapLevelNumber;
    }
}
