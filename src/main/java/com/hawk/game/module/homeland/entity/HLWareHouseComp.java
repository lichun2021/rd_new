...
public class HLWareHouseComp implements SerializJsonStrAble {
     ...
     * @return
     */
    public int checkEffectWareHouse(int buildType,int maxLevel) {
        int maxEffectWareHouse = 0;
        for (HomeLandWareHouse value : wareHouseMap.values()) {
            HomeLandBuildingCfg wareHouseCfg = HawkConfigManager.getInstance().getConfigByKey(HomeLandBuildingCfg.class, value.getCfgId());
            if (wareHouseCfg == null) {
                continue;
            }
            if (wareHouseCfg.getBuildType() != buildType) {
                continue;
            }
            if (wareHouseCfg.getLevel() < maxLevel) {
                continue;
            }
            maxEffectWareHouse += value.getCount();
        }
        return maxEffectWareHouse;
    }
}
