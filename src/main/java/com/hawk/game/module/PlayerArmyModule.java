...
public class PlayerArmyModule extends PlayerModule {
    ...
    private long getNewlyOpTrainTime(int newlyFlag, boolean isImmediate) {
        ...
        // 普通坦克
        if (newlyFlag == NewlyDataType.COMMON_TANK_VALUE||newlyFlag == NewlyDataType.COMMON_TANK_2_VALUE) {
        	trainTime = ConstProperty.getInstance().getGuideFirstTrainSoldierTime();
        }
        // 普通飞机
        if (newlyFlag == NewlyDataType.COMMON_PLANE_VALUE||newlyFlag == NewlyDataType.COMMON_PLANE_2_VALUE) {
        	trainTime = ConstProperty.getInstance().getGuideFirstTrainSoldierTime();
        }
        // 立即步兵
