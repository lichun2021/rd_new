...
//			}
	...
	private void tblyShareIndoReq(HawkProtocol protocol){
		...
		CustomDataEntity customData = player.getData().getCustomDataEntity(CustomKeyConst.SHARE_TBLY_FAME_HALL_KEY);
		//如果为空就创建数据库事件
		if (customData == null) {
			customData = player.getData().createCustomDataEntity(CustomKeyConst.SHARE_TBLY_FAME_HALL_KEY, 0, "");
		}
		//已领取集合
