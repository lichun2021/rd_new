...
public class BattleSoldier_4 extends IPlanSoldier {
	...
	private void eff2205debuf() {
		if(getEffVal(EffType.EFF_2205) == 0 || getBattleRound() % ConstProperty.getInstance().effect2205AtkRound !=0){
			return;
		}
		List<BattleSoldier> enemys = getTroop().getEnemyTroop().getSoldierList().stream().filter(BattleSoldier::isYuanCheng).filter(BattleSoldier::canBeAttack).collect(Collectors.toCollection(ArrayList::new));
		Collections.shuffle(enemys);
		enemys.sort(Comparator.comparingInt((BattleSoldier e) -> e.debuff2205.debuffCnt));
		
		int lastRound = getBattleRound() + ConstProperty.getInstance().effect2205ContinueRound -1;
		for (int i = 0; i < effect2205AtkNums && i < enemys.size(); i++) {
			BattleSoldier tar = enemys.get(i);
			if (tar.debuff2205.startRound != getBattleRound()) {
				tar.debuff2205.startRound = getBattleRound();
				tar.debuff2205.endRound = lastRound;
				tar.debuff2205.debuffVal = 0;
			}
			tar.debuff2205.debuffCnt++;
			tar.debuff2205.debuffVal += getEffVal(EffType.EFF_2205);
			addDebugLog("【2205】直升 【锁定】:{} val:{} 持续:{} 记数:{} ", tar.getUUID(), tar.debuff2205.debuffVal,tar.debuff2205.endRound, tar.debuff2205.debuffCnt);
		}
	}
	...
	private void buff11094() {
		int cnt = getEffVal(EffType.EFF_11094);
		if (cnt > 0 && getTroop().forceFieldPct() > 0) {
		 List<BattleSoldier> tarList = getTroop().getSoldierList().stream()
				 .filter(s -> s.getType() == SoldierType.FOOT_SOLDIER_5||s.getType() == SoldierType.FOOT_SOLDIER_6) .filter(s -> s.isAlive())
				 .collect(Collectors.toList());
		 Collections.shuffle(tarList);
		 int  effect11094Maxinum = ConstProperty.getInstance().effect11094Maxinum;
		 int num = 0;
		 for(BattleSoldier soldier: tarList){
			 if(num >= cnt){
				 continue;
			 }
			 if(soldier.getBuff11094().size()>= effect11094Maxinum){
				 continue;
			 }
			 num++;
			 Buff11094 buff = new Buff11094();
			 buff.eff11095 = getEffVal(EffType.EFF_11095);
			 buff.eff11113 = getEffVal(EffType.EFF_11113);
			 soldier.getBuff11094().add(buff);
			 addDebugLog("11094~11095】回合开始，{} -> {}", getUUID(),soldier.getUUID());
		 }
	...
	public void attackOnce(BattleSoldier defSoldier, int atkTimes, double reducePer, int atkDis) {
		...
		if (trigerSkill_3) {
			skill403Atk(defSoldier, atkTimes, reducePer);
	...
	private void skill403Atk(BattleSoldier defSoldier, int atkTimes, double hurtPer) {
		...
		super.doAttackOnce(assaultSoldier, atkTimes, hurtPer, Integer.MAX_VALUE, false);
	...
	private void put12062Debuff(BattleSoldier tar12062) {
		if (Objects.nonNull(tar12062)) {
			super.doAttackOnce(tar12062, QIAN_PAI_MAX, 0, Integer.MAX_VALUE, false);
	...
	public int skillHurtExactly(BattleSoldier defSoldier) {
		...
		if (trigerSkill_3_1532) {
			skillPer = skillPer + getEffVal(EffType.S403_1533);
		}
