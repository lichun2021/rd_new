...
import com.hawk.game.battle.effect.impl.oldeff.Checker1524;
import com.hawk.game.battle.effect.impl.oldeff.Checker1525;
...
public class BattleSoldier_6 extends IFootSoldier {
	...
	boolean atk11098;
	Hero1116Param hero1116 = null;
	private AtomicLongMap<String> eff2204Cnt = AtomicLongMap.create();
	@Override
	...
	public int skillFireAtkExactly(BattleSoldier defSoldier) {
		...
		result = result + hero1116.eff12725Val;
		return result;
	}
	...
	private boolean eff12361Atk(BattleSoldier defSoldier) {
		...
		try {
			boolean trigger = HawkRand.randInt(10000) < ConstProperty.getInstance().getEffect12361BasePro() + getEffVal(EffType.HERO_1673) + getEffVal(EffType.HERO_12381); 
			...
			eff12361Atk = true;
			super.doAttackOnce(result, QIAN_PAI_MAX, 0, Integer.MAX_VALUE);
	...
	private void atk11098(BattleSoldier defSoldier) {
		if(atk11098 || getTroop().forceFieldPct() == 0 || getEffVal(EffType.EFF_11098) == 0){
			return;
		}
		atk11098 = true;
		
		Debuff11098 debuff = new Debuff11098();
		debuff.from = this;
		debuff.round = getBattleRound() + getEffVal(EffType.EFF_11098);
		debuff.eff11099 = getEffVal(EffType.EFF_11099);
		debuff.eff11115 = (int) (getEffVal(EffType.EFF_11115) * getTroop().forceFieldPct());
		
		defSoldier.getDebuff11098().add(debuff);
		addDebugLog("###【11098~11099】每回合一次，自身出征数量最多的狙击兵，对目标{}造成伤害时施加持续伤害，持续{} {}", defSoldier.getUUID(), debuff.round ,debuff.eff11099);
	}
	...
	private void hero12363Check(BattleSoldier defSoldier) {
		if(getEffVal(EffType.HERO_12363)==0){
			return;
		}
		...
		for(RandomContent<BattleSoldier> sc: objList){
			...
			eff12363Atk = true;
			super.doAttackOnce(sc.getObj(), QIAN_PAI_MAX, 0, Integer.MAX_VALUE);
			eff12363Atk = false;
		}
	}
	...
	public int skillHurtExactly(BattleSoldier defSoldier) {
		...
		if (isSkill5Round) {
			...
			skillPer += skill.getP1IntVal() + p1Add;
		}
		...
		if (trigerSkill(skill, NumberUtils.toInt(skill.getP3()))) {
			skillPer += NumberUtils.toInt(skill.getP4());
		}
		if (trigerSkill(skill, skill.getP1IntVal())) {
			skillPer += NumberUtils.toInt(skill.getP2());
		}
		...
		if (getEffVal(EffType.ARMOUR_11007) > 0 && hasHouPai()) {
			skillPer += getEffVal(EffType.ARMOUR_11007);
		...
		if (eff2204Cnt.incrementAndGet(defSoldier.getUUID()) % eff2204Atkround == 0) {
			skillPer += getEffVal(EffType.EFF_2204);
			addDebugLog("【2204】狙击伤害增加 {}", getEffVal(EffType.EFF_2204));
		}
		return skillPer;
	}
	...
	public double addHurtValPct(BattleSoldier defSoldier, double hurtVal) {
		...
		if (eff12361Atk) {
			...
			hurtVal = hurtVal * (getEffVal(EffType.HERO_12361) + getEffVal(EffType.EFF12721)) * GsConst.EFF_PER
					* xishu * GsConst.EFF_PER;
			addDebugLog("【12361】定点猎杀 {} ,12361: {} 12721:{} ,{}", defSoldier.getUUID(), getEffVal(EffType.HERO_12361),getEffVal(EffType.EFF12721), xishu);
		}
		if (eff12363Atk) {
	...
	public int eff2204Atkround = 100000;
