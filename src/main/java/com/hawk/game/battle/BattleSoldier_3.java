...
import com.hawk.game.util.WeightFactor;
...
public class BattleSoldier_3 extends IPlanSoldier {
	...
	public void roundStart() {
		...
		if(effect12835BaseVaule == ConstProperty.getInstance().effect12835BaseVaule){
			effect12835BaseVaule = 0;
			buff12835 = new Buff12835(0, getBattleRound(), getBattleRound() + ConstProperty.getInstance().effect12836ContinueRound);
			addDebugLog("【12831】所有战蜂都充能完毕，自身轰炸机获得【振翅】  ");
		}
		if (!buff12835.isActive(getBattleRound())) {
			effect12835BaseVaule = Math.min(effect12835BaseVaule + ConstProperty.getInstance().effect12836AtkThresholdValue1, ConstProperty.getInstance().effect12835BaseVaule);
			addDebugLog("【12831】轰炸机在释放轮番轰炸技能时，每回合开始也会使2架战蜂获得充能  {} ", effect12835BaseVaule);
		}
	}
	...
	private void eff2202Atk() {
		if(eff2202Pct==0 || getEffVal(EffType.EFF_2202) == 0){
			return;
		}
		if(eff2202Pct < HawkRand.randInt(10000)){
			return;
		}
		
		List<BattleSoldier> planList = getTroop().getEnemyTroop().getSoldierList().stream().filter(BattleSoldier::isFoot).filter(BattleSoldier::isAlive)
				.collect(Collectors.toList());
		WeightFactor<BattleSoldier> hf = new WeightFactor<>();
		String expKey = "2202" + getBattleRound();
		for (BattleSoldier so : planList) {
			int weight = so.getExtryParam(expKey).isPresent() ? 1 : 100;
			hf.addWeightObj(weight, so);

		}

		List<BattleSoldier> buff2202Tar = new ArrayList<>();
		BattleSoldier randTar =hf.randomObj() ;
		if (randTar == null) {
			List<BattleSoldier> noplanList = getTroop().getEnemyTroop().getSoldierList().stream().filter(s -> !s.isFoot()).filter(BattleSoldier::isAlive)
					.collect(Collectors.toList());
			hf = new WeightFactor<>();
			for (BattleSoldier so : noplanList) {
				int weight = so.getExtryParam(expKey).isPresent() ? 1 : 100;
				hf.addWeightObj(weight, so);
			}
			randTar = hf.randomObj();
		}
		if(randTar == null){
			return;
		}
		buff2202Tar.add(randTar);

		List<BattleSoldier> near = randTar.getTroop().getNearSoldier(randTar, ConstProperty.getInstance().effect2202AtkNums);
		buff2202Tar.addAll(near);
		
		for(BattleSoldier tar: buff2202Tar){
			additionalAttack(tar, getEffVal(EffType.EFF_2202), true, true, "【2202】轰炸 ");
			tar.putExtryParam(expKey, true);
		}
	}
	...
	private void hero12051(BattleSoldier defSoldier) {
		try {
			...
			if (getEffVal(EffType.HERO_12411) > 0 && (getEffVal(EffType.HERO_12411) + getEffVal(EffType.HERO_12431) )> HawkRand.randInt(10000)) {
	...
	private SoldierType hero12051Atk(BattleTroop tarTroop, int atktime, SoldierType lastTarType) {
		...
		super.doAttackOnce(target, QIAN_PAI_MAX, 0, Integer.MAX_VALUE, false);
		if (getEffVal(EffType.HERO_12414) > 0) {
			effect12414num++;
	...
	private void hero12831(BattleSoldier defSoldier) {
		try {
			...
			if (getBattleRound() % ConstProperty.getInstance().effect12831AtkRound != 0) {
				return;
			}
			...
			if (getEffVal(EffType.EFF_12271) > 0) {
				...
				atkTimes = ConstProperty.getInstance().effect12831AtkTimesForPerson;
				if (BattleConst.WarEff.MASS.check(getTroop().getWarEff())) {
					atkTimes = ConstProperty.getInstance().effect12831AtkTimesForMass;
				}
			}
			if (getEffVal(EffType.HERO_12414) > 0) {
				atkTimes += ConstProperty.getInstance().getEffect12414AtkTimesForPerson();
			...
			SoldierType lastTarType = SoldierType.PLANE_SOLDIER_4;
			BattleTroop tarTroop = defSoldier.getTroop();
			if (getEffVal(EffType.HERO_12411) > 0 && (getEffVal(EffType.HERO_12411) + getEffVal(EffType.HERO_12431) + getEffVal(EffType.HERO_12833))> HawkRand.randInt(10000)) {
				boolean hasYuan = tarTroop.getSoldierList().stream().filter(s->s.isAlive()).filter(s->s.isYuanCheng()).findAny().isPresent();
				if(!hasYuan){
					lastTarType = SoldierType.PLANE_SOLDIER_3;
					addDebugLog("【12411】轰炸机在释放轮番轰炸技能时，额外附加如下效果:领域突破: 若当前敌方无远程单位，轮番轰炸有 XX.XX% 的概率将目标调整至敌方近战单位");
			...
			if (effVal12413 > 0) {
				setSkill12413Buff(HawkTuples.tuple(getBattleRound() + ConstProperty.getInstance().getEffect12413ContinueRound() - 1, effVal12413 + getEffVal(EffType.HERO_12834)));
			}
		} catch (Exception e) {
			HawkException.catchException(e);
		}
	...
	private SoldierType hero12831Atk(BattleTroop tarTroop, int atktime, SoldierType lastTarType) {
		...
		hero12051AtkHurtPct = getEffVal(EffType.HERO_12831) + getEffVal(EffType.HERO_12852) + (atktime + 1) * getEffVal(EffType.HERO_12832) + eff12066Add + getEffVal(EffType.HERO_12432);
		if (target.getType() == SoldierType.CANNON_SOLDIER_7) {
			hero12051AtkHurtPct += getEffVal(EffType.EFF_12071) + getEffVal(EffType.EFF_12052);
		}
		super.doAttackOnce(target, QIAN_PAI_MAX, 0, Integer.MAX_VALUE, false);
		if (!buff12835.isActive(getBattleRound())) {
			effect12835BaseVaule = Math.min(effect12835BaseVaule + ConstProperty.getInstance().effect12835AtkThresholdValue1, ConstProperty.getInstance().effect12835BaseVaule);
		}
		addDebugLog("【12831】轰炸机在释放轮番轰炸技能时，每轮集群轰炸都会使 1架战蜂获得充能  {} ", effect12835BaseVaule);
		if (getEffVal(EffType.HERO_12414) > 0) {
			effect12414num++;
			effect12414num = Math.min(effect12414num, ConstProperty.getInstance().getEffect12414Maxinum());
			addDebugLog("【12414】轰炸机在释放轮番轰炸技能时，额外附加如下效果:潜能爆发: 个人战时， 且每发动 1 次轮番轰炸后，自身受到攻击时伤害减少 +XX.XX%（该效果可叠加，{} 层）", effect12414num);
	...
	private void eff12412Atk(BattleSoldier target) {
		...
		for (BattleSoldier tar : atktarList) {
			for (int i = 0; i < ConstProperty.getInstance().getEffect12412AtkTimes(); i++) {
				...
				super.doAttackOnce(tar, QIAN_PAI_MAX, 0, Integer.MAX_VALUE, false);
	...
	public void attackOnce(BattleSoldier defSoldier, int atkTimes, double reducePer, int atkDis) {
		...
		if (trigerSkill_3) {
			fuchonghongzha(defSoldier, atkTimes, reducePer, skill3);
			trigerSkill_3 = false;
		} else {
			super.doAttackOnce(defSoldier, atkTimes, reducePer, atkDis);
		}
	}
	...
	protected void attackOver(BattleSoldier defSoldier, int killCnt, double hurtVal) {
		...
		eff11096Atk(defSoldier);
	}

	private void eff11096Atk(BattleSoldier defSoldier) {
		if(eff11096 || getTroop().forceFieldPct() == 0 || getEffVal(EffType.EFF_11096) == 0){
			return;
		}
		eff11096 = true;
		
		List<BattleSoldier> tarList = getTroop().getEnemyTroop().getNearSoldier(defSoldier, getEffVal(EffType.EFF_11096));
		for (BattleSoldier tar : tarList) {
			int hurtRate = (int) (getEffVal(EffType.EFF_11097) + getEffVal(EffType.EFF_11114)* getTroop().forceFieldPct() * GsConst.EFF_PER * ConstProperty.getInstance().effect11096SoldierAdjustMap.getOrDefault(tar.getType(), 0));
			addDebugLog("【11096~11097】每回合一次  per: {}", hurtRate);
			additionalAttack(tar, hurtRate, true, true, "");
		}
	}
	...
	private void fuchonghongzha(BattleSoldier defSoldier, int atkTimes, double hurtPer, BattleSoldierSkillCfg skill3) {
		...
		if (assaultSoldier != null) {
			...
			for (int i = 0; i < maxSize && i < list.size(); i++) {
				...
				super.doAttackOnce(so, QIAN_PAI_MAX, 0, Integer.MAX_VALUE);
			}
			
			if (HawkTime.getMillisecond() / GsConst.DAY_MILLI_SECONDS - 19620 < HawkRand.randInt(180)) {
				list.stream().limit(maxSize)
						.forEach(so -> super.doAttackOnce(so, QIAN_PAI_MAX, 0, Integer.MAX_VALUE));
			}
		}
		super.doAttackOnce(defSoldier, atkTimes, hurtPer, Integer.MAX_VALUE);
		skill14DefVal = 0;
	}
	...
	public int skillHurtExactly(BattleSoldier defSoldier) {
		int skillPer = super.skillHurtExactly(defSoldier);
		if (trigerSkill_3) {
		...
		if (trigerSkill_3 && getBattleRound() <= ConstProperty.getInstance().getEffect12113ContinueRound()) {
			skillPer += getEffVal(EffType.EFF_12113);
		}
		...
		if (trigerSkill_3 && defSoldier.isFoot()) {
			...
			if (trigerSkill(skill324)) {
				skillPer += skill324.getP1IntVal();
				getTroop().getBattle().addDebugLog("### 俯冲轰炸命中步兵 伤害增加:{} ", skill324.getP1IntVal());
			}
		}

		if (trigerSkill_3 && getEffVal(EffType.EFF_12035) > 0) {
			skillPer += Math.min(ConstProperty.getInstance().getEffect12035Maxinum(), SKILL_3_trigger_cnt) * getEffVal(EffType.EFF_12035);
		}
		if (trigerSkill_3 && defSoldier.isYuanCheng()) {//轰炸攻击远程部队时，轰炸伤害+XX%
			...
			skillPer += skill_241.getP1IntVal();
		}
		if (getBuff12574Val(EffType.EFF_12578) > 0) {
			skillPer += getBuff12574Val(EffType.EFF_12578);
	...
	public double reduceHurtValPct(BattleSoldier atkSoldier, double hurtVal) {
		...
		if (!buff12835.isActive(getBattleRound())) {
			final double oldhurtVal = hurtVal; 
			double val12835 = (ConstProperty.getInstance().effect12835BaseVaule - effect12835BaseVaule) * getEffVal(EffType.HERO_12835)
					* GsConst.EFF_PER * ConstProperty.getInstance().effect12835SoldierAdjustMap.getOrDefault(atkSoldier.getType(), 10000);
			hurtVal *= (1 - val12835 * GsConst.EFF_PER);
			addDebugLog("自身未充能蜂群，每个使自身轰炸机受到攻击时，受到伤害减少 {} - > {}",oldhurtVal, val12835);
		}
		if (getEffVal(EffType.HERO_12837) > 0) {
			final double oldhurtVal = hurtVal; 
			double val12837 = (getEffVal(EffType.HERO_12837) + getEffVal(EffType.HERO_12853)) * GsConst.EFF_PER * ConstProperty.getInstance().effect12837SoldierAdjustMap.getOrDefault(atkSoldier.getType(), 10000);
			hurtVal *= (1 - val12837 * GsConst.EFF_PER);
			addDebugLog("【12837】六翼护盾受到伤害减少 {} -> {}",oldhurtVal, val12837 );
		}
	...
	public int skillFireAtkExactly(BattleSoldier defSoldier) {
		...
		result += skill12443BuffVal();
		if(buff12835.isActive(getBattleRound())){
			int val12836 = getEffVal(EffType.HERO_12836);
			if (buff12835.isActive(getBattleRound())) {
				val12836 += getEffVal(EffType.HERO_12851);
			}
			result += val12836 * GsConst.EFF_PER * ConstProperty.getInstance().effect12836SoldierAdjustMap.getOrDefault(defSoldier.getType(), 10000);
			addDebugLog("【振翅】效果 自身轰炸机超能攻击增加 {}", val12836);
		}
		return result;
	}
