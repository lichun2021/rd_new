...
import com.hawk.game.battle.headOnBattle.HeadOnBattleResult;
import com.hawk.game.cfgElement.EffectObject;
import com.hawk.game.config.Battle2025StageCfg;
import com.hawk.game.config.Battle2025ValueHeroCfg;
import com.hawk.game.config.Battle2025ValueHeroSkillCfg;
import com.hawk.game.config.Battle2025ValueSoldierCfg;
import com.hawk.game.config.Battle2025ValueStrengthCfg;
import com.hawk.game.config.Battle2025ValueSuperCfg;
...
import com.hawk.game.protocol.Army;
...
public class BattleService extends HawkAppObj {
	...
	public BattleOutcome doBattle(IBattleIncome income) {
		...
		HawkLog.logPrintln("battlefield calc costtime: {}", HawkTime.getMillisecond() - beginTimeMs);
...
//		Collections.shuffle(armyList);
	...
	public List<BattleSoldier> buildBattleSoldierList(List<BattleUnity> unitList,List<BattleUnity> tarUnitList, BattleConst.WarEff troopEffType,BattleConst.WarEff tarTroopEffType, Set<SoldierType> tarTypeSet) {
		...
		for (BattleUnity unit : unitList) {
			...
			if(unit.getEffectParams().getRisePage() == null){
				SoldierType risePageType = unitStatic.getPlayerRisePageType(unit.getPlayerId());
				unit.getEffectParams().setRisePage(unit.getPlayer().buildPBHeroRiseShow(risePageType));
			}
		}
		for (BattleUnity tarunit : tarUnitList) {
			...
			if(tarunit.getEffectParams().getRisePage() == null){
				SoldierType risePageType = tarStatic.getPlayerRisePageType(tarunit.getPlayerId());
				tarunit.getEffectParams().setRisePage(tarunit.getPlayer().buildPBHeroRiseShow(risePageType));
			}
		}
	...
	public void calcArmyInfo(Map<String, List<ArmyInfo>> battleArmyMap, Map<String, List<ArmyInfo>> aftArmyMap,
		...
		if(battle.getType() == BattleType.ATTACK_GUNDAM_PVE || battle.getType() == BattleType.ATTACK_TREASURE_386_RES){ // 机甲无伤兵
			convertRate = 0;
		}
...
//	}
	...
	public List<PlayerHero> defCityHeros(Player defPlayer) {
		...
		int defHeroCount = 0;
		boolean noMarch = defPlayer.isInDungeonMap() ? false : WorldMarchService.getInstance().getPlayerMarch(defPlayer.getId()).isEmpty();
		if (Objects.nonNull(defHero1) && (noMarch || defHero1.getState() == PBHeroState.HERO_STATE_FREE)) {
			defHeroList.add(defHero1);
			defHeroCount = defHeroCount + 1;
		}
		if (Objects.nonNull(defHero2) && (noMarch || defHero2.getState() == PBHeroState.HERO_STATE_FREE)) {
		...
		try { // 记录玩家当时的英雄状态
			...
			BehaviorLogger.log4Service(defPlayer, Source.SYSTEM_OPERATION, Action.WORLD_MARCH_REACH_ATTACK_PLAYER,
					...
					Params.valueOf("marchesIfAlternate", marchIds),
					Params.valueOf("map", defPlayer.getDungeonMap())); // 如果英雄替补, 记录占用英雄的行军
		} catch (Exception e) {
			HawkException.catchException(e);
		}
		return defHeroList;
	 ...
	 * 模拟战斗数值对比
	 */
	public HeadOnBattleResult headOnBattleResult(Player atkPlayer, World.PveBattleTroopsInfo marchInfo, int npcId) {
		// NPC配置
		Battle2025StageCfg stageCfg = HawkConfigManager.getInstance().getConfigByKey(Battle2025StageCfg.class, npcId);
		if (stageCfg == null) {
			HawkLog.errPrintln("headOnBattleResult Battle2025StageCfg is null player{} npcId {}", atkPlayer.getId(), npcId);
			return new HeadOnBattleResult(false);
		}
		// 进攻部队强度计算 部队强度 = 士兵强度和 + 英雄强度 + 机甲强度 + atkPlayerStrength
		long atkStrength = 0;
		// 1. 士兵强度和 = 求和（ 士兵数量 * 单兵强度 ）
		long armyStrength = 0;
		for (Army.ArmySoldierPB marchArmy : marchInfo.getArmyInfoList()) {
			BattleSoldierCfg battleSoldierCfg = HawkConfigManager.getInstance().getConfigByKey(BattleSoldierCfg.class, marchArmy.getArmyId());
			if (battleSoldierCfg == null) {
				HawkLog.errPrintln("headOnBattleResult battleSoldierCfg is null player{} cfgId {}", atkPlayer.getId(), marchArmy.getArmyId());
				continue;
			}
			Battle2025ValueSoldierCfg soldierCfg = HawkConfigManager.getInstance().getConfigByKey(Battle2025ValueSoldierCfg.class, battleSoldierCfg.getLevel());
			if (soldierCfg == null) {
				HawkLog.errPrintln("headOnBattleResult soldierCfg is null player{} cfgId {}", atkPlayer.getId(), battleSoldierCfg.getLevel());
				continue;
			}
			armyStrength += ((long) marchArmy.getCount() * soldierCfg.getLevelFactor()) / ConstProperty.getInstance().getBattle2025ValueCoefficientBase();
			atkStrength += armyStrength;
		}
		//   2. 英雄强度 = 英雄基础 + 英雄等级 + 英雄星级 + 每个芯片等级
		long heroStrength = 0;
		for (Integer heroId : marchInfo.getHeroIdList()) {
			Optional<PlayerHero> heroByCfgId = atkPlayer.getHeroByCfgId(heroId);
			if (!heroByCfgId.isPresent()) {
				HawkLog.errPrintln("headOnBattleResult heroByCfgId is null player{} heroId {}", atkPlayer.getId(), heroId);
				continue;
			}
			PlayerHero playerHero = heroByCfgId.get();
			Battle2025ValueHeroCfg heroCfg = HawkConfigManager.getInstance().getConfigByKey(Battle2025ValueHeroCfg.class, heroId);
			if (heroCfg == null) {
				HawkLog.errPrintln("headOnBattleResult heroCfg is null player{} heroId {}", atkPlayer.getId(), heroId);
				continue;
			}
			heroStrength += heroCfg.getBaseValue() + (long) playerHero.getLevel() * heroCfg.getLevelValue() + (long) playerHero.getStar() * heroCfg.getStarValue();
			//  芯片强度 = 等级 * levelFactor
			for (SkillSlot passiveSkillSlot : playerHero.getSkillSlots()) {
				if (!passiveSkillSlot.isUnLock()) {
					continue;
				}
				IHeroSkill skill = passiveSkillSlot.getSkill();
				if (skill == null) {
					continue;
				}
				Battle2025ValueHeroSkillCfg heroSkillCfg = HawkConfigManager.getInstance().getConfigByKey(Battle2025ValueHeroSkillCfg.class, skill.getCfg().getSkillQuality());
				if (heroSkillCfg == null || heroSkillCfg.getLevelFactor() == 0) {
					continue;
				}
				heroStrength += (long) skill.getLevel() * heroSkillCfg.getLevelFactor();
			}
			atkStrength += heroStrength;
		}
		// 机甲强度 = 等级 * levelFactor
		long superSoldierStrength = 0;
		int superSoldierId = marchInfo.getSuperSoldierId();
		if (superSoldierId > 0) {
			Battle2025ValueSuperCfg superCfg = HawkConfigManager.getInstance().getConfigByKey(Battle2025ValueSuperCfg.class, 1);
			if (superCfg != null && superCfg.getLevelFactor() != 0) {
				Optional<SuperSoldier> superSoldierOpt = atkPlayer.getSuperSoldierByCfgId(superSoldierId);
				// 总等级
				int totalLevel = 0;
				if (superSoldierOpt.isPresent()) {
					SuperSoldier superSoldier = superSoldierOpt.get();
					for (SuperSoldierSkillSlot skillSlot : superSoldier.getSkillSlots()) {
						if (!skillSlot.isUnLock()) {
							continue;
						}
						ISuperSoldierSkill skill = skillSlot.getSkill();
						if (skill == null) {
							continue;
						}
						totalLevel += skill.getLevel();
					}
					superSoldierStrength += (long) totalLevel * superCfg.getLevelFactor();
					atkStrength += superSoldierStrength;
				}
			}
		}
		// 玩家真实战力计算
		long atkPlayerStrength = 0;
		Battle2025ValueStrengthCfg strengthCfg = HawkConfigManager.getInstance().getConfigByKey(Battle2025ValueStrengthCfg.class, 1);
		if (strengthCfg != null && strengthCfg.getStrengthFactor() > 0) {
			atkPlayerStrength = atkPlayer.getStrength();
			if (atkPlayerStrength > 0) {
				atkStrength += atkPlayerStrength * strengthCfg.getStrengthFactor();
			}
		}

		int[] coefficient = ConstProperty.getInstance().getBattle2025ValueCoefficient();
		int rand = HawkRand.randInt(coefficient[0], coefficient[1]);
		long realStrength = atkStrength * rand / ConstProperty.getInstance().getBattle2025ValueCoefficientBase();
		boolean atkWin = realStrength >= stageCfg.getCombatPower();
		HawkLog.logPrintln("headOnBattleResult playerId{} npcId{} atkWin{}, armyStrength{}, superSoldierStrength{}, heroStrength{} atkPlayerStrength{},atkStrength{} realStrength{} rand{}"
				, atkPlayer.getId(), npcId, atkWin, armyStrength, superSoldierStrength, heroStrength, atkPlayerStrength, atkStrength, realStrength, rand);
		// 拼装战斗结果
		return new HeadOnBattleResult(atkWin, armyStrength, superSoldierStrength, heroStrength, realStrength);
	}
}
