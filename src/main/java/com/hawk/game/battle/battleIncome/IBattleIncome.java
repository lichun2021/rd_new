...
public interface IBattleIncome {
	...
	/**取得英雄Id*/
	default EffectParams getAtkPlayerEffectParams(String playerId) {
		if(getAtkCalcParames().playerBattleUnity.containsKey(playerId)){
			return getAtkCalcParames().playerBattleUnity.get(playerId).getEffectParams();
		}
		return EffectParams.getDefaultVal();
	}

	default EffectParams getDefPlayerEffectParams(String playerId) {
		if(getDefCalcParames().playerBattleUnity.containsKey(playerId)){
			return getDefCalcParames().playerBattleUnity.get(playerId).getEffectParams();
		}
		return EffectParams.getDefaultVal();
	}
