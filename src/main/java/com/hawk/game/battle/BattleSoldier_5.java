...
import com.hawk.game.battle.effect.impl.forcefield.Debuff11100;
...
public class BattleSoldier_5 extends IFootSoldier {
	...
	Buff12616 buff12616;
	int skill46Cnt;
	List<Debuff11100> debuff11100 = new ArrayList<>();
	int buff11100;
	boolean debuff11100Add;
	public int eff2203Pct;
	@Override
	...
	private void eff2203Atk() {
		if(eff2203Pct==0 || getEffVal(EffType.EFF_2203) == 0){
			return;
		}
		if(eff2203Pct < HawkRand.randInt(10000)){
			return;
		}
		for(BattleSoldier tar: getTroop().getEnemyTroop().getSoldierList()){
			if(tar.canBeAttack() && tar.isJinZhan()){
				additionalAttack(tar, getEffVal(EffType.EFF_2203), true, true, "【2203】突击 回合开始时对敌方全体近战部队进行一次攻击");
			}
		}
		
	}

	private void debuff11100Atk() {
		debuff11100Add = false;
		for (Debuff11100 deb : debuff11100) {
			if (deb.round != getBattleRound()) {
				continue;
			}
			addDebugLog("【11100~11101】受到{} 的伤害 延后伤害为 {}", deb.atk.getUUID(), deb.hurt);
			deb.atk.additionalAttackFixHurtVal(this, deb.hurt, "");
		}
	...
	private void hero12202() {
		...
		for (int i = 0; i < effect12202AtkTimes; i++) {
			...
			for (BattleSoldier tar : jinzhanList) {
				super.doAttackOnce(tar, QIAN_PAI_MAX, 0, Integer.MAX_VALUE);
			}
		}
		sss12201Xuneng = 0;
	...
	public void attackOnce(BattleSoldier defSoldier, int atkTimes, double reducePer, int atkDis) {
		...
		if (skill != null && getHeros().contains(skill.getP1IntVal())) {
			...
			if (trigerSkill5) {
				super.doAttackOnce(defSoldier, QIAN_PAI_MAX, 0, getAtkDis());
				sss1669Check(defSoldier);
				attackCount++;
			}
		}
	...
	private void hero12611(BattleSoldier defSoldier) {
		...
		List<BattleSoldier> tarList = defSoldier.getTroop().getSoldierList().stream().filter(s -> s.canBeAttack()).filter(s -> s != defSoldier)
				.filter(s -> s.isJinZhan() == randtar.isJinZhan())
		...
		for (BattleSoldier tar : atkTars) {
			...
			doAttackOnce(tar, QIAN_PAI_MAX, reduceHurtPer, Integer.MAX_VALUE, false);
		}
	...
	private void mkxn1562Atk(BattleSoldier defSoldier) {
		...
		for (BattleSoldier target : targetList) {
			super.doAttackOnce(target, QIAN_PAI_MAX, 0, Integer.MAX_VALUE);
			sss1669Check(defSoldier);
		}
	...
	private void chuantouAttack(BattleSoldier defSoldier) {
		...
		if (getEffVal(EffType.HERO_1512) > 0 && HawkRand.randInt(GsConst.RANDOM_MYRIABIT_BASE) < ConstProperty.getInstance().getEffect1512Prob()) {
			...
			for (BattleSoldier defsold : soldierList) {
				...
				super.doAttackOnce(defsold, QIAN_PAI_MAX, 0, Integer.MAX_VALUE, false);
				sss1669Check(defSoldier);
				atkcount++;
				if (atkcount == 2) {
					break;
	...
	public double reduceHurtValPct(BattleSoldier atkSoldier, double hurtVal) {
		...
		if (!debuff11100Add && getTroop().forceFieldPct() > 0 && getEffVal(EffType.EFF_11100) > 0) {
			debuff11100Add = true;
			double reduceVal11100 = hurtVal * GsConst.EFF_PER * ConstProperty.getInstance().effect11100DelayDamage;
			Debuff11100 debuff = new Debuff11100();
			debuff.round = getBattleRound() + getEffVal(EffType.EFF_11100);
			debuff.hurt = reduceVal11100;
			debuff.atk = atkSoldier;
			debuff11100.add(debuff);

			hurtVal = hurtVal - reduceVal11100;
			buff11100 = (int) (getEffVal(EffType.EFF_11101) + getEffVal(EffType.EFF_11116) * getTroop().forceFieldPct());

			addDebugLog("【11100~11101】 {} 吸收 {} 超能功击+{}", getUUID(), reduceVal11100, buff11100);
		}
		return hurtVal;
	}
	...
	public int skillFireAtkExactly(BattleSoldier defSoldier) {
		...
		result = (int) (result + buff11100 * GsConst.EFF_PER * ConstProperty.getInstance().effect11100SoldierAdjustMap.getOrDefault(defSoldier.getType(), 10000));
		return result;
	}
	...
	public int skillHurtExactly(BattleSoldier defSoldier) {
		...
		if (trigerSkill5) {
			...
			skillPer += ((NumberUtils.toInt(skill.getP4()) - GsConst.EFF_RATE) + getEffVal(EffType.SOLDIER_SKILL_505_1089));
		}
		...
		if (trigerSkill(skill, EffType.SOLDIER_SKILL_TRIGER_503)) {
			skillPer = skillPer + skill.getP1IntVal() + getEffVal(EffType.SOLDIER_SKILL_503);
		}
		BattleSoldierSkillCfg skill7 = getSkill(PBSoldierSkill.FOOT_SOLDIER_5_SKILL_7);
		if (trigerSkill(skill7)) {
			skillPer = skillPer + skill7.getP1IntVal() + getEffVal(EffType.SOLDIER_SKILL_507_P1);
		}
		skillPer += skill514Cen * skill514CenEff;
		...
		if (getEffVal(EffType.ARMOUR_11009) > 0) {// EFF_1420
			...
			skillPer = skillPer + pct * getEffVal(EffType.ARMOUR_11009);
		...
		if (sss12202Triger && getEffVal(EffType.HERO_12204) > 0 && defSoldier.eff12163DebuffVal().second > 0) {
			skillPer += getEffVal(EffType.HERO_12204);
	...
	private int buff12617(BattleSoldier atk) {
		...
		int result = (int) (hero12617Cen * getEffVal(EffType.HERO_12617) * GsConst.EFF_PER
				* ConstProperty.getInstance().effect12617SoldierAdjustMap.getOrDefault(atk.getType(), 10000));
