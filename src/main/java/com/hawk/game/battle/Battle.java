...
public class Battle {
	...
	public BattleConst.Troop warfare() {
		...
		logger.info("battle playerId : {} pointId : {}", playerId, pointId);
		...
		effect12761Damage(attackerTroop, defencerTroop);
		effect11041Damage(attackerTroop, defencerTroop);
		effect12861Damage(attackerTroop, defencerTroop);
		...
		logger.info("battle playerId:{} over r:{} winTroop {} faf:{} , def:{} costtime : {}ms",
				playerId, round, winTroop, attackerTroop.getForceField(), defencerTroop.getForceField(), costtime);
		...
		try {
			...
			for (BattleSoldier soldier : eff11041Soldiers) {
				...
				for (BattleSoldier tar : tarses) {
					...
					soldier.doAttackOnce(tar, BattleSoldier.QIAN_PAI_MAX, 1 - hurtPer, Integer.MAX_VALUE, false);
	...
	private void effect12861Damage(BattleTroop attackerTroop, BattleTroop defencerTroop) {
		try {
			List<BattleSoldier> eff11041Soldiers = attackerTroop.getSoldierList().stream().filter(s -> s.getEffVal(EffType.EFF_12861) > 0).collect(Collectors.toList());
			if (eff11041Soldiers.isEmpty()) {
				return;
			}
			List<String> pids = defencerTroop.getSoldierList().stream().map(s -> s.getPlayerId()).distinct().collect(Collectors.toList());
			...
			for (BattleSoldier soldier : eff11041Soldiers) {
				String tarpid = pids.get(i % pids.size());
				i++;
				addDebugLog("【12861】进攻战斗时，在战斗开始时，随机选中敌方某指挥官的全部部队进行 1 轮核弹轰炸（伤害率：XX%；此效果对步兵 翻倍）");
				List<BattleSoldier> tarses = defencerTroop.getSoldierList(tarpid);
				for (BattleSoldier tar : tarses) {
					soldier.eff12861atk = true;
					soldier.doAttackOnce(tar, BattleSoldier.QIAN_PAI_MAX, 0, Integer.MAX_VALUE, false);
					soldier.eff12861atk = false;
				}
			}
	...
	private void effect12761Damage(BattleTroop attackerTroop, BattleTroop defencerTroop) {
		try {
			List<BattleSoldier> eff11041Soldiers = attackerTroop.getSoldierList().stream().filter(s -> s.getEffVal(EffType.EFF12761) > 0).collect(Collectors.toList());
			if (eff11041Soldiers.isEmpty()) {
				return;
			}
			List<String> pids = defencerTroop.getSoldierList().stream().map(s -> s.getPlayerId()).distinct().collect(Collectors.toList());
			...
			int index = 0;
			for (BattleSoldier soldier : eff11041Soldiers) {
				String tarpid = pids.get(index % pids.size());
				index++;
				addDebugLog("【12761】战斗效果：进攻战斗时，战斗开始");
				List<BattleSoldier> tarses = defencerTroop.getSoldierList(tarpid);
				for (int i = 0; i < ConstProperty.getInstance().effect12761AtkTimes; i++) {
					for (BattleSoldier tar : tarses) {
						if (!tar.canBeAttack()) {
							continue;
						}
						// 即实际伤害 = 伤害率 * 修正系数 *基础伤害 *（1 + 各类加成）
						double hurtPer = ConstProperty.getInstance().effect12761SoldierAdjustMap.getOrDefault(tar.getType(), 0) * GsConst.EFF_PER
								* soldier.getEffVal(EffType.EFF12761) * GsConst.EFF_PER;
						soldier.doAttackOnce(tar, BattleSoldier.QIAN_PAI_MAX, 1 - hurtPer, Integer.MAX_VALUE, false);
						List<BattleSoldier> nears = defencerTroop.getNearSoldier(tar, ConstProperty.getInstance().effect12761AtkNums);
						for (BattleSoldier near : nears) {
							hurtPer = ConstProperty.getInstance().effect12761SoldierAdjustMap.getOrDefault(near.getType(), 0) * GsConst.EFF_PER
									* soldier.getEffVal(EffType.EFF12761) * GsConst.EFF_PER;
							soldier.doAttackOnce(near, BattleSoldier.QIAN_PAI_MAX, 1 - hurtPer, Integer.MAX_VALUE, false);
						}
					}
				}
			}
	...
	private void saveDebugInfo(int costtime) {
		...
		if (saveDebugLog) {
			...
			LocalRedis.getInstance().updatePlayerBattleLog(battleId, logBuilder.toString());
		}
	}
