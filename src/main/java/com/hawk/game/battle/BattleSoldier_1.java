...
public class BattleSoldier_1 extends BattleSoldier {
	...
	private void buff11092() {
		if (getEffVal(EffType.EFF_11092) > 0 && getTroop().forceFieldPct() > 0) {
			Buff11092 buff = new Buff11092();
			buff.cnt = getEffVal(EffType.EFF_11092);
			buff.eff11093 = getEffVal(EffType.EFF_11093);
			buff.eff11112 = getEffVal(EffType.EFF_11112);
			
			for(BattleSoldier solider : getTroop().getSoldierList()){
				if(solider.isJinZhan() && solider.isAlive()){
					solider.getBuff11092().add(buff);
				}
			}
		}
	}
	...
	public int skillHurtExactly(BattleSoldier defSoldier) {
		...
		if (trigerSkill(skill_5, trigger5)) {
			skillPer = skillPer + skill_5.getP1IntVal();
		}
		return skillPer;
	...
	Set<Integer> buff2207p = new HashSet<>();
	private void buff2207(){
		if(getEffVal(EffType.EFF_2207)==0){
			return;
		}
		try {
			int index = lossRatePct()*100 / ConstProperty.getInstance().effect2207BaseVaule;
			if (index == 0 || buff2207p.contains(index)) {
				return;
			}
			buff2207p.add(index);
			...
			enemys.sort(Comparator.comparingInt((BattleSoldier e) -> e.buff2207.selectCnt));
			
			int lastRound = getBattleRound() + 1;
			for (int i = 0; i < effect2207AtkNums && i < enemys.size(); i++) {
				BattleSoldier tar = enemys.get(i);
				if (tar.buff2207.startRound != lastRound) {
					tar.buff2207.startRound = lastRound;
					tar.buff2207.endRound = lastRound;
					tar.buff2207.value = 0;
				}
				tar.buff2207.selectCnt ++;
				tar.buff2207.value += getEffVal(EffType.EFF_2207);
				addDebugLog("【2207】防坦 【联防】 {} val:{} 记数:{} ", tar.getUUID(), tar.buff2207.value, tar.buff2207.selectCnt);
			}
