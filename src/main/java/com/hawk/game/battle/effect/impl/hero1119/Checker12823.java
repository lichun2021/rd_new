...
@EffectChecker(effType = EffType.HERO_12823)
public class Checker12823 implements IChecker {
	/**
	 * 【12823】
  - 【万分比】【12823】任命在战备参谋部时，集结战斗开始前，集结全队出征攻城车数量每满 5（effect12823NumLimit） 万，自身攻城车受到攻击时，伤害减少 +XX.XX%（12823）（该效果可叠加，至多 20(effect12823Maxinum)层）
    - 战报相关
      - 于战报中隐藏
      - 不合并至精简战报中
    ...
    - 集结全队出征攻城车数量每满 5 万
      ...
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
        - 指定数值读取const表，字段effect12823NumLimit
          - 配置格式：绝对值
    ...
    - （该效果可叠加，至多 20 层）
      - 效果记录在己方部队身上，数值直接叠加；限制该作用号生效层数上限
        - 层数上限读取const表，字段effect12823Maxinum
          - 配置格式：绝对值
	 */
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		if (parames.type == SoldierType.CANNON_SOLDIER_7) {
			...
			int cen = Math.min(ConstProperty.getInstance().effect12823Maxinum, tankCnt/ConstProperty.getInstance().effect12823NumLimit);
