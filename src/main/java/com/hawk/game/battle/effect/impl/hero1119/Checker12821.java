...
/**
 *【12821】
- 【万分比】【12821】任命在战备参谋部时，集结战斗开始前，若集结全队出征战车数量不低于集结部队总数 10%(effect12821AllNumLimit)，己方全体部队攻击、防御、生命增加【固定数值(effect12821BaseVaule)  + XX.XX%（12821）】（多个乔希同时存在时，上述效果可叠加，至多 7  (effect12821Maxinum)层）->针对己方兵种留个内置系数effect12821SoldierAdjust
  - 战报相关
    - 于战报中展示
    - 合并至精简战报中
  - 仅对集结战斗生效（包含集结进攻和集结防守）
  - 在战斗开始前判定，满足条件后本次战斗全程生效
  - 集结战斗开始前，若集结全队出征战车数量不低于集结部队总数 10%
    - 数量1 = 集结全队出征携带的战车数量：采矿车（兵种类型 = 8）、攻城车（兵种类型 = 7）
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量2 = 集结部队的出征数量总和
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 >= 指定数值时生效
      - 指定数值读取const表，字段effect12821AllNumLimit
        - 配置格式：万分比
  - 己方全体部队攻击、防御、生命增加 【固定数值+XX.XX%】
    - 该作用号数值效果为【光环】效果，生效到己方全体部队上
    - 单个玩家提供数值 = 该作用号固定数值 + 英雄参谋值*技能系数/10000
      - 该作用号固定数值读取const表，字段effect12821BaseVaule
        - 配置格式：万分比
    - 该作用号为常规外围属性加成效果，与其他作用号累加计算
      - 即 实际属性 = 基础属性*（1 + 各类加成 +【本作用值】 * 自身兵种修正系数）
  - （多个莉赛同时存在时，上述效果可叠加，至多 7 层）
    ...
    - 效果记录在己方部队身上，数值直接叠加；限制该作用号生效层数上限
      ...
      - 层数上限读取const表，字段effect12821Maxinum
        - 配置格式：绝对值
  - ->针对己方兵种留个内置系数effect12821SoldierAdjust
    - 实际针对自身各兵种类型，单独配置系数；自身兵种修正系数 读取const表，字段effect12821SoldierAdjust
      - 配置格式：兵种类型id1_修正系数1，......兵种类型id8_修正系数8
        - 修正系数具体配置为万分比
    - 配置格式：万分比
 */
@BattleTupleType(tuple = { Type.ATK, Type.DEF, Type.HP })
@EffectChecker(effType = EffType.HERO_12821)
public class Checker12821 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		effPer = (int) (effPer * GsConst.EFF_PER * ConstProperty.getInstance().effect12821SoldierAdjustMap.getOrDefault(parames.type, 10000));
	...
	private int selectPlayer(CheckerParames parames) {
		...
		double tankCnt =  parames.unitStatic.getArmyCountMapMarch().get(SoldierType.CANNON_SOLDIER_7) + parames.unitStatic.getArmyCountMapMarch().get(SoldierType.CANNON_SOLDIER_8);
		if (tankCnt / total < ConstProperty.getInstance().effect12821AllNumLimit * GsConst.EFF_PER) {
			return 0;
		}
		...
		for (BattleUnity unity : parames.unityList) {
			...
			if (unity.getEffVal(effType()) > 0) {
				int effvalue = ConstProperty.getInstance().effect12821BaseVaule + unity.getEffVal(effType());
		...
		int val = valMap.entrySet().stream()
				...
				.limit(ConstProperty.getInstance().effect12821Maxinum)
