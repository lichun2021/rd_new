...
public class Hero1116Param {
	...
	public void roundStart() {
		this.eff12721 = parent.getEffVal(EffType.EFF12721) > 0;
		if (!eff12721) {
			return;
		}
		this.eff12727 = parent.getEffVal(EffType.EFF12727) + parent.getEffVal(EffType.EFF12744);
		this.eff12723Val = 0;
		this.eff12725Val = 0;
		this.eff12723TypeChange = false;
		this.eff12728 = 0;
		skill12722Targes.clear();
		if (parent.getEffVal(EffType.EFF12722) == 0 || parent.getBattleRound() % ConstProperty.getInstance().effect12722AtkRound != 0) {
			return;
		}
		...
		if (eff12723cnt >= ConstProperty.getInstance().effect12723NumLimit) {
			...
			this.eff12725Val = parent.getEffVal(EffType.EFF12725) + parent.getEffVal(EffType.EFF12743);
			newtype = 1;
			...
			this.eff12723Val = parent.getEffVal(EffType.EFF12723) + parent.getEffVal(EffType.EFF12742);
			newtype = 2;
			parent.addDebugLog("狙击形态 自身攻击增加 12723 : {} ", this.eff12723Val);
		}
		if (this.eff12723Type != newtype) {
			this.eff12723TypeChange = true;
			this.eff12728 = parent.getEffVal(EffType.EFF12728) + parent.getEffVal(EffType.EFF12745);
...
//	}
	...
	public void skill12722Atk(BattleSoldier defSoldier) {
		...
		if (this.eff12723Type == 1) {
			...
			debuff.eff12724 = parent.getEffVal(EffType.EFF12724);
	...
	public double reduceHurtValPct(BattleSoldier atkSoldier, double hurtVal) {
		...
		if (parent.getEffVal(EffType.EFF12729) > 0 && parent.getBattleRound() <= ConstProperty.getInstance().effect12729AtkRound) {
			int effVal12729 = (int) (parent.getEffVal(EffType.EFF12729) * GsConst.EFF_PER
		...
		if (parent.getEffVal(EffType.EFF12731) > 0) {
			hurtVal = hurtVal * GsConst.EFF_PER * (GsConst.EFF_RATE - parent.getEffVal(EffType.EFF12731));
			parent.addDebugLog("【12731】 受到伤害降低   {}", parent.getEffVal(EffType.EFF12731));
		}
...
//		if (eff12721Atk) {
//			hurtVal = hurtVal * parent.getEffVal(EffType.EFF12721) * GsConst.EFF_PER;
//			parent.addDebugLog("【12721】棱镜矩阵 {} hurtVal : {} ", defSoldier.getUUID(), hurtVal);
//		}

		if (parent.getEffVal(EffType.EFF12730) > 0 && parent.getBattleRound() >= ConstProperty.getInstance().effect12730AtkRound) {
			int effVal12730 = (int) (parent.getEffVal(EffType.EFF12730) * GsConst.EFF_PER
