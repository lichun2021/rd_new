...
@EffectChecker(effType = EffType.EFF_1000010)
public class Checkereffect2215BaseVaule2 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		if (isSoldier(parames.type)) {
			if (parames.unity.getEffVal(EffType.EFF_2215) > 0) {
				effPer = ConstProperty.getInstance().effect2215BaseVaule2;
			}
		}
