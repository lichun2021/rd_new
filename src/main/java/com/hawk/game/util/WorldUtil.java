...
public class WorldUtil {
	 ...
	 * @return
	 */
	public static double minSpeedInArmy(Player player, IWorldMarch iWorldMarch, List<ArmyInfo> armys) {
		...
		for (ArmyInfo armyInfo : armys) {
			...
			double speed = getBaseArmyEffectSpeed(player, iWorldMarch, armyCfg.getSpeed(), armyCfg.getType());
			if (minSpeed == 0) {
				minSpeed = speed;
				continue;
			}
	...
	private static double getBaseArmyEffectSpeed(Player player, IWorldMarch iWorldMarch, int speed, int armyType) {
		if (player == null) {
			return speed;
		}
		...
		if (WorldUtil.isAtkMonsterMarch(iWorldMarch) || marchType == WorldMarchType.NEW_MONSTER || WorldUtil.isAtkBossMarch(iWorldMarch)
				 || marchType == WorldMarchType.AGENCY_MARCH_MONSTER) {
			cannonEff += player.getEffect().getEffVal(EffType.EFF_CANNON_SPEED_MONSTER_1259, iWorldMarch.getMarchEntity().getEffectParams());
			tankEff += player.getEffect().getEffVal(EffType.EFF_TANK_SPEED_MONSTER_1260, iWorldMarch.getMarchEntity().getEffectParams());
			planeEff += player.getEffect().getEffVal(EffType.EFF_PLANE_SPEED_MONSTER_1261, iWorldMarch.getMarchEntity().getEffectParams());
			footEff += player.getEffect().getEffVal(EffType.EFF_FOOT_SPEED_MONSTER_1262, iWorldMarch.getMarchEntity().getEffectParams());
		}
		
		if (iWorldMarch.isMassJoinMarch()) {
			cannonEff += player.getEffect().getEffVal(EffType.EFF_CANNON_SPEED_JOIN_MASS_1264, iWorldMarch.getMarchEntity().getEffectParams());
			tankEff += player.getEffect().getEffVal(EffType.EFF_TANK_SPEED_JOIN_MASS_1265, iWorldMarch.getMarchEntity().getEffectParams());
			planeEff += player.getEffect().getEffVal(EffType.EFF_PLANE_SPEED_JOIN_MASS_1266, iWorldMarch.getMarchEntity().getEffectParams());
			footEff += player.getEffect().getEffVal(EffType.EFF_FOOT_SPEED_JOIN_MASS_1267, iWorldMarch.getMarchEntity().getEffectParams());
		}
		
		switch (armyType) {
		case SoldierType.TANK_SOLDIER_1_VALUE:
			int effTankSoldier1 =player.getEffect().getEffVal(EffType.MARCH_SPD_TANK_SOLDIER_1) + tankEff;
			effectSpeed = speed * (1 + effTankSoldier1 * GsConst.EFF_PER);
			break;
		case SoldierType.TANK_SOLDIER_2_VALUE:
			int effTankSoldier2 = player.getEffect().getEffVal(EffType.MARCH_SPD_TANK_SOLDIER_2) + tankEff;
			effectSpeed = speed * (1 + effTankSoldier2 * GsConst.EFF_PER);
			break;
		case SoldierType.PLANE_SOLDIER_3_VALUE:
			int effPlaneSoldier3 = player.getEffect().getEffVal(EffType.MARCH_SPD_PLANE_SOLDIER_3) + planeEff;
			effectSpeed = speed * (1 + effPlaneSoldier3 * GsConst.EFF_PER);
			break;
		case SoldierType.PLANE_SOLDIER_4_VALUE:
			int effPlaneSoldier4 = player.getEffect().getEffVal(EffType.MARCH_SPD_PLANE_SOLDIER_4) + planeEff;
			effectSpeed = speed * (1 + effPlaneSoldier4 * GsConst.EFF_PER);
			break;
		case SoldierType.FOOT_SOLDIER_5_VALUE:
			int effFootSoldier5 = player.getEffect().getEffVal(EffType.MARCH_SPD_FOOT_SOLDIER_5) + footEff;
			effectSpeed = speed * (1 + effFootSoldier5 * GsConst.EFF_PER);
			break;
		case SoldierType.FOOT_SOLDIER_6_VALUE:
			int effFootSoldier6 = player.getEffect().getEffVal(EffType.MARCH_SPD_FOOT_SOLDIER_6) + footEff;
			effectSpeed = speed * (1 + effFootSoldier6 * GsConst.EFF_PER);
			break;
		case SoldierType.CANNON_SOLDIER_7_VALUE:
			int effCannonSoldier7 = player.getEffect().getEffVal(EffType.MARCH_SPD_CANNON_SOLDIER_7) + cannonEff;
			effectSpeed = speed * (1 + effCannonSoldier7 * GsConst.EFF_PER);
			break;
		case SoldierType.CANNON_SOLDIER_8_VALUE:
			int effCannonSoldier8 = player.getEffect().getEffVal(EffType.MARCH_SPD_CANNON_SOLDIER_8) + cannonEff;
			effectSpeed = speed * (1 + effCannonSoldier8 * GsConst.EFF_PER);
			break;
		default:
			break;
		}
...
//		}
		...
		case WorldPointType.TREASURE_386_VALUE:
	 ...
	 * @return
	 */
	public static List<Point> searchFreeNearPointPos(int[] pos, int minDis, int maxDis) {
