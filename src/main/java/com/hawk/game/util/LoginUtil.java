...
import com.hawk.sdk.msdk.MSDKAuth;
...
public class LoginUtil {
	...
	public static PlayerEntity createNewPlayer(String playerId, String openid,String puid, String playerName, String serverId, HPLogin cmd) {
		...
		playerEntity.setName(playerName);
		playerEntity.setPlatform(LoginUtil.getLoginPlatform(cmd));
		playerEntity.setChannel(LoginUtil.getLoginChannel(cmd));
		...
		if (cmd.getPlatform().indexOf(MSDKAuth.platHarmony) >= 0) {
			JSONObject json = new JSONObject();
			json.put("channel", cmd.getChannel());
			json.put("platform", cmd.getPlatform());
			playerEntity.setOpenharmony(json.toJSONString());
		}
	 ...
	 * @return
	 */
	public static String getLoginPlatform(HPLogin cmd) {
		String platform = cmd.getPlatform();
		if (platform.indexOf(MSDKAuth.platHarmony) >= 0) {
			platform = "android";
		}
		return platform;
	}
	 ...
	 * @return
	 */
	public static String getLoginChannel(HPLogin cmd) {
		String channel = cmd.getChannel();
		String platform = cmd.getPlatform();
		if (platform.indexOf(MSDKAuth.platHarmony) >= 0 && channel.indexOf("WeChat") >= 0) {
			channel = "wx";
		}
		return channel;
	}
