...
public class BuilderUtil {
	...
	public static UnlockedAreaPB.Builder getUnlockedAreaBuilder(Player player) {
		...
		for (Entry<Integer, Integer> entry : player.getData().getPlayerBaseEntity().getUnlockAreaProgressMap().entrySet()) {
			builder.addFightAreaInfo(Building.BuildingFightAreaInfo.newBuilder().setAreaId(entry.getKey()).setBattleProgress(entry.getValue()).build());
		}
		return builder;
	}
...
//	}
	...
	public static ArmourInfo.Builder genArmourInfoBuilder(ArmourEntity armour) {
		...
		if (ArmourConstCfg.getInstance().canLevelImmort(armour.getLevel())) {
			armourInfo.setImmort(armour.getImmort().getArmourImmortEntity().toBuilder());
		}
		return armourInfo;
	}
