...
public class ConsumeHelper {
	...
	public void addConsume(ItemType type, int itemId, long count, boolean isGold) {
		this.addConsume(type, itemId, count, null, isGold);
	}

	public void addConsume(ItemType type, int itemId, long count, String uuid, boolean isGold) {
		if (count == 0) {
			return;
		}
		...
		//累加之后-检测越界
		this.addConsumeCheck(type, itemId, consume.count);
		...
		if (Objects.nonNull(uuid) && !isGold) { // 指定从某一堆扣除
			consume.addItemEntityCost(uuid, (int)count);
		}

	}
	
	public void addConsumeCheck(ItemType type, int itemId, long count){
		//消耗数量不可以为负
		if (count < 0) {
			throw new RuntimeException("consumeItem count 0 error!");
		}
		//4种资源消耗可以过int32
		if(type == ItemType.PLAYER_ATTR){
			if(itemId == PlayerAttr.GOLDORE_UNSAFE_VALUE || itemId == PlayerAttr.GOLDORE_VALUE ||
			   itemId == PlayerAttr.OIL_UNSAFE_VALUE || itemId == PlayerAttr.OIL_VALUE ||
			   itemId == PlayerAttr.STEEL_UNSAFE_VALUE || itemId == PlayerAttr.STEEL_VALUE ||
			   itemId == PlayerAttr.TOMBARTHITE_UNSAFE_VALUE || itemId == PlayerAttr.TOMBARTHITE_VALUE){
				return;
			}
		}
		//剩下的如果数量过界都不行
		if (count >= Integer.MAX_VALUE) {
			throw new RuntimeException("consumeItem count large error!");
		}
	}
	...
	public ItemConsumPiece addItemConsume(PlayerData playerData, ComsumeItem itemInfo) {
		...
		int needCount = (int)itemInfo.count;
	...
	public HPConsumeInfo beforeCheck(PlayerData playerData) {
		...
		for (ComsumeItem itemInfo : table.values()) {
			...
			switch (playerAttr) {
			case GOLD: {
				gold += (int)itemInfo.getCount();
				break;
			}

			case DIAMOND: {
				diamond += (int)itemInfo.getCount();
				break;
			}

			case LEVEL: {
				builder.setLevel(builder.getLevel() + (int)itemInfo.getCount());
				break;
			}

			case VIP_POINT: {
				builder.setVipExp(builder.getVipExp() + (int)itemInfo.getCount());
				break;
			}

			case VIT: {
				builder.setVit(builder.getVit() + (int)itemInfo.getCount());
				break;
			}

			case GUILD_CONTRIBUTION: {
				builder.setGuildContribution(builder.getGuildContribution() + (int)itemInfo.getCount());
				break;
			}
			...
			case COIN: {
				builder.setCoin(builder.getCoin() + (int)itemInfo.getCount());
				break;
			}

			case EXP: {
				builder.setExp(builder.getExp() + (int)itemInfo.getCount());
				break;
			}
