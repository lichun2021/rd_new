...
public class RedisProxy {
	 ...
	 * 不朽共鸣装备外显
	 */
	static final String EQUIP_IMMORT_ECHO_SHOW = "equip_immort_echo_show";
	...
	public List<ServerInfo> getServerList() {
	    ...
	    try (Jedis jedis = redisSession.getJedis()) {
	        do {
	            ScanResult<Map.Entry<String, String>> result = jedis.hscan(SERVER_LIST_KEY, cursor, scanParams);
	            cursor = result.getStringCursor();
	            HawkLog.debugPrintln("getServerList hscan cursor: {}", cursor);
	            for(Entry<String, String> entry : result.getResult()) {
	            	serverStrList.add(entry.getValue());
	                HawkLog.debugPrintln("getServerList hscan entry key: {}, value: {}", entry.getKey(), entry.getValue());
	            }
	    ...
	    for (String value : serverStrList) {
	...
	public boolean checkRegisterPuidControl(String openid) {
		...
		return GsConfig.getInstance().getServerId().equals(mainServer);
	}
	...
	public void updateYiwaTipMsg(String playerId, String tipMsg) {
		String key = YIWA_RED_POINT + ":" + playerId;
		redisSession.setString(key, tipMsg);
	...
	public String getYiwaTipMsg(String playerId) {
		String key = YIWA_RED_POINT + ":" + playerId;
		String newYiwa = redisSession.getString(key);
		// 新的key能取到，直接返回
		if (!HawkOSOperator.isEmptyString(newYiwa)) {
			return newYiwa;
		}
		
		// 老的key存储会造成线上大key问题： 1、大key访问可能会造成阻塞，2、大key造成的内存不均衡。
		String oldYiwa = redisSession.hGet(YIWA_RED_POINT, playerId);
		// 将信息转储到新的key上，同时从老的key上删除
		if (!HawkOSOperator.isEmptyString(oldYiwa)) {
			redisSession.setString(key, oldYiwa);
...
//		MILITARY_SHOP_BUY
	 ...
	 * @param show
	 */
	public void updateEquipEchoShow(String playerId, int show) {
		try {
			StatisManager.getInstance().incRedisKey(EQUIP_IMMORT_ECHO_SHOW);
			String key = EQUIP_IMMORT_ECHO_SHOW + ":" + playerId;
			redisSession.setString(key, String.valueOf(show));
		} catch (Exception e) {
			HawkException.catchException(e);
		}
	 ...
	 * @param playerId
	 */
	public int getEquipImmortShow(String playerId) {
		try {
			StatisManager.getInstance().incRedisKey(EQUIP_IMMORT_ECHO_SHOW);
			String key = EQUIP_IMMORT_ECHO_SHOW + ":" + playerId;
			String show = redisSession.getString(key);
	 ...
	 * @return
	 */
	public Map<String, String> getPlayerImmgrationLog(String playerId) {
		String playerLogKey = IMMGRATION_LOG + ":" + playerId;
		return redisSession.hGetAll(playerLogKey);
	}
	 ...
	 * @return
	 */
	public Set<Tuple> zRangeByScoreWithScores(String key, long min, long max, int expireSeconds) {
		try (Jedis jedis = redisSession.getJedis(); Pipeline pipeline = jedis.pipelined()) {
			Response<Set<Tuple>> resp = pipeline.zrangeByScoreWithScores(key, min, max);
			if (expireSeconds > 0) {
