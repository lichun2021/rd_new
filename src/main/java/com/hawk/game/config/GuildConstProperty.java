...
public class GuildConstProperty extends HawkConfigBase {
	...
	private final long alliancePercentage1;
	private final long alliancePercentage2;
	private final int allianceActivityRankRefresh;
	
	private final String scienceMaxServer;
	private final String scienceMaxId;
	
	private ImmutableList<String> scienceMaxServerList;
	private ImmutableList<Integer> scienceMaxIdList;
	...
	public GuildConstProperty() {
		...
		beInviteVipCD = 0;
		this.alliancePercentage1 = 0;
		this.alliancePercentage2 = 0;
		this.allianceActivityRankRefresh = 0;
		scienceMaxServer = "";
		scienceMaxId = "";
	}
	...
	protected boolean assemble() {
		...
		if (alliancePercentage1 <= 0 || alliancePercentage2 <= 0) {
			logger.error("guild_const.xml assemble error, alliancePercentage : 1:{} 2:{}", alliancePercentage1, alliancePercentage2);
			return false;
		}
		if (allianceActivityRankRefresh <= 0) {
			logger.error("guild_const.xml assemble error, allianceActivityRankRefresh : {}", allianceActivityRankRefresh);
			return false;
		}
		
		
		List<String> scienceMaxServerTemp = new ArrayList<>();
		if(!HawkOSOperator.isEmptyString(this.scienceMaxServer)){
			String[] strs = this.scienceMaxServer.trim().split(SerializeHelper.BETWEEN_ITEMS);
			for(String serverId :strs ){
				scienceMaxServerTemp.add(serverId);
			}
		}
		this.scienceMaxServerList = ImmutableList.copyOf(scienceMaxServerTemp);
		
		
		List<Integer> scienceMaxIdTemp = new ArrayList<>();
		if(!HawkOSOperator.isEmptyString(this.scienceMaxId)){
			String[] strs = this.scienceMaxId.trim().split(SerializeHelper.BETWEEN_ITEMS);
			for(String scienceId :strs ){
				scienceMaxIdTemp.add(Integer.parseInt(scienceId));
			}
		}
		this.scienceMaxIdList = ImmutableList.copyOf(scienceMaxIdTemp);
	...
	public long getAlliancePercentage1() {
		return alliancePercentage1;
	}

	public long getAlliancePercentage2() {
		return alliancePercentage2;
	}

	public int getAllianceActivityRankRefresh() {
		return allianceActivityRankRefresh;
	}
	
	public List<String> getScienceMaxServerList() {
		return scienceMaxServerList;
	}
	
	public ImmutableList<Integer> getScienceMaxIdList() {
		return scienceMaxIdList;
	}
}
