...
public class StarWarsTimeCfg extends HawkConfigBase {
	...
	protected boolean checkValid() {
		...
		long baseTermId = 0;
		long preEndTimeValue = Long.MAX_VALUE;
		...
		for (StarWarsTimeCfg timeCfg : it) {
			...
			if (timeCfg.getEndTimeValue() < timeCfg.getWarEndTimeThreeValue() || timeCfg.getWarEndTimeThreeValue() < timeCfg.getWarStartTimeThreeValue()
					|| timeCfg.getWarStartTimeThreeValue() < timeCfg.getMangeEndTimeThreeValue() || timeCfg.getMangeEndTimeThreeValue() < timeCfg.getWarEndTimeTwoValue()
					|| timeCfg.getWarEndTimeTwoValue() < timeCfg.getWarStartTimeTwoValue() || timeCfg.getWarStartTimeTwoValue() < timeCfg.getMangeEndTimeTwoValue()
					|| timeCfg.getMangeEndTimeTwoValue() < timeCfg.getWarEndTimeOneValue()
					|| timeCfg.getWarEndTimeOneValue() < timeCfg.getWarStartTimeOneValue() || timeCfg.getWarStartTimeOneValue() < timeCfg.getMangeEndTimeOneValue()
					|| timeCfg.getMangeEndTimeOneValue() < timeCfg.getMatchEndTimeValue() || timeCfg.getMatchEndTimeValue() < timeCfg.getSignEndTimeValue()
					|| timeCfg.getSignEndTimeValue() < timeCfg.getSignStartTimeValue()) {
				HawkLog.errPrintln("star_wars_time check valid failed, Time exists error, termId: {}", termId);
				checkResult = false;
			}
			if (preEndTimeValue == Long.MAX_VALUE) {
				preEndTimeValue = timeCfg.getEndTimeValue();
			} else {
				if (timeCfg.getSignStartTimeValue() <= preEndTimeValue) {
					HawkLog.errPrintln("star_wars_time check valid failed, signStartTime < preEndTimeValue, termId: {}", termId);
					checkResult = false;
				}
