...
public class BuildAreaCfg extends HawkConfigBase {
	...
	// 战斗NPC
	protected final String battleNpc;
	private List<Integer> battleNpcList;
	...
	public List<Integer> getBattleNpcList() {
		if (battleNpcList != null) {
			return Collections.unmodifiableList(battleNpcList);
		}
	...
	protected boolean assemble() {
		...
		if (!HawkOSOperator.isEmptyString(battleNpc)) {
			battleNpcList = new ArrayList<>();
			for (String npc : battleNpc.split(",")) {
				battleNpcList.add(Integer.parseInt(npc));
			}
		}
	...
	protected boolean checkValid() {
		if (battleNpcList.isEmpty() || HawkOSOperator.isEmptyString(battleNpc)) {
			throw new RuntimeException("BuildAreaCfg npc is null" + id);
		}
		for (Integer id : battleNpcList) {
			Battle2025StageCfg cfg = HawkConfigManager.getInstance().getConfigByKey(Battle2025StageCfg.class, id);
			if (cfg == null) {
				throw new RuntimeException("Battle2025StageCfg type not exist" + id);
			}
		}
		return true;
	}
