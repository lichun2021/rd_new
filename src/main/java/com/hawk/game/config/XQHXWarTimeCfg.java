...
public class XQHXWarTimeCfg extends HawkConfigBase {
    ...
    protected boolean checkValid() {
        if (termId < 0 || signupTimeValue <= 0 || matchWaitTimeValue <= 0 || battleTimeValue <= 0 || settleTimeValue <= 0 || endTimeValue <= 0){
            HawkLog.errPrintln("XQHXWarTimeCfg checkValid time error termId:{} signupTime:{} matchTime:{} battleTime:{} settleTime:{} endTime:{}", termId, signupTime, matchTime, battleTime, settleTime, endTime);
            return false;
        }
        // 报名时间戳 < 匹配等待时间戳 < 匹配时间戳 < 匹配结束时间戳 < 战斗时间戳 < 结算时间戳 < 结束时间戳
        if (signupTimeValue > matchWaitTimeValue || matchWaitTimeValue > matchTimeValue || matchTimeValue > matchEndTimeValue || matchEndTimeValue > battleTimeValue || battleTimeValue > settleTimeValue || settleTimeValue > endTimeValue){
            HawkLog.errPrintln("XQHXWarTimeCfg checkValid time error termId:{} signupTime:{} matchTime:{} battleTime:{} settleTime:{} endTime:{}", termId, signupTime, matchTime, battleTime, settleTime, endTime);
            return false;
        }
        return true;
