...
public class DressCfg extends HawkConfigBase {
	 ...
	 * 天奢id（天奢装扮专属字段）
	 */
	protected final int highendDressId;
	/**
	 * 拥有装扮触发作用号列表
	 */
	...
	protected boolean checkValid() {
		if (highendDressId > 0) {
			DressWorldHighendCfg cfg = HawkConfigManager.getInstance().getConfigByKey(DressWorldHighendCfg.class, highendDressId);
			if (cfg == null) {
				return false;
			}
		}
	...
	public int getHighendDressId() {
		return highendDressId;
	}
