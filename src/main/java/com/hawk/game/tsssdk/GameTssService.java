...
public class GameTssService {
	...
	public String wordUicChatFilter(Player player, String content, int reportCategory, int innerCategory,
		...
		try {
			...
			return wordUicChatFilter(player, content, reportCategory, innerCategory, callback, protocol, gameData, player.getName());
		} catch (Exception e) {
			HawkException.catchException(e);
		}
	...
	public String wordUicChatFilter(Player player, String content, int reportCategory, int innerCategory,
		...
		try {
			...
			TssAccountType accountType = TssAccountType.TSSACCOUNT_TYPE_QQ;
			int channelId = 2;
			if ("wx".equalsIgnoreCase(player.getChannel())) {
				channelId = 1;
				accountType = TssAccountType.TSSACCOUNT_TYPE_WECHAT;
			}
			
			TssAccountPlatId platId = TssAccountPlatId.TSSPLAT_ID_IOS; //TSSPLAT_ID_HARMONY_MOBILE
			if (player.isPlatHarmony()) {
				platId = TssAccountPlatId.TSSPLAT_ID_HARMONY_MOBILE;
			} else if (GameUtil.isAndroidAccount(player)) {
				platId = TssAccountPlatId.TSSPLAT_ID_ANDROID;
			}
			...
			UicChatDataInfo dataInfo = UicChatDataInfo.newBuilder()
			        ...
			        .setChannelId(channelId)        //1微信 2 QQ
