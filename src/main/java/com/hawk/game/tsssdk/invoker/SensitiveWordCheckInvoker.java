...
public class SensitiveWordCheckInvoker implements TsssdkInvoker {
	...
	public int invoke(Player player, int result, String word, int protocol, String callback) {
		if (protocol == HP.code2.GUILD_BATTLE_TEAM_MANAGER_REQ_VALUE) {
			PlayerGuildTeamModule module = player.getModule(ModuleType.GUILD_TEAM);
			module.checkInputNameCallback(result);
			return 0;
		}
