...
public class WorldPlayerService extends HawkAppObj {
	...
	public void updateWorldPointProtected(String playerId, long protectedEndTime) {
		...
		Player player = GlobalData.getInstance().makesurePlayer(playerId);
		if (player != null) {
			CustomDataEntity protectShowData = player.getData().getCustomDataEntity(LuxuryDressService.LUXURY_PROTECT_SHOW);
			if (protectShowData != null) {
				worldPoint.setProtectedShowTimeLong(protectShowData.getValue() * 1000L);
				worldPoint.setProtectedShowPlayerType(protectShowData.getArg());
			}
		}
	...
	public WorldPoint mantualSettleCity(Player player, int x, int y, long protectedEndTime) {
		...
		try {
			...
			worldModule.moveCityNotify(x, y);
			// 创建玩家使用的世界点信息
