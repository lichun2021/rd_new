...
public class WorldPointService extends HawkAppObj {
	 ...
	 * 不朽共鸣装备外显(装备星级外显)
	 */
	public Map<String, Integer> showEquipImmortEcho = new ConcurrentHashMap<>();
...
//		}
	 ...
	 * @param player
	 */
	public void updateEquipImmortShow(Player player, boolean change) {
		int[] equipImmortShowInfo = getEquipImmortShowInfo(player.getId());
		equipImmortShowInfo[0] = GameUtil.checkEquipImmortShow(player);
		equipImmortShowInfo[1] = (equipImmortShowInfo[1] == 0 ? 1 : 0);
				
		int showInfoInt = GameUtil.combineXAndY(equipImmortShowInfo[0], equipImmortShowInfo[1]);
		showEquipImmortEcho.put(player.getId(), showInfoInt);
		RedisProxy.getInstance().updateEquipEchoShow(player.getId(), showInfoInt);
	 ...
	 * @param player
	 */
	public void updateEquipImmortShow(Player player) {
		int[] equipStarShowInfo = getEquipImmortShowInfo(player.getId());
		equipStarShowInfo[0] = GameUtil.checkEquipImmortShow(player);
	...
	/**
	 * 获取共鸣装备外显信息 (y << 16) | x   y:开关  x:套装
	 * @param playerId
	 * @return
	 */
	public int[] getEquipImmortShowInfo(String playerId) {
		if (showEquipImmortEcho.containsKey(playerId)) {
			return GameUtil.splitXAndY(showEquipImmortEcho.get(playerId));
		}
		int equipImmortShow = RedisProxy.getInstance().getEquipImmortShow(playerId);
		showEquipImmortEcho.put(playerId, equipImmortShow);
		return GameUtil.splitXAndY(equipImmortShow);
	}
	/**
	 * 获取不朽共鸣装备外显(推给客户端的)
	 * @param playerId
	...
	public int getEquipImmortShow(String playerId) {
		int[] equipStarShowInfo = getEquipImmortShowInfo(playerId);
		// 关闭外显,就推0
