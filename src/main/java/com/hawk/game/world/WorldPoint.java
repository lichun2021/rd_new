...
public class WorldPoint extends HawkDBEntity {
	...
	@Column(name = "protectShowTimeLong")
	@IndexProp(id = 41)
	private int protectShowTimeLong = 0;
	
	@Column(name = "protectShowPlayerType")
	@IndexProp(id = 42)
	private String protectShowPlayerType = "";
	
	
	@Transient
	Set<Integer> protectShowTypeSet = new ConcurrentHashSet<>();
	...
	public boolean initPlayerInfo(PlayerData playerData) {
		...
		try {
			...
			CustomDataEntity protectShowData = playerData.getCustomDataEntity(LuxuryDressService.LUXURY_PROTECT_SHOW);
			if (protectShowData != null) {
				setProtectedShowTimeLong(protectShowData.getValue() * 1000L);
				setProtectedShowPlayerType(protectShowData.getArg());
			}
		} catch (Exception e) {
	...
	public WorldPointPB.Builder toBuilder(WorldPointPB.Builder builder, String viewerId) {
		...
		if (pointType == WorldPointType.GUILD_TERRITORY_VALUE && GuildService.getInstance().isGuildExist(guildId)) {
			...
			builder.setEquipImmortEchoShow(WorldPointService.getInstance().getEquipImmortShow(playerId));
		...
		if (WorldUtil.isPlayerPoint(this) && showProtectedEndTime - now > GsConst.MINUTE_MILLI_SECONDS * pointProtectNotify) {
			builder.setProtectedEndTime(Long.MAX_VALUE);
		} else {
			builder.setProtectedEndTime(showProtectedEndTime);
		}
		
		if (protectShowTimeLong > 0 && showProtectedEndTime > now) {
			builder.setProtectedShowTime(builder.getProtectedEndTime() - protectShowTimeLong);
			builder.addAllProtectedShowType(protectShowTypeSet);
		}
	...
	public PointData.Builder buildPointData() {
		...
		builder.setProtectedShowTimeLong(this.getProtectedShowTimeLong());
		builder.setProtectShowType(this.getProtectedShowPlayerType());
	...
	public void mergeFromPointData(PointData.Builder builder) {
		...
		this.setProtectedShowTimeLong(builder.getProtectedShowTimeLong());
		...
		if (!HawkOSOperator.isEmptyString(builder.getProtectShowType())) {
			this.setProtectedShowPlayerType(builder.getProtectShowType());
		}
	...
	public long getProtectedShowTimeLong() {
		return protectShowTimeLong;
	}

	public void setProtectedShowTimeLong(long protectedShowTimeLong) {
		this.protectShowTimeLong = (int)protectedShowTimeLong;
	}
	
	public Set<Integer> getProtectShowTypeSet() {
		return protectShowTypeSet;
	}
	
	public String getProtectedShowPlayerType() {
		if (protectShowPlayerType == null) {
			return "";
		}
		return protectShowPlayerType;
	}

	public void setProtectedShowPlayerType(String protectedShowPlayerType) {
		this.protectShowPlayerType = protectedShowPlayerType;
		this.protectShowTypeSet.clear();
		if (!HawkOSOperator.isEmptyString(protectedShowPlayerType)) {
			this.protectShowTypeSet.addAll(SerializeHelper.stringToList(Integer.class, protectedShowPlayerType));
		} else {
			this.protectShowPlayerType = "";
		}
	}
