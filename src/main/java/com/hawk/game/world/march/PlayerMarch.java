...
public abstract class PlayerMarch implements IWorldMarch {
	...
	public double getMarchBaseSpeed() {
		...
		return WorldUtil.minSpeedInArmy(this.getPlayer(), this, armyList);
	}
	...
	private double effectToMarchTime(double time, boolean slowDown, double slowDownTime) {
		...
		if (this.isMassJoinMarch()) {
			...
			// 作用号10092： 自身加入集结行军 时间减少
			speedUpEffectVal += player.getEffect().getEffVal(Const.EffType.EFF_SPEED_JOIN_MASS_10092, getMarchEntity().getEffectParams());
		}
