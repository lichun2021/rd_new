...
public class NotifyMarchUpdateTask extends HawkTask{
					...
					if (player != null) {
						...
						if (!player.isCsPlayer()) {
							...
							if (!table.contains(player.getMainServerId(), relation)) {
								table.put(player.getMainServerId(), relation, new HashSet<>());
							}
	...
	private MarchEventSync.Builder createSyncPB(int eventType, WorldMarchRelation relation) {
		...
		if (eventType != MarchEvent.MARCH_DELETE_VALUE) {
			dataBuilder.setMarchPB(GameUtil.simplifiedMarchSyncPB(march.toBuilder(WorldMarchPB.newBuilder(), relation)));
		}
