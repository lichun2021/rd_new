...
public class CrossPlayerProtocolHandler {
	...
	private boolean checkCrossDataExist(ProxyHeader proxyHeader) {
		...
		try {
			...
			boolean exist = CSRedisProxyPool.getInstance().getRedisSession(playerId).hExists(redisKey, PlayerDataKey.PlayerEntity.name(), 0);
			if (!exist) {
			...
			exist = CSRedisProxyPool.getInstance().getRedisSession(playerId).hExists(redisKey, PlayerDataKey.PlayerBaseEntity.name(), 0);
			if (!exist) {
	...
	public boolean initPlayer(CsSession csSession, HPLogin loginCmd) {
		...
		String puid = GameUtil.getPuidByPlatform(loginCmd.getPuid(), platform);
