...
public class CsPlayerData extends PlayerData {
	...
	public void loadAll(boolean isNewly) {
		...
		if (costtime >= 50) {
			HawkLog.logPrintln("csplayer login load data success, playerId: {}, costtime: {}", playerId, costtime);
			HawkProfilerAnalyzer.getInstance().addMsgHandleInfo("csPlayerData-loadAll", costtime);
		}
	}
}
