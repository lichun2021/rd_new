...
public class MonthCardPriceCutLogic implements ItemAddLogic {
	...
	private void setMonthCardPriceCutNotifyTime(Player player) {
		CustomDataEntity customData = player.getData().getCustomDataEntity(CustomKeyConst.GOLD_PRIVILEGE_NOTITY_VALIDTIME);
		if (customData == null) {
			int endTime = HawkTime.getSeconds() + ConstProperty.getInstance().getGoldPrivilegeButtonTime();
			player.getData().createCustomDataEntity(CustomKeyConst.GOLD_PRIVILEGE_NOTITY_VALIDTIME, endTime, "");
		}
	}
}
