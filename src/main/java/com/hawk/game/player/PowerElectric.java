...
public class PowerElectric {
	...
	public void refreshPowerElectric(Player player, boolean isSoliderCure, boolean needSync, PowerChangeReason reason) {
		...
		if ((changeData.getBuildBattleChange() > 100000 && reason != PowerChangeReason.BUILD_LVUP) 
				|| (changeData.getTechBattleChange() > 100000 && reason != PowerChangeReason.TECH_LVUP)) {
			logger.error("PowerChangeData to large playerId: {} , build: {}, tech: {}, reason: {}",player.getId(), changeData.getBuildBattleChange(), changeData.getTechBattleChange(), reason);
			return;
		}
