...
public class PlayerEffect {
	...
	public int getEffVal(EffType effType, String targetId, EffectParams effParams) {
		...
		effDress += LuxuryDressService.getInstance().getEffect(playerId, effId);
...
//		int xqhxEff = getXqhxBuff(effType);
	...
	protected int getHerosOfficeEffVal(EffType effType, EffectParams effParams) {
		...
		try {
			int val = 0;
			PBHeroRiseShow risePage = effParams.getRisePage();
			for (HeroEntity hero : playerData.getHeroEntityList()) {
				PlayerHero heroObj = hero.getHeroObj();
				if (Objects.isNull(heroObj)) {
					continue;
				}
				val = val + heroObj.getOfficeEffVal(effType, effParams);
				HeroRise rise = heroObj.getRise();
				if (risePage == null || risePage.getType() == 0 || rise == null ) {
					continue;
				}
				RisePageType pageType = RisePageType.valueOf(risePage.getType());
				if (rise.getType().contains(pageType) && rise.isEffected(risePage.getType())) {
					val = val + rise.getNodeEffval(effType);
				}
				if (risePage.getLeftHero() == hero.getHeroId() || risePage.getRightHero() == hero.getHeroId()) {
					val = val + rise.getSkillEffval(effType, risePage.getLmLevel());
				}
			}
			return val;
		} catch (Exception e) {
