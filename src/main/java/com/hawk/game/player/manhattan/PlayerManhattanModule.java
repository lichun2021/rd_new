...
public class PlayerManhattanModule extends PlayerModule {
	...
	private void onManhattanBaseStageUp(HawkProtocol protocol) {
		...
		List<ItemInfo> consume = ItemInfo.valueListOf(config.getConsumption());
		GameUtil.reduceByEffect(consume, 9990041, player.getEffect().getEffValArr(Const.EffType.BLACK_TECH_367826));
		GameUtil.reduceByEffect(consume, 9990042, player.getEffect().getEffValArr(Const.EffType.BLACK_TECH_367826));
		ConsumeItems consumeItems = ConsumeItems.valueOf();
		consumeItems.addConsumeInfo(consume);
