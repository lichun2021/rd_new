...
public class MidasService {
	...
	public int checkBalance(Player player, JSONObject pfTokenJson) {
		...
		String platform = player.getMidasPlatform();
		CheckBalanceResult result = SDKManager.getInstance().checkBalance(platform, player.getChannel(), player.getPfTokenJson(), player.getServerId());
		...
		if (result != null && result.getRet() == SDKConst.ResultCode.SUCCESS) {
			...
			// 日志记录
			HawkLog.logPrintln("MSDK check balance success, playerId: {}, platform: {}, channel: {}, diamonds: {}, saveAmt: {}, player chargeAmt: {}",
					player.getId(), platform, player.getChannel(), result.getBalance(), result.getSave_amt(), player.getPlayerBaseEntity()._getChargeAmt());

			return result.getSave_amt();
		}

		HawkLog.errPrintln("MSDK check balance failed, playerId: {}, openId: {}, retCode: {}, platform: {}, channel: {}, serverId: {}, pfToken: {}",
				player.getId(), player.getOpenId(), result != null ? result.getRet() : result, platform,
	...
	public String pay(Player player, int diamond, String actionName, List<PayItemInfo> payItems) {
		...
		do {
			String platform = player.getMidasPlatform();
			// 调用支付
			PayResult result = SDKManager.getInstance().pay(platform, player.getChannel(), player.getPfTokenJson(), player.getServerId(), diamond, billno, array);
			reqCount--;
			...
			if (resultCode == SDKConst.ResultCode.PAY_RESULT_EXP_CODE1 && code == SDKConst.ResultCode.PAY_RESULT_EXP_CODE2) {
				...
				HawkLog.logPrintln("MSDK Pay sucess, playerId: {}, platform: {}, channel: {}, diamonds: {}, saveAmt: {}, cost: {}, billno: {}, action: {}",
						player.getId(), platform, player.getChannel(), result.getBalance(), result.getSave_amt(), diamond, billno, actionName);
				return billno;
			}
			...
			HawkLog.errPrintln("MSDK Pay Failure, playerId: {}, retCode: {}, platform: {}, channel: {}, "
					+ "serverId: {}, diamond: {}, billino: {}, pfToken: {}, action: {}", player.getId(), resultCode, platform,
	...
	public boolean cancelPay(Player player, int diamond, String billno) {
		...
		do {
			String platform = player.getMidasPlatform();
			CancelPayResult result = SDKManager.getInstance().cancelPay(platform, player.getChannel(), player.getPfTokenJson(), player.getServerId(), diamond, billno);
			reqCount--;
			int code = result != null ? result.getRet() : SDKConst.ResultCode.FAILED;
			if (code == SDKConst.ResultCode.SUCCESS ||
				...
				HawkLog.logPrintln("MSDK pay cancel sucess, playerId: {}, platform: {}, channel: {}, diamonds: {}, billno: {}", player.getId(), platform, player.getChannel(), diamond, billno);
				return true;
			}

			resultCode = code;
			HawkLog.errPrintln("MSDK pay cancel failed, playerId: {}, retCode: {}, platform: {}, channel: {}, pftoken: {}, serverId: {}, diamonds: {}, billno: {}",
					player.getId(), resultCode, platform, player.getChannel(), player.getPfTokenJson(), player.getServerId(), diamond, billno);
	...
	public int present(Player player, int diamond, String extendParam, String actionName, String presentReason) {
		...
		try {
			...
			do {
				String platform = player.getMidasPlatform();
				PresentResult presentResult = SDKManager.getInstance().present(platform, player.getChannel(), player.getPfTokenJson(), player.getServerId(), diamond, billno, extendParam, presentReason);
				reqCount--;
				...
				if (code == SDKConst.ResultCode.SUCCESS ||
					...
					HawkLog.logPrintln("MSDK present sucess, playerId: {}, platform: {}, channel: {}, diamonds: {}, add: {}, billno: {}, extendParam: {}, action: {}",
							player.getId(), platform, player.getChannel(), presentResult.getBalance(), diamond, presentResult.getBillno(), extendParam, actionName);
				...
				HawkLog.errPrintln("MSDK present help recharge failed, playerId: {}, retCode: {}, platform: {}, channel: {}, "
						+ "serverId: {}, diamond: {}, pfToken: {}, billno: {}, extendParam: {}, action: {}", player.getId(), resultCode, platform,
	...
	public String payBuyItems(Player player, PayGiftCfg giftCfg) {
		...
		if (result != null && result.getRet() == SDKConst.ResultCode.SUCCESS) {
			...
			HawkLog.logPrintln("MSDK buy item success, playerId: {}, platform: {}, channel: {}, token: {}, urlParam: {}, goods: {}, pfToken: {}", player.getId(), platform, player.getChannel(), result.getToken(), result.getUrl_params(), shoppingGoods, player.getPfTokenJson());
		...
		HawkLog.errPrintln("MSDK buy item failed, playerId: {}, platform: {}, channel: {}, serverId: {}, goods: {}, appMeta: {}, pfToken: {}",
				player.getId(), platform, player.getChannel(), player.getServerId(), shoppingGoods, buff.toString(), player.getPfTokenJson());
		return null;
	}
