...
public class ChatImageItemUseEffect extends AbstractItemUseEffect {
	...
	public boolean useEffect(Player player, ItemCfg itemCfg, int itemCount, String targetId) {
		...
		CustomDataEntity customData = player.getData().getCustomDataEntity(CustomKeyConst.CHAT_IMAGE_UNLOCKED);
		if (customData == null) {
			customData = player.getData().createCustomDataEntity(CustomKeyConst.CHAT_IMAGE_UNLOCKED, 0, String.valueOf(chatImgId));
		} else {
