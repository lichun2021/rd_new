...
public class PlayerDataSerializer {
	...
	public static boolean flushToRedis(PlayerDataCache dataCache, PlayerDataKey dataKey, boolean resetState) {
		...
		int writeLength = 0;
		int pipelineNum = 0;
		String redisKey = RedisKey.PLAYER_DATA_KEY + dataCache.getPlayerId();
		String statusKey = RedisKey.ENTITY_STATUS_KEY + dataCache.getPlayerId();
		HawkRedisSession redisSession = CSRedisProxyPool.getInstance().getRedisSession(dataCache.getPlayerId());
		try (Jedis jedis = redisSession.getJedis(); Pipeline pipeline = jedis.pipelined();){
			for (PlayerDataKey key : EnumSet.allOf(PlayerDataKey.class)) {
				...
				if (bytes != null) {
					if(key.isZlibDeflate()){
						bytes = HawkZlib.zlibDeflate(bytes);
					}
			...
			if (dataKey == null) {
				...
				pipeline.expire(redisKey, crossPlayerCacheExpireTime);
				pipeline.expire(statusKey, crossPlayerCacheExpireTime);
			}
			...
			if (costTime > 50) {
				HawkLog.warnPrintln("dataCacheFlushToRedis costTime:{}, playerId:{}, writeLength:{}, pipelineNum:{}", costTime, dataCache.getPlayerId(), writeLength,pipelineNum);
				HawkProfilerAnalyzer.getInstance().addMsgHandleInfo("playerData-writeRedis", costTime);
			}			
		}		
	...
	public static boolean buildFromRedis(PlayerDataCache dataCache, boolean persistable) {
		try {
			String redisKey = RedisKey.PLAYER_DATA_KEY + dataCache.getPlayerId();
			HawkRedisSession redisSession = CSRedisProxyPool.getInstance().getRedisSession(dataCache.getPlayerId());
			Map<byte[], byte[]> dataMap = redisSession.hGetAllBytes(redisKey.getBytes());
			for (PlayerDataKey key : EnumSet.allOf(PlayerDataKey.class)) {
				String fieldKey = key.name();
				byte[] bytes = dataMap.get(fieldKey.getBytes());
				if (bytes != null) {
					if(key.isZlibDeflate()){
						byte[] bytes2 = HawkZlib.zlibInflate(bytes);
						if(bytes2!=null){
							bytes = bytes2;
						}
					}
	...
	public static  <T> T unserializeData(String playerId, PlayerDataKey key, boolean persistable) throws Exception {
		...
		HawkRedisSession redisSession = CSRedisProxyPool.getInstance().getRedisSession(playerId);
		byte[] bytes = redisSession.hGetBytes(redisKey, key.name());	
		if(key.isZlibDeflate()){
			byte[] bytes2 = HawkZlib.zlibInflate(bytes);
			if(bytes2!=null){
				bytes = bytes2;
	...
	public static void csEntityChanged(HawkDBEntity entity, int opType) {
		...
		if (!player.getData().getDataCache().containsEntityFlag(dataKey)) {
			return;
		}
		// 更新到redis中
		flushToRedis(player.getData().getDataCache(), null, true);
	...
	public static void csSyncPlayerData(String playerId, boolean needRecord) {
		...
		synchronized (player.getSyncObj()) {
			String redisKey = RedisKey.ENTITY_STATUS_KEY + player.getId();
			
			HawkRedisSession redisSession = CSRedisProxyPool.getInstance().getRedisSession(playerId);
			try {
				statusMap = redisSession.hGetAll(redisKey); 
			} catch (Exception e) {
				HawkException.catchException(e);
				return;
			}
			...
			for (PlayerDataKey dataKey : EnumSet.allOf(PlayerDataKey.class)) {
				try {
					...
					// 从redis读取数据
					byte[] bytes = redisSession.hGetBytes(RedisKey.PLAYER_DATA_KEY + playerId, dataKey.name());
					if(dataKey.isZlibDeflate()){
						byte[] bytes2 = HawkZlib.zlibInflate(bytes);
						if(bytes2!=null){
							bytes = bytes2;
						}
