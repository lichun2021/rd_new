...
public class InheritNewService extends HawkAppObj {
	...
	public void roleRechargeAmountSum(Player player) {
		try {
			...
			CustomDataEntity customData = player.getData().getCustomDataEntity(CustomKeyConst.RECHARGE_DATA_REF_KEY);
			if (customData == null) {
				customData = player.getData().createCustomDataEntity(CustomKeyConst.RECHARGE_DATA_REF_KEY, 0, "");
			} else {
