...
public class AbstractSuperWeapon implements IWeapon {
	...
	public void doControlToPace() {
		...
		HawkLog.logPrintln("super weapon doControlToPace ....1");
		// 添加自动报名
		if (commander != null && !HawkOSOperator.isEmptyString(commander.getPlayerGuildId())) {
			HawkLog.logPrintln("super weapon doControlToPace ....2");
			...
			if (seasonTurn % turnMax != 0) {
				HawkLog.logPrintln("super weapon doControlToPace ....3");
	...
	private void addSchedule() {
		try {
			int pos[] = GameUtil.splitXAndY(pointId);
			SuperWeaponConstCfg constCfg = SuperWeaponConstCfg.getInstance();
			long time = HawkTime.getNextTimeDayOfWeek(HawkTime.getMillisecond(), constCfg.getInitday(), constCfg.getInitTime(), constCfg.getInitMinute());
			ScheduleInfo schedule = ScheduleInfo.createNewSchedule(ScheduleType.SCHEDULE_TYPE_7_VALUE, commander.getPlayerGuildId(), time, pos[0], pos[1], String.valueOf(pointId));
	...
	public void logPushEvent() {
		Set<String> guildSet = new HashSet<>();
		guildSet.addAll(signUpSet);
		guildSet.addAll(autoSignUpSet);
		HawkTaskManager.getInstance().getThreadPool("task").addTask(new HawkTask() {
			@Override
			public Object run() {
				int count = 0;
				for(String guildId : guildSet){
					for (String playerId : GuildService.getInstance().getGuildMembers(guildId)) {
						try {
							...
							LogUtil.logPushEvent(player, PushEventType.SUPER_WEAPON, 0);
							count++;
						} catch (Exception e) {
							HawkException.catchException(e);
						}
						if (count == 50) {
							count = 0;
							HawkOSOperator.sleep(); //防止上报太过密集了有数据丢失的风险
						}
					}
				}
				return null;
