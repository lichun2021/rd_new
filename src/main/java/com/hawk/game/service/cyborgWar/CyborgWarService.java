...
public class CyborgWarService extends HawkAppObj {
	...
	protected void fightTick() {
		...
		if (needUpdate) {
			...
			HawkLog.logPrintln("CyborgWarService fightTick update sync, costtime: {}", HawkTime.getMillisecond() - startTime);
		}
	}
	...
	public void syncOnlinePlayers(){
		HawkTask syncTask = new HawkTask("CB-ONLINE-SYNC") {
			@Override
			public Object run() {
				int count = 0;
				long startTime = HawkTime.getMillisecond();
				Map<String,CWStateInfo.Builder> guildStateMap = new HashMap<>();
				Table<String, String, List<CWTeamInfo>> guildTeamTable = HashBasedTable.create();
				Map<String,Map<String, String> > guildMemberMap = new HashMap<>();
				...
				for (Player player : GlobalData.getInstance().getOnlinePlayers()) {
					...
					//活动状态
					CWStateInfo.Builder stateInfo = guildStateMap.get(guildId);
					if(Objects.isNull(stateInfo)){
						stateInfo = genStateInfo(guildId);
						guildStateMap.put(guildId, stateInfo);
					}
					...
					List<CWTeamInfo> guildTeamList = guildTeamTable.get(guildId, playerServer);
					if(Objects.isNull(guildTeamList)){
						guildTeamList = genGuildTeamList(guildId, playerServer);
						guildTeamTable.put(guildId, playerServer, guildTeamList);
					}
					listBuilder.addAllTeamList(guildTeamList);
					
					//自己的队伍ID
					Map<String, String> ptMap = guildMemberMap.get(guildId);
					if(Objects.isNull(ptMap)){
						ptMap = getMemberTeamMap(guildId);
						guildMemberMap.put(guildId, ptMap);
					}
					String selfTeamId = ptMap.getOrDefault(player.getId(), "");
					listBuilder.setSelfTeam(selfTeamId);
					...
					player.sendProtocol(HawkProtocol.valueOf(HP.code.CYBORG_WAR_INFO_SYNC, builder));
				}
				HawkLog.logPrintln("CyborgWarService-syncOnlinePlayers update sync, costtime: {}, playerCount: {}", HawkTime.getMillisecond() - startTime, count);
				return null;
			}
		};
		HawkThreadPool taskPool = HawkTaskManager.getInstance().getThreadPool("task");
		if (Objects.nonNull(taskPool)) {
			taskPool.addTask(syncTask, -1, true);
		}
	}
	...
	private void createBattle(int timeIndex) {
		...
		//推送TLOG
		CyborgWarOpenPushTask.addPushEvnet(termId, timeIndex, dataList);
	}
	...
	private void sendAward(int timeIndex) {
		...
		for (String teamId : teamIds) {
			...
			for (String playerId : idList) {
				...
				PlayerActiveStatService.getInstance().updateActiveStatInfo(playerId, ActiveStatType.TYPE_CYBOR, termId, isComplete ? selfScore : 3000);
