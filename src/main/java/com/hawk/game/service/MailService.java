...
//		}
	...
	public List<MailLiteInfo.Builder> listMailEntity(List<String> mailIds) {
		mailIds = new ArrayList<>(new HashSet<>(mailIds));
		if (mailIds.isEmpty()) {
	...
	public HPCheckMailRes.Builder createHPCheckMailResBuilder(String playerId, MailLiteInfo.Builder mail) {
		...
		try {
			...
			case FSH_ATK_WIN:
			case FSH_DEFENSE_LOSS:
			case FSH_ATK_LOSS:
			case FSH_DEFENSE_WIN:
	...
	public boolean getMailReward(Player player, MailLiteInfo.Builder entity, AwardItems award) {
		...
		if (entity.getMailId() == MailId.DEAD_SOLDIER_RESOURCE_BUCHANG_VALUE
				|| entity.getMailId() == MailId.ALLIANCE_SHARE_GLORY_RETURN_REWARDA_VALUE
				|| entity.getMailId() == MailId.ALLIANCE_SHARE_GLORY_RETURN_REWARDB_VALUE) {
			// 战损补给不限制数量.
		...
		if (diamonds > 0) {
			if (StringUtils.isNotEmpty(entity.getAdditionalParam())) {
				...
				player.increaseDiamond(diamonds, Action.XINYUE_HELP_RECHARGE, entity.getAdditionalParam(), DiamondPresentReason.RECHATGE, entity.getMailId(), entity.getId());
			} else if (GameUtil.isRewardMail(entity.getMailId())) {
				// 赠送原因  customer_experience
				player.increaseDiamond(diamonds, Action.GET_MAIL_REWARD, entity.getAdditionalParam(), DiamondPresentReason.EXPERIENCE, entity.getMailId(), entity.getId());
