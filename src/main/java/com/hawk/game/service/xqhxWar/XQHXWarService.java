...
import com.hawk.game.config.XQHXSeasonConst;
...
public class XQHXWarService extends HawkAppObj {
     ...
     * value:房间数据
     */
    private Map<String, XQHXWarRoomData> roomDataMap = new ConcurrentHashMap<>();
    /**
     * 通过时间段索引房间id
     * key:时间索引
     * value:房间id集合
            ...
            for (int i = 1; i <= constCfg.getWarCount(); i++){
                ...
                while (teamList.size() > 1){
                    ...
                    if(teamData1 != null && teamData2 != null){
                        ...
                        //设置战场服
                        String roomServerId = this.selectRoomServer(roomServerMap, teamData1, teamData2);
                        //创建房间
     ...
     * @return
     */
	public String selectRoomServer(Map<String, Integer> roomServerMap, GuildTeamData teamData1, GuildTeamData teamData2) {
		String rlt = teamData1.serverId;
		try {
			//往压力小的分配
			int cnt1 = roomServerMap.getOrDefault(teamData1.serverId, 0);
			int cnt2 = roomServerMap.getOrDefault(teamData2.serverId, 0);
			if(cnt1 > cnt2){
				rlt = teamData2.serverId;
			}else if(cnt2 > cnt1){
				rlt = teamData1.serverId;
			}else{
				rlt = (Integer.parseInt(teamData1.serverId)%10000) >= (Integer.parseInt(teamData2.serverId)%10000) ? teamData1.serverId : teamData2.serverId;
			}
			//开放数量累计
			int openCnt = roomServerMap.getOrDefault(rlt, 0) + 1;
			roomServerMap.put(rlt, openCnt);
		} catch (Exception e) {
			rlt = teamData1.serverId.compareTo(teamData2.serverId) >= 0 ? teamData1.serverId : teamData2.serverId;
		}
		return rlt;
	}
    ...
    public void sendMatchFailResult() {
        // 轮空机制修改：轮空胜场+1，积分不变 不区分 排位和普通
//        if (XQHXSeasonManager.getInstance().getState() != XQHXSeasonStateEnum.QUALIFIER) {
//            return;
//        }
        for (String teamId : signUpMap.keySet()) {
            ...
            HawkLog.logPrintln("XQHXWarService sendMatchFailResult state{} termId{} teamId{}", XQHXSeasonManager.getInstance().getState(), getTermId(), teamId);
            XQHXSeasonManager.getInstance().addScore(teamId, 0, true);
        }
    }
    ...
    public void onBattleOpen(int timeIndex){
        ...
        for(String roomId : roomIds){
            //房间数据
            XQHXWarRoomData roomData = roomDataMap.get(roomId);
        ...
        //推送
        XQHXWarOpenPushTask.addPushEvnet(timeIndex, roomIds, roomDataMap);
    }
    ...
    private void onBattleFinish(XQHXBilingInformationMsg msg){
        ...
        try {
            ...
            XQHXSeasonManager.getInstance().updateBattleInfo(teamDataA, teamDataB, roomData);
		} catch (Exception e) {
			HawkException.catchException(e);
		}
    }
     ...
     * @param index 索引
     */
    public void signUp(String teamId, int index) {
        //更新缓存
            ...
            for(String teamId : signUpMap.keySet()) {
                try{
                    ...
                    if(teamData.timeIndex != signUpMap.get(teamId)){
                        HawkLog.logPrintln("XQHXWarService updateSignUpInfo teamId:{} timeIndex:{} != signUpMap.get(teamId):{}", teamId, teamData.timeIndex, signUpMap.get(teamId));
                        continue;
                    }
                    Set<String> playerIds = XQHXGuildTeamManager.getInstance().getTeamPlayerIds(teamId);
                    if (XQHXSeasonManager.getInstance().getState() != XQHXSeasonStateEnum.RANKING) {
                        //不是排位赛 玩家数量不达标跳过
                        if (playerIds == null || playerIds.size() < constCfg.getMemberSignupLimit()) {
                            HawkLog.logPrintln("XQHXWarService updateSignUpInfo Insufficient teamId:{} playerIds size:{}", teamId, playerIds == null ? 0 : playerIds.size());
                            continue;
                        }
                    }
                    //记录联盟信息
                    ...
                    if (XQHXSeasonManager.getInstance().getState() != XQHXSeasonStateEnum.RANKING) {
                        //不是排位赛 玩家数量不达标跳过
                        if (teamData.battlePoint <= 0) {
                            HawkLog.logPrintln("XQHXWarService updateSignUpInfo battlePoint 0 teamId:{} ", teamId);
                            continue;
                        }
                    }
                    //更新报名信息
                    ...
                    // 改为 添加赛季队伍信息
                    XQHXSeasonData seasonData = XQHXSeasonManager.getInstance().loadData(teamId);
                    if (seasonData == null) {
                        XQHXSeasonConst seasonConst = HawkConfigManager.getInstance().getKVInstance(XQHXSeasonConst.class);
                        seasonData = new XQHXSeasonData();
                        seasonData.score = seasonConst.getInitPoint();
                        seasonData.teamId = teamId;
                        seasonData.isNew = (XQHXSeasonManager.getInstance().getServerType() == PBCommonMatch.PBCMWServerType.NEW_SERVER);
                        XQHXSeasonManager.getInstance().update(seasonData);
                    }
        ...
        try {
            ...
            for (String roomStr : roomList) {
                try {
                    ...
                    //解析房间数据
                    XQHXWarRoomData roomData = XQHXWarRoomData.unSerialize(roomStr);
                    //数据为空的话跳过
                    if (roomData == null) {
     ...
     * @return 战场数据
     */
    public XQHXWarRoomData getRoomData(Player player) {
    ...
    public void sendMatchResult(){
        try {
            for (XQHXWarRoomData roomData : roomDataMap.values()) {
                try {
                    if(roomData == null){
                        continue;
                    }
     ...
     * @param roomData 战场数据
     */
    public void updateRoomScore(XQHXWarRoomData roomData) {
        //如果战场数据为空直接返回
        if(roomData == null){
            return;
        }
    ...
    public void sendAward(int timeIndex){
        int termId = getTermId();
        XQHXSeasonStateEnum state = XQHXSeasonManager.getInstance().getState();
        //遍历报名数据
        for(String teamId : signUpMap.keySet()) {
            //报名时间段
            ...
            HawkLog.logPrintln("XQHXWarService sendAward termId{} teamId {} roomId {}", termId, teamId, roomId);
            //获得战场数据
            XQHXWarRoomData roomData = roomDataMap.get(roomId);
            //战场数据为空跳过
            if(roomData == null){
            ...
            long selfScore = teamId.equals(roomData.campA) ? roomData.scoreA : roomData.scoreB;
            try {
                XQHXSeasonManager.getInstance().addScore(teamId, selfScore, teamId.equals(roomData.winnerId));
            } catch (Exception e) {
                HawkException.catchException(e);
            }
            //记录历史记录
                ...
                try {
                    ...
                    if (!HawkOSOperator.isEmptyString(scoreStr)) {
                        try {
                            ...
                            ActivityManager.getInstance().postEvent(new XQScoreEvent(playerId, playerSelfScore, state != XQHXSeasonStateEnum.CLOSE));
                            PlayerActiveStatService.getInstance().updateActiveStatInfo(playerId, ActiveStatType.TYPE_XQHX, termId , playerSelfScore);
                        } catch (Exception e) {
                            HawkException.catchException(e);
                        }
                    }
            ...
            // try {
            //     ActivityService.getInstance().dealMsg(GameConst.MsgId.ON_GUILD_BACK_DROP, new GuildBackDropInvoker(new ArrayList<>(XQHXGuildTeamManager.getInstance().getTeamPlayerIds(teamId))));
            // } catch (Exception e) {
            //     HawkException.catchException(e);
            // }
        }
    ...
    public boolean checkEnter(Player player) {
        try {
            ...
            XQHXWarRoomData roomData = roomDataMap.get(teamIdToRoomId.get(playerData.teamId));
            if(roomData == null){
                player.sendError(HP.code2.XQHX_WAR_ENTER_INSTANCE_REQ_VALUE, Status.XQHXError.XQHX_NOT_IN_THIS_WAR_VALUE,0);
                return false;
            }
    ...
    public void updateTeamEnterNum(String teamId, String playerId, boolean join){
        try {
            if(HawkOSOperator.isEmptyString(teamId)){
                return;
            }
            ...
            if(join){
            	String val = String.valueOf(HawkTime.getMillisecond());
            	RedisProxy.getInstance().getRedisSession().hSet(teamEnterKey, playerId, val, TiberiumConst.TLW_EXPIRE_SECONDS);
            }else{
            	RedisProxy.getInstance().getRedisSession().hDel(teamEnterKey, playerId);
            }
        }catch (Exception e){
            HawkException.catchException(e);
        }
    ...
    public int getTeamEnterNum(String teamId){
        try {
            String teamEnterKey = String.format(XQHXWarResidKey.XQHX_WAR_TEAM_ENTER, getTermId(), teamId);
            long value = RedisProxy.getInstance().getRedisSession().hLen(teamEnterKey, false);
            return (int) value;
        } catch (Exception e) {
            HawkException.catchException(e);
            return 100;
    ...
    public boolean joinRoom(Player player) {
        ...
        updateTeamEnterNum(playerIdToTeamId.get(player.getId()), player.getId(), true);
	...
	public void onXQHXDataUpdate(HawkProtocol hawkProtocol) {
        ...
        XQHXWarRoomData redisData = XQHXWarRoomData.unSerialize(roomStr);
        if(roomData.roomState != 2){
        	return;
        }
    ...
    public void seasonSignup(String teamId, int timeIndex){
        ...
        if(teamData != null){
            ...
            XQHXGuildTeamManager.getInstance().updateTeam(teamData);
            signUp(teamData.id, teamData.timeIndex);
            HawkLog.logPrintln("XQHX seasonSignup teamId{} timeIndex{}", teamId, timeIndex);
        }
    }

    public boolean isLocalTeam(String teamId){
        GuildTeamData teamData = XQHXGuildTeamManager.getInstance().getTeamData(teamId);
        return teamData != null && teamData.serverId.equals(GsConfig.getInstance().getServerId());
    }

    /*******************************************************************************************************************
     * OB相关
     ******************************************************************************************************************/
    public PBXQHXOBRoomInfoResp.Builder genPBXQHXOBRoomInfoRespBuilder(){
    	PBXQHXOBRoomInfoResp.Builder builder = PBXQHXOBRoomInfoResp.newBuilder();
    	XQHXWarStateEnum stateEnum = this.getState();
    	if(stateEnum != XQHXWarStateEnum.BATTLE){
    		return builder;
    	}
    	//当前正在战斗的时间段
    	int termId = 0;
    	int timeIndex = 0;
    	for(XQHXWarBattleStateData battleStateData : battleStateDataMap.values()){
            if(battleStateData.getState() == XQHXWarStateEnum.BATTLE_OPEN){
            	termId = battleStateData.getTermId();
            	timeIndex = battleStateData.getTimeIndex();
            }
        }
    	if(termId <= 0 || timeIndex <= 0){
    		return builder;
    	}
    	//常量配置
        XQHXConstCfg constCfg = HawkConfigManager.getInstance().getKVInstance(XQHXConstCfg.class);
        int obFlag = constCfg.getObFlag();
    	//战斗开始时间
        long battleStartTime = this.getBattleStartTime(termId, timeIndex);
        //战斗结束时间
        long battelEndTime = battleStartTime + constCfg.getBattleTime();
        //全部房间
        Map<String,String> rooms = RedisProxy.getInstance().getRedisSession().hGetAll(String.format(XQHXWarResidKey.XQHX_WAR_ROOM, termId));
        
        if(Objects.isNull(rooms)){
        	return builder;
        }
        
        Map<String,XQHXWarRoomData> mormalRooms = new HashMap<>();
        Map<String,XQHXWarRoomData> seasonRooms = new HashMap<>();
        for (String roomStr : rooms.values()) {
        	XQHXWarRoomData roomData = XQHXWarRoomData.unSerialize(roomStr);
        	if(roomData.timeIndex != timeIndex || roomData.termId != termId){
        		continue;
        	}
        	if(roomData.group > 0){
        		seasonRooms.put(roomData.id, roomData);
        	}else{
        		mormalRooms.put(roomData.id, roomData);
        	}
        	
        }
    	//联赛
    	if(obFlag == 0 || obFlag == 1){
    		int season = XQHXSeasonManager.getInstance().getSeason();
    		PBXQHXOBRoomData.Builder seasonData = this.genPBXQHXOBRoomData(season, termId, battleStartTime, battelEndTime, seasonRooms);
            builder.setSeasonRooms(seasonData);
    	}
    	//普通赛
    	if(obFlag == 1){
    		PBXQHXOBRoomData.Builder normalData = this.genPBXQHXOBRoomData(0, termId, battleStartTime, battelEndTime, mormalRooms);
            builder.setNormalRooms(normalData);
    	}
    	return builder;
    }
    
 
    public PBXQHXOBRoomData.Builder genPBXQHXOBRoomData(int season,int termId,long battleStartTime,long battelEndTime,Map<String,XQHXWarRoomData> rooms){
    	PBXQHXOBRoomData.Builder builder = PBXQHXOBRoomData.newBuilder();
    	builder.setSeason(season);
    	builder.setTermId(termId);
    	builder.setWarStartTime(battleStartTime);
    	builder.setWarFinishTime(battelEndTime);
		//队伍
		Set<String> teams = new HashSet<>();
		for(XQHXWarRoomData data : rooms.values()){
			teams.add(data.campA);
			teams.add(data.campB);
		}
		//加载报名数据
        Map<String,GuildTeamData> teamMap = XQHXGuildTeamManager.getInstance().loadTeamMap(teams);
        //组织数据
        for(XQHXWarRoomData roomData : rooms.values()){
        	XQHXOBRoomInfo.Builder rbuilder = XQHXOBRoomInfo.newBuilder();
        	rbuilder.setRoomId(roomData.id);
        	rbuilder.setRoomServer(roomData.roomServerId);
        	rbuilder.setTermId(roomData.termId);
        	rbuilder.setWarStartTime(battleStartTime);
        	rbuilder.setWarFinishTime(battelEndTime);
        	
        	XQHXMatchInfo.Builder mbuilder = XQHXMatchInfo.newBuilder();
        	mbuilder.setRoomId(roomData.id);
        	mbuilder.setWinnerId(roomData.winnerId);
        	mbuilder.setGroup(roomData.group);
        	mbuilder.setNewServer(roomData.isNew);
        	//A阵营
        	GuildTeamData campA = teamMap.get(roomData.campA);
        	if(Objects.nonNull(campA)){
        		GuildTeamInfo.Builder gbuilder = campA.toPB();
        		mbuilder.setGuildTeamA(gbuilder);
        	}
        	//B阵营
        	GuildTeamData campB = teamMap.get(roomData.campB);
        	if(Objects.nonNull(campB)){
        		GuildTeamInfo.Builder gbuilder = campB.toPB();
        		mbuilder.setGuildTeamB(gbuilder);
        	}
        	rbuilder.setMatchInfo(mbuilder);
        	builder.addRoomInfo(rbuilder);
        }
    ...
    public String gm(Map<String, String> map){
        ...
        HawkLog.logPrintln("XQHXWarService gm: {} stateData {}", cmd + stateData.toString() + battleStateDataMap.toString());
        switch (cmd) {
            case "info":{
                return printInfo();
            }
            case "next":{
                if(stateData.getState() == XQHXWarStateEnum.BATTLE){
                    ...
                    if (XQHXSeasonManager.getInstance().getState() != XQHXSeasonStateEnum.CLOSE && getState() == XQHXWarStateEnum.PEACE) {
            ...
            case "allSignFirst": {
                for (GuildTeamData teamData : XQHXGuildTeamManager.getInstance().teamDataMap.values()) {
                    if (teamData != null) {
            ...
            case "allOneSignFirst": {
                ...
                guildIds.forEach(id -> {
                    try {
                        ...
                        if (guildInfoObject != null) {
                            GuildTeamData teamData = XQHXGuildTeamManager.getInstance().teamDataMap.get(XQHXGuildTeamManager.getInstance().getTeamId(id, 1));
                            if (teamData != null) {
                                teamData.timeIndex = 1;
                                signUp(teamData.id, teamData.timeIndex);
                                XQHXGuildTeamManager.getInstance().updateTeam(teamData);
    ...
    public String printMatchInfo(int termId, int timeIndex) {
        ...
        try {
            ...
            //组装页面信息
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=info\">主页</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchInfo\">全部</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchInfo&timeIndex=1\">时间段1</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchInfo&timeIndex=2\">时间段2</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchInfo&timeIndex=3\">时间段3</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchInfo&timeIndex=4\">时间段4</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=signupInfo\" target=\"_blank\">报名信息</a><br>";
            //获得期数
            termId = termId == 0 ? getTermId() : termId;
            Map<String, String> roomMap = RedisProxy.getInstance().getRedisSession().hGetAll(String.format(XQHXWarResidKey.XQHX_WAR_ROOM, termId));
    ...
    public String printSignupInfo(int termId, int timeIndex) {
        ...
        try {
            ...
            //组装页面信息
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=info\">主页</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=signupInfo\">全部</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=signupInfo&timeIndex=1\">时间段1</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=signupInfo&timeIndex=2\">时间段2</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=signupInfo&timeIndex=3\">时间段3</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=signupInfo&timeIndex=4\">时间段4</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchInfo\" target=\"_blank\">战场信息</a><br>";
            //获得期数
            termId = termId == 0 ? getTermId() : termId;
            if (timeIndex == 0) {
                XQHXConstCfg constCfg = HawkConfigManager.getInstance().getKVInstance(XQHXConstCfg.class);
                for (int i = 1; i <= constCfg.getWarCount(); i++) {
                    ...
                    if (teamIds != null && !teamIds.isEmpty()) {
                        for (String teamId : teamIds) {
                            info += "------------------------------------------------------------------------------------------";
                            info += gmFillTeamData(XQHXGuildTeamManager.getInstance().getTeamData(teamId));
                        }
                    }
                    info += "==========================================================================================<br>";
                }
                ...
                String signUpTimeKey = String.format(XQHXWarResidKey.XQHX_WAR_SIGNUP_TIME, termId, timeIndex);
                Set<String> teamIds = RedisProxy.getInstance().getRedisSession().sMembers(signUpTimeKey);
                if (teamIds != null && !teamIds.isEmpty()) {
                    for (String teamId : teamIds) {
                        info += "------------------------------------------------------------------------------------------";
    ...
    public String printMatchTestInfo(Map<String, String> map) {
        ...
        try {
            ...
            //组装页面信息
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=info\">主页</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestInfo\">刷新</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestAddGuild\">加100联盟</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestAddGuild&count=10\">加10联盟</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestAddGuild&count=1\">加1联盟</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestAddTeam\">加满战队</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddPower\">战力随机</a>        ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamCleanPower\">战力清空</a>        ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestClean\">清空数据</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamRankInfo\" target=\"_blank\">战队排行</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestResultInfo\" target=\"_blank\">匹配结果</a><br><br>";

            Map<String, String> guildMap = RedisProxy.getInstance().getRedisSession().hGetAll(XQHXWarResidKey.XQHX_WAR_TEST_GUILD);
            Map<String, String> teamMap = RedisProxy.getInstance().getRedisSession().hGetAll(XQHXWarResidKey.XQHX_WAR_TEST_TEAM);
    ...
    public String printMatchTestTeamRankInfo(Map<String, String> map) {
        ...
        try {
            ...
            //组装页面信息
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=info\">主页</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamRankInfo\">刷新</a><br><br>";
            Map<String, String> teamMap = RedisProxy.getInstance().getRedisSession().hGetAll(XQHXWarResidKey.XQHX_WAR_TEST_TEAM);
    ...
    public String printMatchTestResultInfo(Map<String, String> map) {
        ...
        try {
            ...
            //组装页面信息
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=info\">主页</a>           ";
            info += "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestResultInfo\">刷新</a><br><br>";
            Map<String, String> teamMap = RedisProxy.getInstance().getRedisSession().hGetAll(XQHXWarResidKey.XQHX_WAR_TEST_TEAM);
    ...
    public String gmFillMatchTestGuildData(String guildId, Map<String, String> guildMap, Map<String, String> teamMap) {
        ...
        try {
            ...
            info = "\t<tr>\n" +
                    ...
                    "\t\t<td colspan=\"8\">" +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestAddTeam&guildId=" + guildId + "\">加满战队</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestClean&guildId=" + guildId + "\">清空战队</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestAddTeam&guildId=" + guildId + "&count=1\">战队加1</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddPower&guildId=" + guildId + "\">战力随机</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamCleanPower&guildId=" + guildId + "\">战力清空</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddDay&guildId=" + guildId + "&add=1\">天数加1</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamCleanDay&guildId=" + guildId + "\">天数清空</a>        " +
                    "</td>\n" +
                    "\t</tr>\n";
            for (int i = 1; i <= constCfg.getTeamNumLimit(); i++) {
                info += gmFillMatchTestTeamData(i, XQHXGuildTeamManager.getInstance().getTeamId(guildId, i), teamMap);
            }
        } catch (Exception e) {
            HawkException.catchException(e);
        }
    ...
    public String gmFillMatchTestTeamData(int index, String teamId, Map<String, String> teamMap) {
        ...
        try {
            ...
            info = "\t<tr>\n" +
                    ...
                    "\t\t<td colspan=\"2\">" +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddPower&teamId=" + teamId + "&add=1000\">加1000战力</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddPower&teamId=" + teamId + "&add=-1000\">减1000战力</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddPower&teamId=" + teamId + "&add=100\">加100战力</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddPower&teamId=" + teamId + "&add=-100\">减100战力</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddPower&teamId=" + teamId + "&add=10\">加10战力</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddPower&teamId=" + teamId + "&add=-10\">减10战力</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddPower&teamId=" + teamId + "&add=1\">加1战力</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddPower&teamId=" + teamId + "&add=-1\">减1战力</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamCleanPower&teamId=" + teamId + "\">战力清空</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamAddDay&teamId=" + teamId + "&add=1\">天数加1</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestTeamCleanDay&teamId=" + teamId + "\">天数清空</a>        " +
                    "<a href=\"http://" + ipAddress + ":" + port + "/script/whcgm?opt=XQHXGM&cmd=matchTestClean&teamId=" + teamId + "\">删除</a>        " +
                    "</td>\n" +
                    "\t</tr>\n";
        } catch (Exception e) {
            HawkException.catchException(e);
    ...
    public void matchTestTeamAddPower(Map<String, String> map) {
        ...
        if (map.containsKey("guildId")) {
            ...
            for (int i = 1; i <= constCfg.getTeamNumLimit(); i++) {
                String teamId = XQHXGuildTeamManager.getInstance().getTeamId(guildId, i);
                String teamStr = RedisProxy.getInstance().getRedisSession().hGet(XQHXWarResidKey.XQHX_WAR_TEST_TEAM, teamId);
                GuildTeamData teamData = GuildTeamData.unSerialize(teamStr);
                if (teamData == null) {
                    continue;
                }
            ...
            RedisProxy.getInstance().getRedisSession().hSet(XQHXWarResidKey.XQHX_WAR_TEST_TEAM, teamId, teamData.serialize());
        } else {
            Map<String, String> teamMap = RedisProxy.getInstance().getRedisSession().hGetAll(XQHXWarResidKey.XQHX_WAR_TEST_TEAM);
            for (String teamId : teamMap.keySet()) {
                String teamStr = teamMap.get(teamId);
        ...
        if (!teamTmpMap.isEmpty()) {
            RedisProxy.getInstance().getRedisSession().hmSet(XQHXWarResidKey.XQHX_WAR_TEST_TEAM, teamTmpMap, 0);
        }
    }
    ...
    public void matchTestClean(Map<String, String> map) {
        ...
        if (map.containsKey("guildId")) {
            ...
            for (int i = 1; i <= constCfg.getTeamNumLimit(); i++) {
                String teamId = XQHXGuildTeamManager.getInstance().getTeamId(guildId, i);
                RedisProxy.getInstance().getRedisSession().hDel(XQHXWarResidKey.XQHX_WAR_TEST_TEAM, teamId);
            }
        } else if (map.containsKey("teamId")) {
            String teamId = map.get("teamId");
            RedisProxy.getInstance().getRedisSession().hDel(XQHXWarResidKey.XQHX_WAR_TEST_TEAM, teamId);
        } else {
            RedisProxy.getInstance().getRedisSession().del(XQHXWarResidKey.XQHX_WAR_TEST_GUILD);
            RedisProxy.getInstance().getRedisSession().del(XQHXWarResidKey.XQHX_WAR_TEST_TEAM);
        }
    ...
    public void matchTestAddGuild(Map<String, String> map) {
        ...
        RedisProxy.getInstance().getRedisSession().hmSet(XQHXWarResidKey.XQHX_WAR_TEST_GUILD, guildTmpMap, 0);
    }
        ...
        if (map.containsKey("guildId")) {
            ...
            if (map.containsKey("count")) {
                for (int i = 1; i <= constCfg.getTeamNumLimit(); i++) {
                    String teamId = XQHXGuildTeamManager.getInstance().getTeamId(guildId, i);
                    if (!teamMap.containsKey(teamId)) {
                        GuildTeamData teamData = new GuildTeamData();
                        teamData.id = teamId;
    ...
    public String gmFillRoomData(XQHXWarRoomData roomData) {
        String info = "==========================================================================================<br>";
        ...
        info += "------------------------------------------------------------------------------------------";
        info += "<br>小队1:" + gmFillTeamData(XQHXGuildTeamManager.getInstance().getTeamData(roomData.campA));
        info += "------------------------------------------------------------------------------------------";
        info += "<br>小队2:" + gmFillTeamData(XQHXGuildTeamManager.getInstance().getTeamData(roomData.campB));
        info += "==========================================================================================<br>";
        return info;
    }

    public String gmFillTeamData(GuildTeamData teamData) {
        String info = "<br>";
        if (teamData == null) {
            return info;
        }
        Set<String> playerIds = teamIdToBattlePlayerIds.get(teamData.id);
        long matchPower = 0L;
        //如果为空直接返回
        if (playerIds != null && !playerIds.isEmpty()) {
            for (String playerId : playerIds) {
        ...
        float param = 1f;
        XQHXConstCfg constCfg = HawkConfigManager.getInstance().getKVInstance(XQHXConstCfg.class);
        List<XQHXWarHistoryData> historyDataList = XQHXWarHistoryData.load(teamData.id, constCfg.getRecordPointCount());
