...
public class FightMailService extends MailService {
	...
	public void sendFightMail(int pointType, IBattleIncome battleIncome, BattleOutcome battleOutcome, Map<String, long[]> resources) {
		...
		if (isAtksWin) {
			...
			}else if(pointType == WorldPointType.TREASURE_386_VALUE){
				attackMailId = MailId.FSH_ATK_WIN;
				defenseMailId = MailId.FSH_DEFENSE_LOSS;
				atktips = new Object[] {  defPlayer.getName() };
				defTips = new Object[] {  atkPlayer.getName() };
			...
			if (pointType == WorldPointType.PLAYER_VALUE) {
				...
				attackMailId = MailId.FSH_ATK_LOSS;
				defenseMailId = MailId.FSH_DEFENSE_WIN;
			} else if(pointType == WorldPointType.WAR_FLAG_POINT_VALUE){
