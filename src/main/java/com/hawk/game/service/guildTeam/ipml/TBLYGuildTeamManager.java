    ...
    public boolean checkMemberManager(Player player, GuildBattleMemberManagerReq req) {
        ...
        switch (req.getAuth()){
            case GT_STARTER:{
                ...
                boolean limit = TBLYSeasonService.getInstance().teamManageLastTeamLimit(req.getTeamId(),req.getPlayerId());
                if(limit){
                	player.sendError(HP.code.TIBERIUM_WAR_TEAM_MEMBER_MANAGE_C_VALUE, Status.Error.TIBERIUM_LEAGUA_JOIN_WIN_VALUE, 0);
                	return false;
                }
                limit = TBLYSeasonService.getInstance().teamManageTLWServerLimit(req.getTeamId(),req.getPlayerId());
                if(limit){
                	player.sendError(HP.code.TIBERIUM_WAR_TEAM_MEMBER_MANAGE_C_VALUE, Status.Error.TIBERIUM_LEAGUA_JOIN_TEAM_ZONE_ERR_VALUE, 0);
                	return false;
                }
