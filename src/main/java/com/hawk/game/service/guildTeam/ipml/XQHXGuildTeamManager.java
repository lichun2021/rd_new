...
public class XQHXGuildTeamManager extends GuildTeamManagerBase {
    ...
    public void signUp(Player player, String teamId, int index) {
        XQHXWarService.getInstance().signUp(teamId, index);
        //日志
    ...
    public boolean isInSeason(String teamId) {
        return XQHXSeasonManager.getInstance().isInSeason(teamId);
    }
    ...
    public void dissmissFromSeason(String teamId) {
        XQHXSeasonManager.getInstance().onTeamDismiss(teamId);
    }
    ...
    public boolean checkTeamManager(Player player, GuildBattleTeamManagerReq req) {
        ...
        // 小队解散赛季限制
        if (req.getOpt() == GuildTeamOpt.GT_TEAM_DISMISS) {
            if (XQHXSeasonManager.getInstance().getState() != XQHXSeasonStateEnum.CLOSE) {
                player.sendError(HP.code2.GUILD_BATTLE_TEAM_MANAGER_REQ_VALUE, Status.XQHXError.XQHX_IN_SEASON, 0);
                return false;
            }
        }
        return true;
