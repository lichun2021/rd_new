...
public class XHJZSeasonManager extends CMWManagerBase {
    ...
    public void onQualifierMatch(){
        ...
        for(int i = 1; i <= constCfg.getWarCount(); i++){
            ...
            createRooms(termId, i, newTeamList, roomStrMap, serverIdToRoomIdMap,roomServerMap, true, PBCommonMatch.PBCMWGroupType.QUALIFIER_GROUP_VALUE, 0);
            createRooms(termId, i, oldTeamList, roomStrMap, serverIdToRoomIdMap,roomServerMap, false, PBCommonMatch.PBCMWGroupType.QUALIFIER_GROUP_VALUE, 0);
    ...
    public void onRankingMatch(){
        ...
        for(int i = 1; i <= constCfg.getWarCount(); i++){
            ...
            for(int count : oldSTeamListMap.keySet()){
                ...
                createRooms(termId, i, teamList, roomStrMap, serverIdToRoomIdMap, roomServerMap,false, PBCommonMatch.PBCMWGroupType.S_GROUP_VALUE, count);
                noMatchList.addAll(teamList);
            }
            for(int count : newSTeamListMap.keySet()){
                List<XHJZWarTeamData> teamList = newSTeamListMap.get(count);
                createRooms(termId, i, teamList, roomStrMap, serverIdToRoomIdMap, roomServerMap,true, PBCommonMatch.PBCMWGroupType.S_GROUP_VALUE, count);
                noMatchList.addAll(teamList);
            }
            for(int count : oldATeamListMap.keySet()){
                List<XHJZWarTeamData> teamList = oldATeamListMap.get(count);
                createRooms(termId, i, teamList, roomStrMap, serverIdToRoomIdMap, roomServerMap,false, PBCommonMatch.PBCMWGroupType.A_GROUP_VALUE, count);
                noMatchList.addAll(teamList);
            }
            for(int count : newATeamListMap.keySet()){
                List<XHJZWarTeamData> teamList = newATeamListMap.get(count);
                createRooms(termId, i, teamList, roomStrMap, serverIdToRoomIdMap, roomServerMap,true, PBCommonMatch.PBCMWGroupType.A_GROUP_VALUE, count);
                noMatchList.addAll(teamList);
            }
            for(int count : oldBTeamListMap.keySet()){
                List<XHJZWarTeamData> teamList = oldBTeamListMap.get(count);
                createRooms(termId, i, teamList, roomStrMap, serverIdToRoomIdMap, roomServerMap,false, PBCommonMatch.PBCMWGroupType.B_GROUP_VALUE, count);
                noMatchList.addAll(teamList);
            }
            for(int count : newBTeamListMap.keySet()){
                List<XHJZWarTeamData> teamList = newBTeamListMap.get(count);
                createRooms(termId, i, teamList, roomStrMap, serverIdToRoomIdMap, roomServerMap,true, PBCommonMatch.PBCMWGroupType.B_GROUP_VALUE, count);
                noMatchList.addAll(teamList);
            }
            createRooms(termId, i, normalTeamList, roomStrMap, serverIdToRoomIdMap,roomServerMap,false, 0, 0);
        }
        if(!roomStrMap.isEmpty()){
            RedisProxy.getInstance().getRedisSession().hmSet(String.format(XHJZRedisKey.XHJZ_WAR_ROOM, termId), roomStrMap, 0);
        }
...
//        }
    ...
    public void createRooms(int termId, int i, List<XHJZWarTeamData> teamList, Map<String, String> roomStrMap, Map<String, Set<String>> serverIdToRoomIdMap, Map<String, Integer> roomServerMap, boolean isNew, int group, int count){
        teamList.sort((o1, o2) -> {
     ...
     * @return
     */
	public String selectRoomServer(Map<String, Integer> roomServerMap, XHJZWarTeamData teamData1, XHJZWarTeamData teamData2) {
		...
		try {
			...
			HawkLog.logPrintln("XHJZSeasonManager-selectRoomServer-ONE-{},{},{}",teamData1.id,teamData2.id,rlt);
		} catch (Exception e) {
			rlt = teamData1.serverId.compareTo(teamData2.serverId) >= 0 ? teamData1.serverId : teamData2.serverId;
			HawkLog.logPrintln("XHJZSeasonManager-selectRoomServer-TWO-{},{},{}",teamData1.id,teamData2.id,rlt);
		}
		return rlt;
	}
