...
import com.hawk.game.config.ItemChestFixCfg;
...
public class AwardService {
	...
	public AwardItems takeAward(Player player, int awardId, int itemCount, Action action, boolean popup, int... orginArgs){
		...
		if (orginArgs.length > 1 && ItemChestFixCfg.isConfigItem(orginArgs[1])) {
			dailyGiftSpecialBox(player, awardItemArray, ItemChestFixCfg.getConfig(orginArgs[1]));
		}
	 ...
	 * @param times
	 */
	private void dailyGiftSpecialBox(Player player, AwardItems[] itemsArray, ItemChestFixCfg config) {
		String customKey = CustomKeyConst.DAILY_GIFT_BOX;
		//840700是老数据，这里要做兼容
		if (config.getItem() != 840700) {
			customKey = CustomKeyConst.DAILY_GIFT_BOX + "_" + config.getItem();
		}
		CustomDataEntity entity = player.getData().getCustomDataEntity(customKey);
		if (entity == null) {
			entity = player.getData().createCustomDataEntity(customKey, 0, "");
		}
		
		int times = config.getFailedNum();
		int awardId = config.getFixAward();
		for (int i = 0; i < itemsArray.length; i++) {
			...
			for (int itemId : config.getObtainListenItemIds()) {
