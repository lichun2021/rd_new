...
public class TBLYWarOpenPushTask extends HawkTask{

	private boolean season;
	private List<GuildTeamRoomData> roomList;

	
	public TBLYWarOpenPushTask(int timeIndex,Set<String> roomIds,Map<String,GuildTeamRoomData> rooms) {
		this.season = false;
		this.roomList = new ArrayList<>();
		for(String roomId : roomIds){
            GuildTeamRoomData roomData = rooms.get(roomId);
	...
	public TBLYWarOpenPushTask(Map<String,GuildTeamRoomData> rooms) {
		this.season = true;
		if(Objects.nonNull(rooms)){
			this.roomList = new ArrayList<>();
			this.roomList.addAll(rooms.values());
		}
	}
	...
	public void warPush(){
		...
		for(GuildTeamRoomData roomData : this.roomList){
			tempList.clear();
		...
		for(String teamId : pushTeamIds){
			...
			for(String memberId : pids){
				...
				LogUtil.logPushEvent(player, PushEventType.DUNGEON_TIBERIUM, 0);
			}
		}
	}
	...
	public static void  addNormalPushEvnet(int timeIndex,Set<String> roomIds,Map<String,GuildTeamRoomData> rooms){
		try {
			...
			TBLYWarOpenPushTask task = new TBLYWarOpenPushTask(timeIndex, roomIds, rooms);
			HawkThreadPool taskPool = HawkTaskManager.getInstance().getThreadPool("task");
			if (Objects.nonNull(taskPool)) {
				task.setTypeName("TB_OPEN_PUSH");
	...
	public static void  addSeasonPushEvnet(Map<String,GuildTeamRoomData> rooms){
		try {
			if(Objects.isNull(rooms)){
				return;
			}
			...
			TBLYWarOpenPushTask task = new TBLYWarOpenPushTask(rooms);
			HawkThreadPool taskPool = HawkTaskManager.getInstance().getThreadPool("task");
			if (Objects.nonNull(taskPool)) {
				task.setTypeName("TB_SEASON_OPEN_PUSH");
