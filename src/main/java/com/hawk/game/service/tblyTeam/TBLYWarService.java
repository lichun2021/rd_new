...
public class TBLYWarService extends HawkAppObj {
	...
	public String selectRoomServer(Map<String, Integer> roomServerMap, GuildTeamData teamData1, GuildTeamData teamData2) {
		...
		try {
			int seasonId = TBLYSeasonService.getInstance().getSeason();
			int seasonTerm = TBLYSeasonService.getInstance().getTermId();
			if(seasonId == 11 && seasonTerm < TiberiumConstCfg.getInstance().getEliminationStartTermId()){
				//第11赛季 前7场是小组赛，这个时候先往后面的新服分
				...
				HawkLog.logPrintln("TBLYWarService-selectRoomServer-one-{},{},{}",teamData1.id,teamData2.id,rlt);
				return rlt;
			}
			//往压力小的分配
			...
			HawkLog.logPrintln("TBLYWarService-selectRoomServer-TWO-{},{},{}",teamData1.id,teamData2.id,rlt);
		} catch (Exception e) {
			rlt = teamData1.serverId.compareTo(teamData2.serverId) >= 0 ? teamData1.serverId : teamData2.serverId;
			HawkLog.logPrintln("TBLYWarService-selectRoomServer-THR-{},{},{}",teamData1.id,teamData2.id,rlt);
		}
		return rlt;
	}
    ...
    public void onBattleOpen(int timeIndex){
        ...
        //推送
        TBLYWarOpenPushTask.addNormalPushEvnet(timeIndex, roomIds, roomDataMap);
    }
                ...
                try {
                    ...
                    if(!HawkOSOperator.isEmptyString(scoreStr)){
                        try {
                            ...
                            PlayerActiveStatService.getInstance().updateActiveStatInfo(playerId, ActiveStatType.TYPE_TBLY, termId , playerSelfScore);
                        } catch (Exception e) {
                            HawkException.catchException(e);
                        }
                    }
    ...
    public TLWGetOBRoomInfo.Builder getTWRoomInfo() {
        ...
        for(String roomStr : roomMap.values()){
            ...
            guildBuilderB.setName(guildB.guildName);
