...
public class BuildingUpGradeToMax extends HawkScript {
	...
	public static String doAction(Map<String, String> params) {
		...
		try {
            ...
            try {
				// 解锁全部里程碑
				HawkScript script = HawkScriptManager.getInstance().getScript("milestoneMissionFinish");
				Map<String, String> paramsMap = new HashMap<>();
				paramsMap.put("playerId", player.getId());
				paramsMap.put("chapter", "14");
				String result = script.action(paramsMap, null);
				HawkLog.logPrintln("milestoneMissionFinish levelMax result: {}", result);
            } catch (Exception e) {
			...
			// 解锁机甲
			try {
				HawkScript script = HawkScriptManager.getInstance().getScript("unlockFirstSuperSoldier");
				Map<String, String> paramsMap = new HashMap<>();
				paramsMap.put("playerId", player.getId());
				paramsMap.put("playerName", player.getName());
				String result = script.action(paramsMap, null);
				HawkLog.logPrintln("unlockFirstSuperSoldier levelMax result: {}", result);
			} catch (Exception e) {
				HawkException.catchException(e);
			}
	...
	private static void unlockShareBlockBuild(Player player, int buildType, AtomicInteger indexObj) {
		...
		int breakIndex = 0;
		while (count < 5 && breakIndex < 20) {
			BuildingCfg buildingCfg = HawkConfigManager.getInstance().getConfigByKey(BuildingCfg.class, (buildType * 100) + 1);
			String buildIndex = indexObj.get() + "";
