...
public class YQZZMatchTestHandler extends HawkScript {
	...
	private void getMatchInfo(StringBuilder builder){
		...
		YQZZJoinServer serverData = dataManager.getServerData();
		...
		builder.append("本服信息写入(1 写入  0未写入):"+(serverData.getSaveServerInfo() >0?1:0)).append("\r\n</br>");
		builder.append("本服匹配信息加载(1 已经加载  0未加载):"+(dataManager.getMatchRoomDatas()==null?0:1)).append("\r\n</br>");
		Map<String,YQZZMatchRoomData> roomDatas = dataManager.getMatchRoomDatas();
		if(roomDatas != null){
			for(YQZZMatchRoomData roomData : roomDatas.values()){
				builder.append("战场房间信息:"+roomData.getRoomId()+">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>").append("\r\n</br>");
				builder.append("战场服ID:"+roomData.getRoomServerId()).append("\r\n</br>");
				builder.append("***********************************************").append("\r\n</br>");
				builder.append("参与服务器:"+roomData.getGuilds().toString()).append("\r\n</br>");
				Map<String, YQZZJoinGuild> guilds = dataManager.getBattleGuilds();
				for(String joinGuild : roomData.getGuilds()){
					YQZZJoinGuild guild = guilds.get(joinGuild);
					builder.append("参与服务器信息:"+guild.getServerId()+",联盟名字:"+guild.getGuildName()+",匹配战力:"+guild.getPower()).append("\r\n</br>");
				}
				builder.append("***********************************************").append("\r\n</br>");
			}
