...
import com.hawk.game.dress.luxury.LuxuryDressService;
import com.hawk.game.module.PlayerHeroModule;
import com.hawk.game.module.mechacore.cfg.*;
...
import com.hawk.game.player.hero.rise.HeroRiseNode;
...
import com.hawk.game.util.CustomKeyConst;
...
public class TestOperationHandler extends HawkScript {
	...
	public String action(Map<String, String> params, HawkScriptHttpInfo httpInfo) {
		...
		if (opType.equals("activeStatScore")) {
			return addActiveStatScore(params);
		}
	 ...
	 * @return
	 */
	private String addActiveStatScore(Map<String, String> params) {
		String playerId = params.get("playerId");
		String type = params.get("type");
		String termIdParam = params.get("termId");
		String score = params.get("score");
		if (HawkOSOperator.isEmptyString(playerId) || HawkOSOperator.isEmptyString(type) || HawkOSOperator.isEmptyString(score)) {
			return HawkScript.failedResponse(-1, "playerId or type or score param empty error");
		}
		ActiveStatType statType = ActiveStatType.valueOf(Integer.parseInt(type));
		if (statType == null) {
			return HawkScript.failedResponse(-1, "type param error: " + type);
		}
		Player player = GlobalData.getInstance().makesurePlayer(playerId);
		if (player == null) {
			return HawkScript.failedResponse(-1, "player not exist: " + playerId);
		}
		int termId = 0;
		if (statType != ActiveStatType.TYPE_PLAYER_DAILY) {
			if (HawkOSOperator.isEmptyString(termIdParam)) {
				return HawkScript.failedResponse(-1, "termId param empty error");
			}
			termId = Integer.parseInt(termIdParam);
		}
		
		PlayerActiveStatService.getInstance().updateActiveStatInfo(playerId, statType, termId, Integer.parseInt(score));
		return HawkScript.successResponse("activeStatScore");
	}
	...
	private String chargeShowControl(Map<String, String> params) {
		...
		int val = 0;
		String key = CustomKeyConst.CHARGE_SHOW + ":" + player.getId();
		...
		if (player.isActiveOnline()) {
			CustomDataEntity entity = player.getData().getCustomDataEntity(CustomKeyConst.CHARGE_SHOW);
			if (entity == null) {
				entity = player.getData().createCustomDataEntity(CustomKeyConst.CHARGE_SHOW, val, "");
			} else {
				entity.setValue(val);
			}
	...
	private String testOp(Map<String, String> params) {
		...
		/** 添加机甲核心模块  */
		// http://localhost:8080/script/testOp?op=heroRiseNode&playerId=1aat-3syuot-1
		if (opType.equals("heroRiseNode")) {
			Player player = GlobalData.getInstance().scriptMakesurePlayer(params);
			if (player == null) {
				return HawkScript.failedResponse(-1, "对应玩家不存在");
			}
			heroRiseNode(player);
			return HawkScript.successResponse("heroRiseNode succ");
		}
		/** 添加机甲核心模块  */
		...
		/** 机甲核心槽位满级  */
		// http://localhost:8080/script/testOp?op=mechacoreModuleSlot&playerId=1aat-3syuot-1
		if (opType.equals("mechacoreModuleSlot")) {
			Player player = GlobalData.getInstance().scriptMakesurePlayer(params);
			if (player == null) {
				return HawkScript.failedResponse(-1, "对应玩家不存在");
			}
            mechacoreModuleSlot(player);
			return HawkScript.successResponse("mechacoreModuleSlot succ");
		}
        /** 机甲核心生成顶级模块  */
        // http://localhost:8080/script/testOp?op=mechacoreModuleAddtional&playerId=1aat-3syuot-1&addtional=10000101|1_2_3,10000102|1_2_3
        if (opType.equals("mechacoreModuleAddtional")) {
            Player player = GlobalData.getInstance().scriptMakesurePlayer(params);
            if (player == null) {
                return HawkScript.failedResponse(-1, "对应玩家不存在");
            }
			String addtionalVal = "10000106|120_180_750_810_1620_1830," +
					"10000206|30_90_750_810_1590_1830," +
					"10000306|300_360_840_900_1680_1830," +
					"10000406|210_270_840_900_1650_1830," +
					"10000506|480_540_930_990_1740_1830," +
					"10000606|390_450_930_990_1710_1830," +
					"10000706|660_720_1020_1080_1800_1830," +
					"10000806|570_630_1020_1080_1770_1830," +
					"20000906|120_180_750_810_1620_1830," +
					"20000906|30_90_750_810_1590_1830," +
					"20001006|480_540_930_990_1740_1830," +
					"20001006|390_450_930_990_1710_1830," +
					"20001106|300_360_840_900_1680_1830," +
					"20001106|210_270_840_900_1650_1830," +
					"20001206|660_720_1020_1080_1800_1830," +
					"20001206|570_630_1020_1080_1770_1830," +
					"30000106|120_180_750_810_1620_1830," +
					"30000206|30_90_750_810_1590_1830," +
					"30000306|300_360_840_900_1680_1830," +
					"30000406|210_270_840_900_1650_1830," +
					"30000506|480_540_930_990_1740_1830," +
					"30000606|390_450_930_990_1710_1830," +
					"30000706|660_720_1020_1080_1800_1830," +
					"30000806|570_630_1020_1080_1770_1830," +
					"40000906|120_180_750_810_1620_1830," +
					"40000906|30_90_750_810_1590_1830," +
					"40001006|480_540_930_990_1740_1830," +
					"40001006|390_450_930_990_1710_1830," +
					"40001106|300_360_840_900_1680_1830," +
					"40001106|210_270_840_900_1650_1830," +
					"40001206|660_720_1020_1080_1800_1830," +
					"40001206|570_630_1020_1080_1770_1830," +
					"50000106|120_180_750_810_1620_1830," +
					"50000206|30_90_750_810_1590_1830," +
					"50000306|300_360_840_900_1680_1830," +
					"50000406|210_270_840_900_1650_1830," +
					"50000506|480_540_930_990_1740_1830," +
					"50000606|390_450_930_990_1710_1830," +
					"50000706|660_720_1020_1080_1800_1830," +
					"50000806|570_630_1020_1080_1770_1830," +
					"60000906|120_180_750_810_1620_1830," +
					"60000906|30_90_750_810_1590_1830," +
					"60001006|480_540_930_990_1740_1830," +
					"60001006|390_450_930_990_1710_1830," +
					"60001106|300_360_840_900_1680_1830," +
					"60001106|210_270_840_900_1650_1830," +
					"60001206|660_720_1020_1080_1800_1830," +
					"60001206|570_630_1020_1080_1770_1830";
            String addtional = params.get("addtional");
			if (HawkOSOperator.isEmptyString(addtional)) {
				addtional = addtionalVal;
			}
            String[] addtionalParam = addtional.split(",");
            for (String param : addtionalParam) {
				String[] p = param.split("\\|");
				if (p.length <= 0) {
					continue;
				}
				int moduleId = Integer.parseInt(p[0]);
				String addtionalStr = p[1];
				String[] addStr = addtionalStr.split("_");
				if (addStr.length <= 0) {
					continue;
				}
                List<Integer> addtionalList = Arrays.stream(addStr).map((Integer::parseInt)).collect(Collectors.toList());
                mechacoreModuleAddtional(player, moduleId, addtionalList);
            }
            return HawkScript.successResponse("mechacoreModuleAddtional succ");
        }
		...
		//查询本服的天奢技能使用情况
		if (opType.equals("serverLuxurySkill")) {
			return queryServerLuxurySkill(params);
		}
	...
	private String queryServerLuxurySkill(Map<String, String> params) {
		StringBuilder builder = new StringBuilder();
		builder.append(HawkScript.HTTP_NEW_LINE);
		Field field = HawkOSOperator.getClassField(LuxuryDressService.getInstance(), "globalSkillMap");
		try {
			long time = HawkTime.getMillisecond();
			Map<Integer, List<ServerLuxuryInfo>> globalSkillMap = (Map<Integer, List<ServerLuxuryInfo>>) field.get(LuxuryDressService.getInstance());
			for (Entry<Integer, List<ServerLuxuryInfo>> entry : globalSkillMap.entrySet()) {
				int skillId = entry.getKey();
				List<ServerLuxuryInfo> killList = entry.getValue();
				for (ServerLuxuryInfo info : killList) {
					try {
						...
						if (info.getEndTime() < time) {
							builder.append("已过期的技能id: ").append(skillId);
						} else {
							builder.append("未过期的技能id: ").append(skillId);
						}
						
						builder.append(",    所属人openid: ").append(player.getOpenId());
						builder.append(",    所属人名字: ").append(player.getName());
						builder.append(",    开始生效时间: ").append(HawkTime.formatTime(info.getStartTime()));
						builder.append(",    生效结束时间: ").append(HawkTime.formatTime(info.getEndTime()));
						builder.append(",    使用技能的时间: ").append(HawkTime.formatTime(skillInfo.getCastTime()));
						builder.append(",    技能冷却开始时间: ").append(HawkTime.formatTime(skillInfo.getCoolStartTime()));
						builder.append(HawkScript.HTTP_NEW_LINE);
						
						HawkLog.logPrintln("技能id: {}, 所属人名字： {}， 开始生效时间： {}， 生效结束时间：{}，使用技能的时间： {}， 冷却开始时间： {}", 
								skillId, player.getName(), HawkTime.formatTime(info.getStartTime()), HawkTime.formatTime(info.getEndTime()), 
								HawkTime.formatTime(skillInfo.getCastTime()), HawkTime.formatTime(skillInfo.getCoolStartTime()));
					} catch (Exception e) {
						HawkException.catchException(e);
					}
				}
		...
		builder.append(HawkScript.HTTP_NEW_LINE);
		return HawkScript.successResponse(builder.toString());
	}
...
//		}
	 ...
	 * @param player
	 */
    private void mechacoreModuleAddtional(Player player, int moduleId, List<Integer> addtional) {
		int threadNum = HawkTaskManager.getInstance().getThreadNum();
		HawkTaskManager.getInstance().postTask(new HawkTask() {
			@Override
            public Object run() {
                ...
                moduleEntity.setPlayerId(player.getId());
                moduleEntity.setCfgId(moduleCfg.getId());
                moduleEntity.setQuality(moduleCfg.getModuleQuality());
                for (Integer id : addtional) {
                    MechaCoreModuleAddtionalCfg addtionalCfg = HawkConfigManager.getInstance().getConfigByKey(MechaCoreModuleAddtionalCfg.class, id);
                    if (addtionalCfg == null) {
                        continue;
                    }
                    MechaCoreModuleEffObject moduleEffObject = new MechaCoreModuleEffObject(addtionalCfg.getId(), addtionalCfg.getEffect(), HawkRand.randInt(addtionalCfg.getRandMin(), addtionalCfg.getRandMax()), addtionalCfg.getQuality());
                    moduleEntity.addRandomAttrEff(moduleEffObject);
                }
				...
				player.getPlayerMechaCore().syncModuleInfo(moduleList);
                player.getPlayerMechaCore().getEntity().notifyUpdate();
                return null;
            }
	 ...
	 * @param player
	 */
	private void mechacoreModuleSlot(Player player) {
		int threadNum = HawkTaskManager.getInstance().getThreadNum();
		HawkTaskManager.getInstance().postTask(new HawkTask() {
			@Override
			public Object run() {
				for (int slotType : MechaCoreModuleSlotCfg.getSlotTypes()) {
					int maxLevel = MechaCoreModuleSlotCfg.getMaxLevel(slotType);
					MechaCoreModuleSlotCfg cfg = MechaCoreModuleSlotCfg.getConfig(slotType, maxLevel);
					MechaCoreSlotObject slotObject = player.getPlayerMechaCore().getSlotObj(slotType);
					slotObject.setUnlocked(1);
					slotObject.setSlotId(cfg.getId());
					slotObject.setLevel(maxLevel);
				}
				player.getPlayerMechaCore().getEntity().notifyUpdate();
	 ...
	 * @param player
	 */
	private void heroRiseNode(Player player) {
		int threadNum = HawkTaskManager.getInstance().getThreadNum();
		HawkTaskManager.getInstance().postTask(new HawkTask() {
			@Override
			public Object run() {
				ConfigIterator<HeroRiseFragmentsCfg> riseFragmentsCfgs = HawkConfigManager.getInstance().getConfigIterator(HeroRiseFragmentsCfg.class);
				for (HeroRiseFragmentsCfg upcfg : riseFragmentsCfgs) {
					PlayerHero hero = player.getHeroByCfgId(upcfg.getHeroId()).orElse(null);
					if (hero == null || hero.getStar() < 5) {
						continue;
					}
					HeroRise rise = hero.getRise();
					HeroRiseNode node = rise.getNodeById(upcfg.getNodeId());
					if (node == null) {
						hero.getRise().nodeUnlock(upcfg.getNodeId());
					}
					node = rise.getNodeById(upcfg.getNodeId());
					node.techUpGrade(upcfg);
					hero.notifyChange();
				}
				
				PlayerHeroModule module = player.getModule(GsConst.ModuleType.HERO);
				module.heroRiseCheck(true);
				return null;
			}
		}, player.getXid().getHashThread(threadNum));
	}
