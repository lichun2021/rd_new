    ...
    public String action(Map<String, String> map, HawkScriptHttpInfo hawkScriptHttpInfo) {
        ...
        switch (opt) {
            ...
            case "saveCrossPresidentPlayer":{
                ...
                PresidentOfficier.getInstance().unsetOfficer(player.getId(), officerId);
            	...
            	return "saveCrossPresidentPlayer sucess:"+ guildId;
            }
            case "setCrossPresident":{
            	String currPresidentId = "";
            	CrossPlayerStruct player = null;
            	if(map.containsKey("guildId")){
            		String guildId = map.get("guildId");
                	player = RedisProxy.getInstance().getCrossGuildLeaderInfo(guildId);
                	currPresidentId = player.getPlayerId();
                    //上届国王Id
            		String lastPresidentId = null;
            		President president = PresidentFightService.getInstance().getPresidentCity().getPresident();
            		int currTime = PresidentFightService.getInstance().getPresidentCity().getTurnCount();
            		// 上一届没有国王
            		if(president == null){
            			president = new President();
            			president.setTenure(currTime);
            			PresidentFightService.getInstance().getPresidentCity().setPresident(president);
            		} else {
            			// 老国王相关信息
            			lastPresidentId = president.getPlayerId();
            			if (!player.getPlayerId().equals(lastPresidentId)) {
            				president.setTenure(currTime);
            			}
            		}
            		president.setPlayerId(player.getPlayerId());
            		president.setPlayerName(player.getName());
            		president.setPlayerGuildId(player.getGuildID());
            		president.setPlayerGuildName(player.getGuildName());
            		president.setTenureCount(currTime);
            		president.setIcon(player.getIcon());
            		president.setPfIcon(player.getPfIcon());
            		president.setPlayerGuildTag(player.getGuildTag());
            		president.setPlayerGuildFlag(player.getGuildFlag());
            		president.setLastPresidentPlayerId(lastPresidentId);
            		president.setServerId(player.getMainServerId());
            		// 存储国王信息
        		...
        		//老国王的作用号不需用同步.
    			OfficerEntity entity = PresidentOfficier.getInstance().getEntityById(OfficerType.OFFICER_01_VALUE); 
    			if (!currPresidentId.equals(entity.getPlayerId())) {
    				entity.setPlayerId(currPresidentId);
    				PresidentOfficier.getInstance().setOfficer(entity.getPlayerId(), entity.getOfficerId());
    			}
    			//王战结束的时候清理一下跨服国王信息
    			RedisProxy.getInstance().deleteCrossServerKing(GsConfig.getInstance().getServerId());
    			
    			if(Objects.nonNull(player)){
    				PresidentFightService.getInstance().getPresidentCity().pushCrossKing(player);
    				PresidentFightService.getInstance().getPresidentCity().pushKingInfo(player);
    			}
    			PresidentFightService.getInstance().getPresidentCity().loadCrossServerKing();			
                return "setPresident success";
            }
            case "loadCrossPresident":{
    			PresidentFightService.getInstance().getPresidentCity().loadCrossServerKing();			
                return "loadCrossPresident success";
            }
            case "clearPresident":{
                //清除国王
            	try {
            		Method method1 = HawkOSOperator.getClassMethod(PresidentOfficier.getInstance(), "onWarStart");
					method1.invoke(PresidentOfficier.getInstance());
					RedisProxy.getInstance().deleteCrossServerKing(GsConfig.getInstance().getServerId());
					PresidentFightService.getInstance().getPresidentCity().setPresident(null);
					PresidentFightService.getInstance().getPresidentCity().loadCrossServerKing();			
					LocalRedis.getInstance().updatePresidentDataByKey("president", ""); 
				} catch (Exception e) {
					e.printStackTrace();
				} 
				//清理掉国王
                return "clearPresident success";
            }
            case "addRewardFlag":{
            ...
            case "XQHXSEASONGM":{
                return XQHXSeasonService.getInstance().gm(map);
            }
            case "TBLYGM":{
            ...
            case "CROSSSEASON":{
             ...
             case "SearchPower":{
            	 String playerId = map.get("playerId");
                 Player player = GlobalData.getInstance().makesurePlayer(playerId);
                 String msg =  "去兵战力:"+player.getNoArmyPower();
                 return msg;
            }
            case "StarWarEmail":{
           	 String playerId = map.get("playerId");
                ...
                endMailContent.add(player.getMainServerId());
                endMailContent.add(guildObj.getName());
                
                endMailContent.add(player.getMainServerId());
                endMailContent.add(guildObj.getName());
                
                GuildMailService.getInstance().sendGuildMail(guildId, MailParames.newBuilder()
        				.setMailId(MailId.SW_WAR_ONE_END)
        				.addContents(endMailContent.toArray()));
                
                
                GuildMailService.getInstance().sendGuildMail(guildId, MailParames.newBuilder()
        				.setMailId(MailId.SW_WAR_TWO_END)
        				.addContents(endMailContent.toArray()));
                
    			endMailContent.clear();
    			endMailContent.add(player.getMainServerId());
                endMailContent.add(guildObj.getName());
        		GuildMailService.getInstance().sendGuildMail(guildId, MailParames.newBuilder()
        				.setMailId(MailId.SW_WAR_THREE_END)
        				.addContents(endMailContent.toArray()));
